...
Are CAPTCHA-gated and require an interactive browser session, out of reach
...
`x402-research.agfarms.dev` (with an "pending deploy" note) and a
...
Done and behaves under upstream failure.
...
### 1. X402-research-gateway

NOW LIVE at `https://x402-research.agfarms.dev`.
...
| Wallet (pay-to) | `0xa91115B1AB8412f380Fd62446F523559F668b96B` (from existing `.env.prod`, kept in place of the zero-address placeholder) |
...
The box uses **system nginx** (Ubuntu 1.24.0) as the edge, in place of Caddy as CLAUDE.md suggested. Caddy binary exists but no service is running; 80/443 are owned by system nginx. Updated pattern accordingly.
