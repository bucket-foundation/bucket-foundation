...
**Fee:** 0% on standard tier (no upfront, no per-donation cut; HCB relies on float + donor cover).
...
- [ ] Confirm physical address for application, recommend founder's home address in place of the AGFarms address (per nonprofit-application memo §6, keep COI separation)
