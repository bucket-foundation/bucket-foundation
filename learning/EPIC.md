...
The strongest learning system + the verifiable mastery map.
...
No learning product spans more than **two** of the four capabilities that
...
**Bucket targets all four at once**, unoccupied. The wedge user is the serious
...
The best resume you can have, because it's *evidence* rather than a claim:
 ...
 **verifier (employer) pays, never the learner**. So Bucket gives away the learning *and*
 ...
 (~85% of firms want it; ~0.14% of hires use it, no trusted instrument exists).
 The credential is also the strongest viral loop in the field, people self-broadcast
...
- **Diagnostic:** ALEKS-style binary-search-over-states (~10-25 questions) so experts start
 mid-graph instead of at "what is energy."
...
**Phase 1, Adaptive core** (make the engine strong):
...
- Public Mastery Profile page (map + rating + evidence) at `/m/<handle>`, **MVP**.
...
- "Unlocks →" impact line + post-mastery reveal animation.
