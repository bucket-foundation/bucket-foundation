# Bucket Learning — Unified UX Specification (Apple-grade)
...
## Design principles (Apple HIG → learning)
...
5. **Honest, not flattering** — progress is *informational, not controlling* (People + SDT). No fake "you're doing great." The graph lighting up *is* the reward.
...
## Onboarding (value before account — from Customer Success, screen by screen)
...
8. **Soft, skippable signup wall**, loss-framed: "Save your progress so tomorrow's review targets exactly what you got wrong" (the FSRS benefit, not a gate).

**Activation metric:** ≥3 nucleus concepts across ≥2 days in week 1 + a 3-day streak.

## The daily learning loop (the atom screen, screen by screen)
...
4. **Progress** — XP awarded inline before you leave, **mastery-weighted** (derive ≫ recall) so the gradient points at understanding, not streak-farming. The atom lights up on the branch map; the **"Unlocks →"** line shows what just became available (the leverage Anki/Duolingo lack).
5. **Optional share** — at the success peak, offer the shareable concept-art card (deep-links back to the atom = the growth loop).

## The nucleus map (the hardest UX problem — solved by curation, not physics)

- **Three concentric shells:** prerequisite (outer) → nucleus (mid) → frontier (inner), or rendered as rings by `shell`.
- **Encoding:** node *size* = centrality (Leverage Score), *color* = shell, *fill* = your mastery.
...
- **Always-useful local view:** tap a node → its neighborhood (requires ← node → unlocks), not the whole graph.
...
## Components & micro-interactions (the Apple-grade gate)

- 8-pt grid; 44pt min tap targets (24px WCAG floor, 44 for comfort); SF-grade type scale.
- Motion 200–500 ms with `prefers-reduced-motion` honored; feedback < 100 ms; system haptics on correct/incorrect/unlock.
...
## The load-bearing-art contract (People — binds the whole design system)
...
## Scholar / Studio (Whop's model, Apple's restraint)
...
## The 10 highest-leverage UX decisions (from Product, kept verbatim as the checklist)
1. Graph = map, path = route; the computed route is the zero-decision default.
2. Onboarding teaches a real atom + animates the unlock *before* signup.
...
7. The "Unlocks →" line + post-route unlock animation make leverage visible.
