# Bucket Academy
UX Craft & Design-Engineering Toolkit.
...
AI builder (Claude Code) produces premium UX by construction.
...
The **craft layer**: it does not re-derive the IA or the pedagogy, it specifies
...
> **Aesthetic guardrail (non-negotiable):** Bucket is *classical/editorial*, holding a deliberate distance from Silicon-Valley
...
> *restrained and inevitable*, holding back from bounce and play; the parchment must stay legible; the art and
...
### 1.1 The four schools of motion
...
| **Material 3 (Expressive)** | Migrating from duration-based to a **spring physics** motion system; spring tokens come in *spatial* (movement) and *effects* (color/opacity) families, each with fast/default/slow; **emphasized** easing for begin-and-end-at-rest moves, **standard** easing for most UI. ([M3 easing & duration](https://m3.material.io/styles/motion/easing-and-duration), [M3 motion overview](https://m3.material.io/styles/motion/overview/how-it-works)) | The *token* discipline: separate **spatial** vs **effects** motion tokens; emphasized easing for the opening/celebration moves, standard easing for routine transitions. | Material's bouncy "expressive" personality, too playful for parchment. |
...
for the two "earned" moments (opening, celebration).* This reconciles UX-SPEC's 200-500ms band with
Emil's stricter < 300ms rule: **< 300ms is the default; 400-500ms is reserved for celebration/opening
choreography only**, because those are rare and deserve weight.

### 1.2 The Bucket easing & duration token set
...
### 1.3 Motion mapped to each Bucket surface
...
| **Depth tabs** (`.depth-tabs`, Feynman→Formal→Derivation) | Switch depth | Cross-fade content + a 240ms slide of the active-pill background (`transform`, `--ease-in-out`); content height auto-animates (auto-animate lib, §3). | light impact | Progressive disclosure should feel like *unfolding*, well clear of a jump-cut. |
| **Opening animation** (the Bucket-unique moment) | Atom mastered → downstream nodes light up | **The earned moment.** On the MAP: target node `node-fill` fills (scaleY mask 0→1), then edge `stroke-dashoffset` draws trigger→target, then target `node-base` stroke-width pulses once. Choreograph in sequence, `var(--ease-emph) var(--dur-unlock)`, ~120ms between steps. | medium impact + warm "opened" tone | This is the edge no competitor has (UX-SPEC #7). Give it weight (460ms) and sequence, it must read as *cause → effect*. |
...
| **End-of-route celebration** (`.celebrate`) | Route complete | The **second earned moment**: `pop` glyph (already in `app.css`) + a *restrained* particle burst in gold/laurel (tsParticles confetti, §3), sparse and classical. | success | Peak moment; parchment restraint means *a few gold motes*. |
...
### 1.4 Celebration restraint
The Bucket rule.

Duolingo celebrates *constantly*; that erodes meaning and conflicts with the "informational progress" principle
(UX-SPEC principle 5). **Bucket has exactly two earned motion moments**, the **opening** (a node lit,
...
## PART 2
DESIGN SYSTEM & COMPONENT QUALITY.

### 2.1 What "Apple/Linear-grade" means
...
A typographic scale built on a ratio instead of arbitrary px; **(d)** contrast that meets WCAG AA (4.5:1
...
### 2.2 Audit of the live `app.css`
What's already excellent.

`app.css` is strong and on-aesthetic. Credit where due:
- **Color tokens are semantic and complete**, ground/ink/rules/accents/shells all named, with no hex-in-place.
...
9. **Density: a `--density` line-height pair.** Body `line-height:1.55`/`1.62` is right for reading; lists
 (`.route-row`, `.mrow`) could tighten to 1.4 for scan-density. Two tokens instead of one global value.
...
### 3.4 Graph / network visualization

| Library | Repo | What / Why | License | When to use |
|---|---|---|---|---|
| **Cytoscape.js** | `cytoscape/cytoscape.js` | Graph rendering **with built-in graph algorithms** (PageRank, betweenness, i.e. our *centrality* computation), rich layouts, gestures. ([graph viz comparison](https://www.pkgpulse.com/blog/cytoscape-vs-vis-network-vs-sigma-graph-visualization-javascript-2026)) | MIT | **Recommended for the MAP** when we need analytics + interaction at biophysics scale. Use a *curated/preset* layout, keeping clear of force-directed (avoid the Obsidian hairball, UX-CASE-STUDIES §7.1). |
...
### 3.5 Gesture
Particles, skeletons, the small stuff.

| Library | Repo | What / Why | License | When to use |
|---|---|---|---|---|
| **@use-gesture/react** | `pmndrs/use-gesture` | Reliable pointer/drag/pinch/wheel hooks; pairs with Motion for spring-back. | MIT | Pinch-zoom L1↔L2 on the map; drag on the share card; any custom gesture Vaul doesn't cover. |
| **tsParticles** (`@tsparticles/react`) | `tsparticles/tsparticles` | configurable confetti/particles/fireworks. ([repo](https://github.com/tsparticles/tsparticles)) | MIT | The **restrained** end-of-route gold/laurel motes (§1.3). Configure *sparse* and classical. |
| **canvas-confetti** | `catdad/canvas-confetti` | Tiny "on-demand confetti gun." ([repo via search](https://github.com/catdad/canvas-confetti)) | ISC | Lighter alternative if tsParticles is overkill, single celebratory burst on the first atom opened. |
...
### 3.6 Design-engineering reference repos
...
### 4.2 The Motion / Spacing / State Cheat-Sheet
...
### 4.3 The Design-Engineering Playbook
...
## PART 5
PRIORITIZED UX-UPGRADE PLAN FOR BUCKET ACADEMY.

Ranked by **impact ÷ effort**, each tied to a file/surface. (L/M/H = effort.)

| # | Upgrade | Surface / file | Effort | Why it's high-impact |
|---|---|---|---|---|
| 1 | **The opening animation**, node fills, edge draws trigger→target, pulse, as a real GSAP/Motion sequence | `.graph`/`.node` in `app.css` + map component | M | The one thing *no competitor has* (UX-SPEC #7). Makes reach visible; the core "why am I learning this" payoff. |
| 2 | **Add the motion token block + retoken** ad-hoc curves/durations | `:root` in `app.css` (§1.2) | L | Enables consistency for *every* subsequent animation; one small diff, system-wide payoff. |
...
| 9 | **XP roll-up + restrained gold-mote celebration** (number-flow + sparse tsParticles) | `.celebrate` + XP component | M | Duolingo's dopamine coupling, on-aesthetic (classical motes, no confetti cannon). |
