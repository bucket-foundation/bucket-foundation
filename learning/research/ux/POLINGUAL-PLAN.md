...
## 0. Ground truth — what the substrate ACTUALLY is today (measured, not claimed)
...
- **Model:** **`sentence-transformers/LaBSE`** (Language-agnostic BERT Sentence Embedding, Feng et al. 2020,
  ...
  cross-lingual*. The current 384-d vectors are almost certainly `bge-small-en` (English-only, per
...
- **What to embed:** The **gloss**, not the surface. Embed the **first sense's `meaning_en`** (the
  primary gloss), not the full ` · `-joined blob — concatenated senses blur the centroid. Store one
  vector per *primary sense*; emit extra sense-photons for highly polysemous words (PHOTON-SPEC open
...
### 1.2 Phonetic / sound — "words that sound like X" (the 64-d vectors)
...
### 1.3 Orthographic / spelling — "words spelled like X" + a spelling↔sound axis
...
- **Algorithm:** (a) **Edit distance** — `rapidfuzz` (MIT) Damerau-Levenshtein, top-K by normalized
  ...
  *phonetic* representations: align surface graphemes to IPA segments, score grapheme-phoneme regularity.
  Surfaces a per-language "orthographic depth" number (English deep, Spanish shallow) — a genuinely novel
  Polingual view.
...
- **Query:** "etymology of *entropy*" → `ἐντροπή (grc) → entropie (fr) → entropy (en)`; and the inverse:
  "all descendants of PIE `*lewk-`" → the cognate tree (light/lux/leukos/licht…).
...
### 3.2 The learn-a-language drill — usability fixes (the toy → real)
...
## 4. Academy integration — one substrate, two surfaces
...
- **What "every language" really costs:** Kaikki covers ~1000+ langs but quality is power-law — ~50 langs
  are rich, the long tail is sparse inflection tables. "Every language" = ingest-on-demand of the long
  tail, **not** pre-build everything. The honest target is **35 rich langs deep + lazy long tail.**
...
- This already beats the current state by a mile and is a genuinely useful product.

**What "every language + full dictionaries" actually requires (NOT this quarter):**
- ANN infra (FAISS/hnswlib) + sharded storage for 3M+ photons; sense-disambiguation (polysemy) at scale;
  ...
  program, not a sprint. **Don't promise it on the landing page.** Promise "27 languages, growing."
...
## Appendix — measured facts (for the next builder, so they don't re-discover)
