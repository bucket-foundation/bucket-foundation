# Bucket Academy — Graphics, Rendering & Mobile-Performance Research
...
and fast on phones*, grounded in how the app is actually built today and in the binding
...
- **Apple-grade motion** — 200–500 ms, feedback < 100 ms, `prefers-reduced-motion` honored,
  haptics on correct/incorrect/unlock, all four states designed.
...
- `app.js` `screenMap()` — an **SVG** concentric-shell graph: 3 rings, nodes placed by angle,
  `requires` edges as `<line>`, node size = leverage, fill-radius = mastery. ~150–360 atoms
...
## 1. Web rendering tech for our needs (SVG vs Canvas vs WebGL vs CSS)

### 1.1 The decision, grounded in numbers
...
quickly… past a point the DOM becomes your tax bill," Canvas beats SVG on large/animated
...
**The strategic caveat that actually decides it for us** (yWorks again, and it is *exactly*
...
graphs "provide very little real value." Our spec already mandates this: **curated concentric
...
- **Full-branch nucleus map (`screenMap`)** — today ~150–360 nodes + their `requires` edges.
  ...
  of what stays buttery if we animate every node (pan/zoom/physics-y entrance). The honest
  ...
  2. **Canvas for the overview + SVG/HTML overlay for interaction** — the widely-recommended
     hybrid: Canvas draws edges/nodes in one paint; a thin SVG (or absolutely-positioned HTML)
...
- **Ambient brand texture** (parchment grain, shell auras) — **CSS gradients / CSS Paint API
  (Houdini)**, not images — see §2.3.

### 1.3 Do we need a graph-viz library? Mostly no — and that's a feature

Our map is *curated layout* (we compute ring x/y ourselves), not force-directed, so the
heavy graph engines solve a problem we deliberately don't have. The library landscape, for the
...
- **Sigma.js** (MIT, WebGL) — the performance winner at **100k+ nodes**; "considerably quicker"
...
- **Cytoscape.js** (MIT) — the best *small-graph* choice: built-in graph algorithms, many
  ...
  haven't hand-laid), this is the one to reach for. **Candidate, not needed yet.**
...
- **D3** (ISC) — not a renderer; `d3-force` for layout, `d3-scale`/`d3-shape` for the math.
...
## 2. Generative / procedural concept art (the load-bearing-art anchor)
...
- Diffusion models "still struggle to faithfully capture factual properties… often generating
...
- For *educational diagrams* specifically: "potential problems such as hallucinations present
...
### 2.2 The recommendation: deterministic, build-time, procedural **SVG** figures

The anchor should be a **figure**, not a "render": a deterministic, parameterized SVG that
...
- **Paper.js** (MIT, [paperjs.org](https://paperjs.org/)) — full vector scene-graph + bezier
  boolean ops on Canvas; heavier, reach for it only for genuinely complex constructions.
- **Zdog** (MIT) — pseudo-3D round/flat illustration on Canvas *or* SVG, extremely light;
...
### 2.3 Ambient texture without images: CSS gradients + CSS Paint API (Houdini)

The parchment grain, the shell-tinted auras behind the art card, and the map's ring glow
should be **drawn, not downloaded**. The **CSS Paint API (Houdini paint worklet)** lets us
...
### 2.4 Equation-to-figure rendering (the workhorse for §2.2 tier 1)

For plotting the actual function an equation describes, use a **tiny SVG plotter**, not a
charting monolith:
...
- For the cleanest result and total brand control, **author a ~80-line SVG path generator
  ...
  scale this is genuinely the right amount of code and keeps zero deps. Mafs/Desmos-grade
  interactivity is *not* needed for a static anchor figure.

### 2.5 The build-time pipeline (cheap, cacheable, offline)
...
- **Quality gate:** because output is deterministic and inspectable, the 8-point load-bearing
  ...
  spot-review — something you simply cannot do reliably on diffusion output.
...
## 3. Mobile rendering & performance (make it feel native)
...
75th percentile — and the score that matters is **CrUX field data, not Lighthouse** (use
...
- **`will-change` sparingly.** It promotes an element to its own layer ahead of time, but
  ...
  Add it only to the element about to animate (the unlocking node, the sheet), remove after.
...
### 3.2 Lazy rendering of long study chapters (`screenStudy`)
...
the glyphs we actually render (display caps + math/mono), `<link rel="preload">` the body
...
### 3.6 PWA polish (install, safe-area, haptics, offline)
...
- **Haptics — the iOS gotcha.** `navigator.vibrate()` works on **Android Chrome** but **iOS
  ...
  trick on iOS, no-op otherwise. Wire to correct/incorrect/unlock per the UX spec.
...
## 4. The GitHub / library toolkit (license · when-to-use)
...
**Graph / map**
- **(none — bespoke SVG)** — current scale; zero deps, crisp, accessible. *Default.*
...
- **(bespoke ~80-line SVG path generator)** — *default* for the anchor's plotted figures.
...
- **web-vitals** (Google, Apache) — measure INP/LCP/CLS as field data, not Lighthouse guesses.
...
| **2** | **Lazy study rendering: `content-visibility: auto` + per-block KaTeX via `IntersectionObserver`** | `screenStudy()` in `app.js`, `.study-block` in `app.css` | Removes the app's biggest INP/scroll risk (360 blocks + full-tree KaTeX). Often janky→instant on phones. Directly buys INP ≤ 200 ms + 60 fps scroll. | **M** |
...
| **4** | **Cross-platform `haptic()` helper** (Android `vibrate` + iOS `<input switch>` trick) wired to correct/incorrect/unlock | new `app/js/haptic.js`; `drill()`, unlock, `next()` in `app.js` | Closes a named UX-SPEC gap; the single biggest "feels native" delta on iOS, where vibration is otherwise impossible. | **S** |
| **5** | **GPU-only animation audit** — convert `width`-animating bars to `transform: scaleX()`, scope `will-change` to the animating element, ensure all motion is transform/opacity | `app.css` (`.mbar i`, `.bar .fill`, `.node`, sheet), `app.js` | Guarantees 60 fps everywhere; cheap, broad. Foundation for any richer map/unlock animation. | **S** |
| **6** | **Map polish + entrance/unlock animation (still SVG)** — animate only the unlocked node + tapped neighborhood (transform/opacity), add labels-on-focus, focus ring, `aria`, and the **local-neighborhood tap view** the spec mandates | `screenMap()` in `app.js`, `app.css` | Makes the map feel alive and *useful* (local view) without a graph engine; delivers the "post-route unlock animation" + "Unlocks →" leverage from Phase 4. | **M** |
| **7** | **Equation→figure plotting for equation atoms** — bespoke ~80-line SVG path generator (fallback to function-plot for hard cases), feeding upgrade #1 | `tools/art/build-art.mjs`; corpus | The most pedagogically load-bearing art for the majority (equation) atoms — *shows* the relation, dual-coding done right. | **M** |
...
- **Map:** **bespoke dependency-free SVG** at current scale (curated concentric shells, never
...
(SVG authoring; +`Zdog` for motifs) · bespoke SVG plotter (+`function-plot` for hard math) ·
...
6. **Map entrance/unlock animation + local-neighborhood tap view** (SVG) — `screenMap()` — **M**
