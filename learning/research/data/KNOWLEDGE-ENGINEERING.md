...
The Science of the Knowledge Layer.
...
## 0. The five layers and their chosen algorithms
...
| **2. Nucleus** | Which atoms ARE the high-reach core? | **Reach score** = personalized PageRank on the `unlocks` graph, gated by topological order + k-core, blended with reachable-frontier count | Captures "how much downstream depends on this" the way PageRank captures fundamentality in course-prereq networks |
...
### 1.1 Why spacing + active recall at all
...
- **The spacing effect**, distributing study over time beats massing it. Cepeda et al. (2006)
 meta-analysis: spacing increases retention ~10-30%.
...
(how encoded) from **retrieval strength** (how accessible now). A review done when
...
Leech detection. Good engineering, same theoretical ceiling: a heuristic interval multiplier, one step short of a
...
### 1.3 The FSRS memory model, made precise
...
6. **Anti-pattern guard:** never show the answer before requiring an active-recall attempt; FSRS
 grades are meaningless without retrieval. The four ratings map to our quiz grader:
 Again = wrong, Hard = correct-but-slow/partial, Good = correct, Easy = correct-and-instant.

---

## 2. The nucleus computation
...
- **V** = Concept Atoms.
- **`unlocks` edge** A → B means "mastering A opens B." This is the **reach** direction
...
| **Reachable-set size** (transitive closure on `unlocks`) | total # of downstream atoms this *eventually* opens | The purest "reach" number, how much of the field collapses if you know this |
| **PageRank** (on `requires`-direction, i.e. Importance flows to prerequisites) | "fundamentality", an atom is important if important atoms depend on it; recursively | The headline nucleus ranking (validated meaning in course-prereq nets: PageRank surfaces the foundational, introductory nodes) |
...
| **In-degree** (on `unlocks`) | how many prerequisites an atom has | A *difficulty/readiness* signal, distinct from reach |

Why PageRank: in course-prerequisite networks, "the importance of a course depends not
...
**Step B, Compute the Reach Score per atom** (the nucleus ranking):
```
...
- `reach(v)` = size of the transitive `unlocks`-closure of v, normalized by |V|, "fraction of the
 branch this single atom opens downstream."
...
- **Default weights: α=0.45, β=0.30, γ=0.15, δ=0.10.** These are a *prior* open to tuning,
 against a held-out "which atoms did exam questions require" signal once we have one (the
 kill-criterion: if reach ranking doesn't correlate with exam-question frequency, re-weight).

The **nucleus shell of a branch = the top atoms by reach** that the syllabus tags `[N]`. The
score is what decides *which* of the candidate atoms get promoted into the curated nucleus and in
what priority, it operationalizes "smallest set of load-bearing ideas that open the most."

**Step C, Generate the optimal learning path** (reach-weighted topological order):
...
- `depth_discount` mildly prefers shallower atoms first so prerequisites land before their dependents
 even among equally high-reach options.
This yields a path that is **valid (never teaches B before A) AND greedy-optimal for reach**
(front-loads the concepts that open the most). It is the data behind Duolingo's "skill tree," but
...
- **Anchor-constrained, provenance-tracked extraction** is the current best practice: every extracted
 ...
 makes it impossible to verify extraction faithfulness." ([Grounded KG Extraction via LLMs, anchor-constrained + provenance, MDPI Computers 2026](https://www.mdpi.com/2073-431X/15/3/178))
...
5. **Edge cross-check.** Each LLM-proposed `requires` edge is cross-checked against (a) the
 ...
 exist / is it plausibly upstream?). Disagreements are flagged for review.
...
### 3.3 How we know an atom is correct
...
- **BKT, Bayesian Knowledge Tracing** (Corbett & Anderson 1994). A 2-state Hidden Markov Model per
 ...
 battle-tested* (pyBKT)., BKT has a **proven formal link to IRT**: the stationary
...
- **DKT, Deep Knowledge Tracing** (LSTM). +up to ~25% AUC over classic BKT on benchmarks by
 ...
 opens what*. ([DKT gains + interpretability cost](https://arxiv.org/pdf/2105.06266); [explainable-KT survey, arXiv 2403.07279](https://arxiv.org/pdf/2403.07279))

### 4.2 RECOMMENDATION

A layered mastery model.

**Interpretability is a hard product requirement** (PRODUCT.md: "you always know *why* you're
Learning this and *what it opens*"; analytics surface a mastery heatmap). So:
...
### 4.3 The "what to study next" recommender
...
- **frontier_value** = for atoms on the KST outer fringe (prereqs satisfied, still untouched),
 `reach(atom)` from §2, pick the highest-reach *ready* new concept (front-load the nucleus).
...
FSRS + BKT state, one step short of a forecast model. This is the Pro "projected exam-readiness date."

### 5.4 The gap-report
After each session (and the Exam-Simulator), emit a **gap-report**: the weakest atoms (low BKT, low
FSRS R), the *un-mastered prerequisites blocking high-reach frontier atoms* (the highest-ROI
Fixes, unblock a high-reach atom and its whole reachable set opens), and any atoms decaying
...
### 5.5 Brain-feed metrics
Per learner: atoms-mastered, nucleus-coverage %, mean stability, review-load/day, projected
ready-date. Per branch (content health): atom count, mean reach of nucleus, extraction
...
- **P0 (now, days):** `engine/fsrs.py` (FSRS-6, default weights, target 0.90) + `engine/nucleus.py`
 (reach score + path gen on the biophysics atom graph) + a terminal quiz loop with a BKT
...
2. **Reach-score weights (α,β,γ,δ) are a prior**, unfitted, we need a held-out "exam questions
 required these atoms" signal to validate the nucleus ranking. Until then, treat the
