# Adaptive Learning — State of the Art (the technical architecture that makes it strong)
...
document goes *deeper* on the five things that decide whether the learning system is genuinely
...
| **(e) Map computation/viz** | Leverage Score; react-force-graph; concentric shells | **CORRECT the PageRank orientation bug**, **ADD Borda rank-aggregation**, **ADD topological stratification + Sugiyama layout** as the hairball fix, and the library/scale matrix. | High — directly from course-prerequisite-network papers |
...
## (a) The knowledge-graph curriculum + the diagnostic

KNOWLEDGE-ENGINEERING.md §2 already nailed the *nucleus* (Leverage Score) and the *ordering* (KST
fringe walk). Two things were missing, and both come straight from production systems.

### a.1 Make the graph two-layered (Math Academy's key move)
...
not hand-filled. And crucially: *"a graph does not have to be fully encompassed for its efficiency to
...
For our biophysics pilot (~40–400 atoms) this is hours, not months.
...
### a.2 Validate edges against learner data, not just expert intuition
...
marks *in-state* should be answered correctly ~0.8 of the time, *out-of-state* ~0.1, and *boundary*
...
validator for the Leverage-Score weights (the DECISIONS.md open question #1): once we have a question
bank, the same held-out signal tells us whether leverage ranking predicts exam-question frequency.

### a.3 The diagnostic: binary-search over knowledge states (the biggest single gap we're filling)
...
— "the assessment quickly obtains a fairly accurate picture, then fine-tunes." So we can place a
learner among an astronomical number of states in **~10–25 questions**.

**Slip/guess robustness (cheap, high-ROI — copy all three):**
...
### a.4 What-to-learn-next = the outer fringe, refilled after every mastered atom
...
*which* fringe item to pick is decided by the Leverage Score (front-load the nucleus), tie-broken to
...
## (b) The mastery model — fusing proficiency + retention into a credential-grade score
...
### b.1 The proficiency stack (three layers, each doing one job)
...
1. **Elo with an uncertainty function = online IRT (the live estimator, solves cold-start).** Treat
   ...
   b := b + K·(P − correct)
   ```
   Pelánek shows Elo is literally **single-pass SGD on the same log-likelihood IRT maximizes**, so it
...
3. **AKT (Rasch-embedded attentive KT) = the predictor / item-selector, NOT the certified number.**
   ...
   **DKT (0.817) actually beats SAKT (0.752) on ASSISTments2009**
...
### b.2 Retention is a *different* signal — keep FSRS, and use R(t) explicitly
...
### b.3 The fusion formula (our synthesis — conjunctive, conservative, retention-gated)
...
- **Proficiency** `P_k = σ(a_k·(θ_k − b*_k))`, evaluated at the **minimal-competency difficulty** `b*`
  ...
  `θ_k − z·SE(θ_k)` — so we certify what we are *confident* they know, not a lucky point estimate.
...
AND still retrievable at horizon T." **Aggregate to the credential conjunctively**, not by averaging
...
### b.4 What a credential legally needs (the validity/reliability evidence)
...
α-reported score is what lets a Bucket credential actually mean something.

---

## (c) Assessment generation + anti-gaming
...
### c.1 Generating valid items (not recall)
...
- **Force higher-order cognition.** LLMs reliably hit Remember/Understand and degrade at
...
- **Distractors from student-error models, not free generation.** "LookAlike"/student-choice methods
...
### c.3 The anti-gaming stack (what makes the mastery score un-farmable)

Layered detectors (each a cited, deployed method) + design rules:

| Attack | Counter | Source |
|---|---|---|
| **Rapid guessing** | **NT10** response-time threshold (10% of item's mean time, capped 10s) → **Effort-Moderated IRT** drops rapid-guess responses from the ability estimate, so guessing literally cannot raise the score | [NT thresholds](https://link.springer.com/article/10.1186/s40536-021-00100-w); [Effort-Moderated IRT](https://www.researchgate.net/publication/229985871) |
...
## (d) Trustworthy AI content generation (lessons + problems)
...
Canon-only retrieval is the **firewall**: the generator literally cannot ground in unverified
...
### d.2 Verification + eval stack (concrete metrics + thresholds)
...
**CI harness:** a golden set of expert-vetted (topic, content, context) cases; the build **fails** if
...
### d.3 The 4-tier provenance model (keeps AI content out of canon)
...
## (e) Map computation + visualization

KNOWLEDGE-ENGINEERING.md §2 + DECISIONS.md #9 already chose the Leverage Score and react-force-graph.
The deeper pass surfaces **one bug to fix, one method to add, and the hairball fix**.

### e.1 The orientation bug (fix this first)
...
KNOWLEDGE-ENGINEERING.md §2.2 said "PageRank on the `requires`-direction" — that's correct *if*
...
### e.2 The four measures + Borda aggregation (replaces the hand-set α,β,γ,δ weights)
...
- **transpose-PageRank @ α=0.85** = recursive fundamentality (favors the deepest foundations); use
  **personalized PageRank** (teleport vector on the learner's goal atoms) for learner-relative leverage;
- **reach** = BFS transitive-closure size = "learn this and N total atoms become reachable" (the most
  intuitive leverage metric; [Applied Network Science 2024](https://link.springer.com/article/10.1007/s41109-024-00637-z));
...
measure → Borda points → sum), using **Trimmed/Robust Borda** for noise resistance
...
Rank aggregation is parameter-free and robust to any one measure misbehaving — strictly better than
guessing four weights. Keep **k-core** only for *visual shells*, not ranking (real prerequisite DAGs
...
fix and is often a *better deliverable* than a brittle single line — and it **reveals hidden
...
### e.4 Visualization — the hairball fix + the library/scale matrix
...
strata as headings, atoms as keyboard-navigable list items with "prerequisites: …" / "unlocks: …"
...
## Build-order delta (vs KNOWLEDGE-ENGINEERING.md §6 / DECISIONS.md build sequence)
...
- **P1:** **ADD the ALEKS state-search diagnostic** (`engine/diagnostic.py`) and the **Elo online
  estimator** (replaces BKT as the live proficiency number); Borda leverage aggregation; Sugiyama
...
## Honest gaps / kill-criteria

1. **The fusion formula `M_k = P_k^α·R_k^β` is our synthesis, not a cited result.** It's principled
...
4. **Diagnostic state-search assumes a clean DAG;** condense SCCs first (§e.3) or the state space is
   ill-defined.
5. **LLM graders/judges are foolable** — the adversarial-canary CI (§c.2) is mandatory, not optional;
   if a canary ever passes, the credential gate is compromised until re-tuned.

---

## Primary sources (new in this doc; see KNOWLEDGE-ENGINEERING.md §9 for FSRS/KST/extraction)
