...
> what you know**, grounded in Bucket's canon, measured by mechanisms that are
> *impossible to increment except by learning*, and anchored (optionally) on Story
...
Stability** so "mastered" means "retained" (`DECISIONS.md` #11). That means the
number behind a Bucket Mastery Profile is *already* "you keep retrieving this against
...
Credential**, ahead of bolting a badge on after the fact.
...
first-class value is the single most credibility-defining UI choice, an credential
...
> **informational rather than controlling**, it must read as a *diagnostic* of command, never a
...
### 1.3 Depth: recency / evidence per concept
 ...
 **Derive** → **Teach-back**). *Derive* and *Teach-back* are the deep retrieval levels
 ...
 signal points at *understanding* over card-flipping.
- **Recency**, FSRS retrievability right now (a live forgetting-curve readout).
- **Evidence trail**, *this is the differentiator employers inspect.* Not "earned a
...
### 1.4 Durable mastery milestones
...
### 1.5 Machine-readable skill alignment
...
3.0 `Alignment` field. A profile is then *searchable and comparable* instead of a wall of opaque
...
## 2. How mastery is measured *credibly*
...
through?"** Bucket's answer is a stack, cheapest-and-highest-return first. None of it requires
Heavy surveillance.

### 2.1 The loop is already retrieval-with-feedback
...
"complete" an atom by scrolling; you raise mastery only by *retrieving* it. This is
the structural difference from a Coursera-style "completion certificate."

### 2.2 Adaptive: item-calibrated assessment / kills cram & key-leaks
...
### 2.3 A continuously-earned rating beats any one-time test
...
Fused with **FSRS spaced verification**, the rating is "you re-demonstrated this at
...
([sandbagging/boosting detection](https://www.chess.com/forum/view/site-feedback/sandbagging-control)). <!-- voice-ignore-line: cited link title and URL (chess sandbagging/boosting) -->
...
### 2.4 "How do I know they didn't just click through?"

The explicit answer trail.

For each public mastery claim, the profile can surface (employer-gated, see §3):
1. **The retrieval/derivation transcripts** over a pass/fail, the *work*: their teach-back
 explanation, their derivation steps, graded by the canon-grounded tutor.
2. **Spaced verification history**, *when* and *how* they re-demonstrated it (FSRS),
 proving durability across time.
...
Work* rather than a token ([Open Badges 3.0 spec](https://www.imsglobal.org/spec/ob/v3p0), the
`Evidence` + `Result`/`ResultDescription` model captures *where on the scale* instead of binary
earned/not-earned).
...
### 2.6 Proctoring only as a *backstop*, only human-final
...
*structure of the measurement* rather than from watching people.

### 2.7 The "can't be farmed without learning" test
Every component that feeds the public rating must pass the rule already in `DECISIONS.md` #20:
**if it can be incremented without retrieval, it does not feed the rating.** Streaks,
...
Events over controlling rewards.)

---

## 3. How it's verified & shareable
...
credential standard, it's just a plain, useful public profile.
...
Story Protocol attestation.

The expert consensus: **off-chain VCs are the right default; put on-chain *only* the narrow
Functions where it beats a signed badge**, durable timestamp, cross-boundary
...
### 3.4 What an employer / PI clicks
...
3. Click **"Inspect evidence"** (learner-gated) → the *work trail*: derivation transcripts,
 ...
 resume offers**, the employer reads the actual proof.
4. (Optional) the rating's **RD/recency** tells them how *proven* and how *current* it is, an
 credential that flags its own staleness.

---

## 4. Why this beats a normal resume

| A normal resume | The Bucket Mastery Profile |
|---|---|
| **Self-asserted** ("proficient in X") | **Earned**, the score is inseparable from retrieval (§0, §2) |
| **Static snapshot**, instantly stale | **Continuous & live**, updates as you learn; recency is shown openly (FSRS) |
...
- **No credential standard yet**, this is "the best public profile," already a
...
- **→ Data (heaviest).** Owns the **IRT item-bank calibration** and the **Mastery Rating math**
 ...
 People supplies the *spec* (what makes the rating, the anti-gaming requirements);
...
- **→ Product.** Owns the **Profile / Knowledge Portfolio UI** (map encoding, the confidence-band
 ...
 constraint: the rating is **informational rather than controlling** (no leaderboard vanity), RD is
...
- **→ Revenue.** The Mastery Profile is a **free, mission-consistent acquisition + retention
 engine** rather than a SKU. Any external claim about what the rating *means* must cite the
 measurement without overclaim, same discipline as the efficacy-claim rule (cite replicated
...
- **→ Customer Success.** Onboarding must frame the profile as **"a credential you build by
 learning"** so the-progress UI (RD shown, recency decaying) reads as *integrity*, not
...
- [AI-proctoring bias, Watching the Watchers, arXiv 2205.03009](https://arxiv.org/pdf/2205.03009) ·
 [Sandbagging/boosting detection (chess.com)](https://www.chess.com/forum/view/site-feedback/sandbagging-control) <!-- voice-ignore-line: cited link title and URL (chess sandbagging/boosting) -->
