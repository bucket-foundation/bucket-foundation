...
Learning Science & AI-Tutor Safety.

**Pillar:** People · **Bead:** bkt-xo0 · **Author:** People pillar (Nucleus) · 2026-06-11
**Pilot domain:** biophysics general-exam prep
**Mandate:** *Not fast, correct and amazing.* This is the **evidence base** that
Makes Bucket Academy *teach* (not gamify-toward-nothing), plus the **AI-tutor
...
1. **Learning ≠ performance.** Bjork & Bjork's *New Theory of Disuse* (1992) splits memory
 ...
 methods that work feel worse than the methods that don't**, so learners choose
 wrong. This is the single strongest argument for *system-imposed* (algorithmic)
 scheduling and for ** progress signals.
...
2. **In a teaching product, a confident wrong answer is the worst failure mode.** RLHF
 ...
 calibration"). A fluent, confident, wrong derivation installs a durable
...
High-return techniques and how Bucket uses them.

### 1.1 The utility tiering
...
Two highest-return strategies, testing and spacing, are *underused*. **Bucket's core
...
184 articles**. Distributed practice beats massed; the central finding is an
**interaction**: the inter-study interval that maximizes retention *grows* as the retention
Interval grows. (Cite the interaction over a single "spacing %", which is condition-dependent.)
...
*engineering* benchmark on self-selected users rather than a randomized cognitive experiment,
Cite it as scheduling efficiency rather than a clean learning effect.
...
> retention target and gap schedule from the **time-to-exam** instead of a generic default, a
...
5 min restudy slightly wins (≈.81 vs.75); at **1 week testing wins (≈.56 vs
...
testing ≈ 33-36%**; continued *studying* of already-learned items did nothing.
...
**g ≈ 0.93**. Holds across format and level; strongest with feedback.
...
> *retrieval* over rereading, the atom's Feynman explanation is the *teach* surface, but the
...
> across a branch's nucleus. **Surface the illusion** (§4): tell the learner "this
...
> **desirable** zone: hard enough to force effortful retrieval, and never so hard that retrieval
...
### 1.6 Dual coding & multimedia

The scientific justification for AI concept ART.
 ...
 **distinctiveness** over literal dual coding drives it. This does **not** weaken the
...
- Harp & Mayer (1998), *How seductive details do their damage* (*J. Educational Psychology*
 ...
 solutions, by **priming wrong schemas** (active mis-structuring beyond mere distraction).
...
> relationship, the terms of an equation) over the topic's *vibe*. *"If I removed this,
...
> 4. **Spatial contiguity**, labels ON the relevant part of the art instead of in a separate
> legend.
> 5. **Signaling**, arrows/highlight/zoom direct attention to the load-bearing element;
> strip irrelevant detail.
> 6. **Distinctive over busy**, distinctiveness drives memory; clean beats ornate.
...
>
> **One-line thesis for the spec:** *generate diagrams of the idea, placed and
...
**replication caveat (use 2σ as a ceiling rather than a fact).**
...
81-112): feedback among the most powerful influences (synthesis ≈0.79σ), **but
 ...
 **>2× active-lecture**, in **less time** (median 49 min); effect ≈ **0.63** standardized
 (ceiling-corrected 0.73-1.3σ)., the AI was **prompt-engineered**
...
- **Design exemplar (not RCT-grade).** Khanmigo (Khan Academy) uses a Socratic,
 ...
 outcomes RCT yet**, treat as a design pattern rather than efficacy evidence.
...
> **hard requirement** rather than a style choice. Guard against the **"crutch"** failure mode.
> 3. **Mastery gating**, don't open the next atom until the current one is mastered
...
> is excellent, **not** the folklore 2σ. Set product expectations and any
> marketing claim to the *replicated* numbers.

---

## 3. MOTIVATION: Self-Determination Theory / gamification / the crowding-out risk
...
**What gamification buys.** Sailer & Homner (2020), *The Gamification of Learning: A
...
boosts motivation." Hamari, Koivisto & Sarsa (2014): effects are **context-dependent** <!-- voice-ignore-line: verbatim quotation from cited study -->
With **novelty effects that fade**. Ryan, Rigby & Przybylski (2006): games motivate when they
**satisfy SDT needs** (autonomy + competence) rather than when they dispense points.
...
Conversation**. The framing: **gamification optimizes the metric it rewards (daily
engagement), which is not the same as the learning goal.**

> **Bucket mapping, gamify the *needs* over the *metric*.**
...
> - **Competence:** make XP/progress **informational rather than controlling**, show *what you can
> now do* and *what it unlocked downstream on the graph* (mastery signal), and lean
...
> competence-signaling milestone) over raw daily app-opens.
...
> claim on §1, §2 rather than on streaks.

---

## 4. METACOGNITION: calibration / the illusion of knowing / progress
...
** retrievable; restudy inflates confidence without diagnosticity. A **failed
Retrieval attempt directly reduces overconfidence**, feedback restudy never delivers
...
> **Bucket mapping, surface progress, never fake encouragement.**
...
> overconfidence, design the loop so a miss is framed as diagnostic signal instead of a
> shame-event.
> - **No fake "you're doing great."** Progress must be **diagnostic and true**: a mastery
> heatmap that shows real weak atoms, a forgetting-curve view, an **exam-readiness
...
Based, spaced** assessment that delivers **** competence signals. They converge on one
...
The floor engineering must implement.
...
"**, GPT-4 "can be confidently wrong … not taking care to double-check." Tian
...
**Grounding (RAG), necessary and insufficient alone.** Lewis et al. (2020, NeurIPS; arXiv:
...
**Evaluation suite (block deploys on regression).**
...
## EVIDENCE-BACKED FEATURE REQUIREMENTS
...
5. **The `art_prompt` contract (load-bearing art only).** Enforce the 8-point checklist in
 ...
 Adesope 2020.)*
6. **Mastery-gated, worked-examples-that-fade.** Open downstream atoms only on mastery;
...
7. **Socratic, answer-withholding tutor with step-level feedback.** Hints not answers; guard
 the "crutch" failure mode; target the realistic ~0.5-0.8σ ITS band over folklore 2σ.
 *(Kestin 2024/25; Bastani 2024/25; VanLehn 2011.)*
8. **Gamify the SDT needs.** Informational (not controlling) progress;
 ...
 **soften streaks** (freeze/repair, no loss-aversion pushes) and tie rewards to mastery
 events over app-opens. *(Ryan & Deci 2000; Deci/Koestner/Ryan 1999 d≈−0.3 to −0.4;
 Sailer & Homner 2020.)*
9. **progress & calibration.** Pre-retrieval confidence prompts + predicted-vs-actual;
...
S2. **Retrieval precision: rerank, tight context, edge-placement, relevance threshold.**
 Weak/empty retrieval triggers **abstention** instead of a guess; explicit "ignore irrelevant
...
S5. **Atomic-claim eval suite in CI; block on regression.** FActScore-style %-supported-by-
...
S7. **Never trust the model's own fluency/confidence.** RLHF broke calibration, verification
 is external, layered, and enforced **in code** rather than in the system prompt. *(GPT-4 card;
 Huang 2025.)*

## CROSS-PILLAR DEPENDENCIES

- **→ Engineering:** owns S1, S7 (RAG pipeline, reranker, citation validator, abstention
 gate, FActScore/TruthfulQA eval suite in CI, per-turn safety re-checks) and the FSRS
...
- **→ Data:** the **human-verified canon corpus + canon index** that S1/S3 ground against
 ...
 optimization and the mastery/forgetting model behind progress (req. 9). People needs
...
-progress UI and harder-feeling drills don't read as the product being broken.
