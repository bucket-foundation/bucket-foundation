...
Onboarding, Retention & Community.
...
**We are a Facilitator.** Nir Eyal's *Manipulation Matrix* asks two questions
...
gives us a hard test for every feature: **if a mechanic only lifts our metrics without
...
## 1. Case study, Duolingo onboarding

Duolingo's onboarding is treated as the gold standard because it inverts the normal
...
| 8 | **THE FIRST LESSON, before signup** | A real, completable lesson with instant feedback and a celebration | The aha moment. Value is *delivered* up front. Now the user has progress (XP, a nascent streak) to protect |
| 9 | **Notification opt-in** | Asked *after* the win, framed as "Duo will remind you" | Timed to peak goodwill, opt-in rates are far higher right after a success than at cold start |
...
3. **Prior-knowledge fork is critical for us.** Our users skew *expert-adjacent* (Gian is
 ...
 strong learner mid-graph** instead of at "what is energy." Boredom is our #1 churn risk with a
...
|---|---|---|---|
| **External trigger** | Push notification, email | A *well-timed* "X concepts due for review today" reminder | Healthy if it surfaces real, due FSRS reviews instead of fake urgency |
| **Internal trigger** | Boredom, FOMO, "I should study" | Curiosity, exam anxiety converted to agency, the polymath itch | We want the internal trigger to be **mastery-seeking** rather than anxiety |
...
Reported effects (vendor/case-study figures, directional, awaiting peer review):
...
1. **Streak counts *learning* rather than logins.** A day counts only if you complete your due
 FSRS reviews *with recall attempts* (not a 2-second skip). Tie the streak to the
 thing that's good for the user.
...
4. **Never weaponize the streak count for upsell.** Don't sell "streak insurance" that
 ...
 losing part of your identity", is precisely the pressure we must not monetize.)
...
### 2d. Notification strategy

The famous reminders.
...
- **Notify about real value.** "3 concepts are due, review them
...
- **Smart timing, opt-in, easy to dial down.** Adopt the bandit-style "send when *this*
 user studies", that's respectful (right message, right time). Offer
 granular controls and a one-tap "fewer reminders."
- **Encouraging, never shaming.** Tone = a good tutor rather than a passive-aggressive pet. Our
 mascot can be warm and funny without weaponizing sadness.
- **Cap frequency.** No nagging cascades. One well-timed daily nudge, max.

### 2e. Leagues

Leaderboards & loss aversion.
...
- Competition can corrupt the goal: people grind easy XP to climb instead of to learn (same
...
Variable reward, surprise & delight.
...
> Our mascot is a delighted fellow-scholar.
...
- **Variable reward:** opening a beautiful (occasionally rare/holographic) card per
...
*causally downstream of understanding*: you earn the art by mastering the concept; the
...
"share with friends," done healthily.
...
- Product lessons Duolingo published: the magic is **"social without interaction"**, "like
 ...
 that, ahead of power-user retention). And **57% of users already had a friend on-platform**,
...
### 4b. Bucket Academy's social design
...
| **Public knowledge portfolio** | Strava profile + GitHub contribution graph | Public profile showing mastered nuclei across branches, "the polymath flex" | Identity investment that's *true* (you mastered it) |
...
social mode (a reading group is how scholars have always learned foundations) and
...
**Viral mechanics, ethical version.** Share because the artifact is *good and
...
One-tap** and never coercive.

---

## 5. First-session → habit
...
Activate are more likely to retain."
...
D1 / D7 / D30 are the standard checkpoints; D7 and D30 reflect *habit formation.*
...
2. **Day 1-2:** FSRS surfaces yesterday's concept for review (spacing) + one new nucleus
 ...
 one-tap).
3. **Day 3:** 3-day streak → open the **Streak Society**-style milestone + first rare art
 ...
 *value demonstration over discounts.*
...
- **Bucket win-back sequence (value-led, ethical):**
 ...
 2. **Day 7:** "You mastered this, and here is what it opens next, [the graph image]." Show
 the portfolio they built; remind them of sunk value (true sunk value, earned).
 3. **Day 14:** a *new* hook, "[New frontier paper / a polymath bridge you haven't seen]."
 Lead with novel value.
...
AI-native help, FAQ & feedback at scale.

As an AI-native product we should run support the way 2025 best-practice prescribes:
**reduce friction**, with a **RAG (retrieval-augmented) help assistant** over
Our own docs/atoms.
...
## Manipulative-mechanic register
...
| Sharing/virality | Forced-invite friction, fake tag notifications | One-tap, delightful; share because the artifact is good |
...
better?* If no to either → it's the Dealer move → redesign or cut.
