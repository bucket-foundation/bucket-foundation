# Bucket Academy — Operations: Cost, Compliance & Risk
...
> One-line thesis: the variable cost of Bucket Academy is **AI image/art generation, not LLM tokens**.
...
|---|---|---|
| **LLM cost** | gpt-4o-mini / Haiku-class for gen + tutor; cache aggressively | ~$0.15/$0.60 per 1M tokens (in/out) — effectively rounding error per user once cached |
...
| **Accessibility** | **WCAG 2.2 AA** is a hard design requirement, not a nice-to-have | 4.5:1 contrast, full keyboard, SR semantics, reduced-motion, dyslexia mode |
...
## 1. AI Cost Model — the big variable cost

### 1.1 LLM token costs (current, 2026)
...
|---|---|---|---|---|
| Generate 1 atom's card set (recall+cloze+derive) | ~2,500 | ~800 | ~$0.0009 | **Done once, cached/stored — not per user** |
...
### 1.2 Image / ART generation — the cost wildcard
...
> **Concept art is a property of the *atom*, not of the *user*.** A biophysics nucleus has ~30–60 atoms per
...
> tutor/art endpoints). That is a *security + metering* problem, solved in §2 and §7, not a pricing problem.

### 1.4 Caching strategy + usage caps (the sustainability mechanism)
...
Caps are **budget policies**, not hard-coded app logic — they live in Viatika (§2) so they're auditable and adjustable without a deploy.

---

## 2. Viatika metering (vendor API — no duplicate ledger)

**Org policy (AGFarms CLAUDE.md, Strategic Priority #6 + Viatika architecture):** every metered AI/data
endpoint in any AGFarms venture **MUST call the Viatika vendor API**, not roll its own ledger. Viatika is a
...
2. Map the §1.4 caps to **Viatika `BudgetPolicy` rows** (per-user daily, per-tier monthly, per-org global) — not app constants. Free vs Pro = different Cedar policy + budget tier.
...
## 3. Copyright compliance — where the lines are
...
### 3.3 User PDF-import — the precise legal boundary
...
5. **Extraction = facts, not expression** — the cards generated are the user's private study aids built from facts; we don't republish the book.
...
### 3.5 AI-generated art copyright (US, settled 2026)
...
- For **Story Protocol minting** of user contributions (§5): a mintable IP asset needs **meaningful human authorship** (the human's selection, arrangement, derivation text, creative control) — not a bare AI image. Disclose AI-generated portions. This shapes what's eligible to mint (the human's *atom/derivation*, not the auto-art).

---

## 4. Privacy — COPPA, FERPA, GDPR + data-minimization

Bucket targets EU learners and may have student/minor users → **all three regimes are in scope.**

### 4.1 COPPA (US, under-13) — deadline already passed (2026-04-22)
...
### 4.3 GDPR (EU learners)
- **Legal basis** — pick and document one of the six before processing. *Contract* (delivering the learning service) for core account data; *consent* (freely given, specific, informed, withdrawable as easily as given) for anything optional (marketing, non-essential analytics).
...
- **Special-category data** — disability/health/SEN data is sensitive. A **"dyslexia-friendly / accessibility needs"** toggle (§6) could reveal a disability → store it as an opaque UI preference, not a health record; don't profile on it.
...
### 4.4 Data-minimization for learning records (cross-cutting)
...
category." Principles:
- Store the **minimum** needed for the scheduler (per-atom stability/difficulty/retrievability + review timestamps) — that's a compact numeric model, not a transcript archive.
...
## 5. Content moderation — UGC atoms/decks (Scholar tier)
...
1. **Quality / canon-fit** — does it meet Bucket's "foundations only" bar (axioms, real math, laws, primary derivations — not outcomes/opinion)? Route through the existing **Longtail chisel review queue** (`longtail.agfarms.dev/chisel`, already wired for Bucket grant drafts) for fast yes/no/unsure on quality axes. Reuse, don't rebuild.
...
## 6. Accessibility — WCAG 2.2 AA (part of Apple-grade UX)

The founder wants Apple-grade UX; **accessibility is a pillar of that, not an add-on.** Target: **WCAG 2.2 Level
...
**Operable**
- **Full keyboard operability** — the whole learning loop (flip card, grade recall, navigate skill tree, send tutor message) works without a mouse; logical focus order; no keyboard traps.
...
- **Dyslexia-friendly mode** — optional dyslexia-friendly font, increased letter/line spacing, off-white background, left-aligned text. Store as a *UI preference*, not a health record (§4.3).
- **Reduced-motion default for the "deep content"** so the streak/league animations never compete with learning.
- **Test with real assistive tech + automated (axe) in CI**; accessibility is a release gate, not a backlog item.
...
- AI generation is **the abuse + cost vector**. A script hitting tutor/art endpoints can run up real spend. Defenses, layered:
  1. **Viatika BudgetPolicy** (§2) as the hard backstop — per-user daily + per-tier monthly caps; "most restrictive wins"; over-cap returns a friendly limit message, not an error or unbounded spend.
