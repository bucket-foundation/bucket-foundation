...
> The unified UX specification for Bucket Academy. It rigorously dissects three case
...
Open up the most adjacent understanding, by walking a learner along a **dependency graph** of Concept
...
foundations* (you always know *why* you're learning this and *what it opens up*), a *real citeable corpus*
...
## 1. APPLE
Human Interface Guidelines: deference, clarity, depth.
 ...
 *informative*. Decoration is out of scope.
 ([developer.apple.com/design/human-interface-guidelines](https://developer.apple.com/design/human-interface-guidelines/))
 - **Replicate:** when a concept is mastered and *opens* a downstream concept, the motion must
 *show the dependency*, the new node lights up and an edge animates from the just-mastered node to
 it. Depth = the graph getting deeper.

### 1.2 Concrete numbers and rules to enforce
...
| **Haptics** | Use *system-defined* haptics (`UIImpactFeedbackGenerator`, `UINotificationFeedbackGenerator`); users lean on them most when not looking at the screen | [developer.apple.com/.../feedback](https://developer.apple.com/design/human-interface-guidelines/patterns/feedback/) |
...
### 1.4 "It just works" inevitability
...
## 2. DUOLINGO
The engagement engine, decomposed.

### 2.1 The 2022 path redesign
*why they killed the skill tree*.
...
1. Learners "weren't sure whether they were using the app." The tree's freedom of choice
...
3. **Ordering is grounded in spaced repetition**, instead of finishing all five crown levels of one
 skill before moving on, concepts from different skills are *interspersed* and revisited on an  <!-- voice-ignore-line: describes a learner moving on between skills -->
 expanding schedule.
 ([blog.duolingo.com](https://blog.duolingo.com/new-duolingo-home-screen-design/),
...
- Feedback is "instantaneous" and praise is "frequent, within sessions, after sessions, and between
 players."
 ([deconstructoroffun.com](https://www.deconstructoroffun.com/blog/2025/4/14/duolingo-how-the-15b-app-uses-gaming-principles-to-supercharge-dau-growth))
- **Variable rewards** (XP bonuses, streak freezes, timer extensions, gems) are dispensed *randomly* after
...
### 2.4 Streak
XP, leagues, the retention surfaces.
...
### 2.8 What to AVOID from Duolingo

- **Gamification eating the learning**, optimizing streak over understanding. Our content is *deep
 foundations*; the metric of success is **mastery + transfer**. Streak length is secondary. Guardrails in §13.
- **Dark-pattern guilt**, the crying-owl notification is funny once and manipulative at scale. We use
 ...
 to keep it solid") ahead of any guilt.
...
## 3. WHOP
The creator/marketplace engine.
...
- **Replicate:** checkout is a clean, conversion-optimized modal, *not* a page navigation. Access is a
 ...
 policy; Whop is the *UX* reference here; the billing backend is separate.)
...
- Real fees are **higher than the 3% headline** (FX, payout charges → ~7%).
 ([dodopayments.com/blogs/whop-fees-explained](https://dodopayments.com/blogs/whop-fees-explained))
 Be radically transparent about the author's take in the mint/earn UI, show net figures.

---

# PART II
SYNTHESIS: THE BUCKET ACADEMY UX SPECIFICATION.
...
Progress is a *linear flow* rather than a navigation tree (Duolingo path discipline).
...
| 9 | **Notifications opt-in** | *"Want a nudge when Boltzmann is due for review?"*, framed as *learning value* ahead of guilt. | Avoid dark-pattern (our guardrail §2.8) |

**Empty-state-as-onboarding:** for branches not yet live, the card says *"Mathematics nucleus, in
construction. Get notified."*, never a dead end.
 ...
 *"we'll bring this back sooner"* note (FSRS reschedules; failure is a *feature* here, a scheduling signal,
...
5. **PROGRESS / END-OF-ROUTE SCREEN (copy Duolingo's reward screen):**
 ...
 (a surprise: a streak freeze, an art-card reveal, an XP bonus, *randomly*, Duolingo §2.3).
...
## 7. The nucleus / dependency-graph visualization
...
### 7.2 The principle: the graph is the MAP
...
### 7.3 Three zoom levels of the MAP
...
1. **L1, Constellation (branch overview).** Not a raw force-directed graph. A **curated, hand-laid-out
 ...
 as fill. This is a *designed* layout, and that is the whole point. (Think a subway
 map / tech-tree, kept legible.)
...
### 7.4 Concrete visual grammar

| Encoding | Meaning | Source basis |
|---|---|---|
| **Node size** | Centrality / reach (the nucleus core is biggest) | size encodes importance ([yfiles](https://www.yfiles.com/resources/how-to/guide-to-visualizing-knowledge-graphs)) |
...
- **"Opens"** is the line no competitor has, it makes reach visible and ties the atom back to the
...
Motion, haptics, and the four states.

### 9.1 Motion & haptics budget
...
| Concept opened | node lights + edge draws on MAP | medium impact | warm "opened" tone | 400-500 ms |
...
- **Error (answer):** see §6.3, non-punishing, names the misconception, reschedules sooner. This is the
 ...
 you rather than the system judging you.** (Counter to Duolingo's heart-loss frustration, §2.8.)
- **Success:** the reward screen (§6.5), XP, streak, variable reward, and the reveal animation.

---

## 10. STUDIO
The Scholar/creator surface.
...
- **The trigger:** offered at the *peak moment*, the success/reveal screen (§6.5), when emotion is
...
- **The loop:** card → shared to social → links back to the *exact atom* (deep link) → recipient lands in
 ...
 content itself rather than a generic landing page.
- **Collectibility (Duolingo-style delight):** mastered atoms become a **collectible card set** on
 ...
 higher-fidelity (a Pro perk rather than a content gate; on-mission, foundations stay free).
...
## 12. Accessibility as part of Apple-grade quality
...
- **Mastery is the headline metric** on PROFILE: "nuclei mastered across branches" is the
...
- **Quality gates on XP:** XP for a *derive*/*teach* mastery event ≫ XP for a recall tap, so the reward
 gradient points at *understanding* ahead of volume.
- **No streak-gambling dark patterns:** notifications lead with *learning value* ("Boltzmann is due,
 keep it solid") ahead of any guilt (counter to Duolingo §2.8). Streak freeze is generous and free.
- **"Caught up" is a valid, celebrated end state**, we never manufacture busywork to extend a session.

---

## 14. Cross-pillar dependencies

| Need | Owner | Why |
|---|---|---|
| **Curated graph layout data** (L1 constellation is hand-laid rather than force-directed) | Data + Product | Avoids the Obsidian hairball (§7.1); requires per-branch x/y or shell-ring layout on top of edges |
| **Centrality scores per atom** (node size = reach) | Data | The nucleus computation (PageRank/betweenness) already specced in KNOWLEDGE-ARCHITECTURE §1 |
...
2. **Onboarding teaches a real atom and shows what it opens, before signup.** The first nucleus concept is
 learned in the onboarding, then the MAP animates the dependency opening; signup is deferred until sunk
...
4. **The feedback moment is non-punishing.** Wrong answers are amber (never red), name the misconception,
 and reschedule sooner, failure feels like help rather than judgment. The single most important emotional
...
7. **The "Opens" line + post-route reveal animation** make reach visible, the one thing no
