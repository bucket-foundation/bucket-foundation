...
Against the current repo state: the Academy importer produces 487 nodes and 820 `prerequisite` edges across eight files, zero review items. The canon importer produces 8 nodes (6 entries plus 2 bibliographic source nodes for the two law-kind dossiers), 4 edges (2 `cites`, 2 `derives_from`), and 4 `unmatched_derives_from` review items. Combined: 495 new nodes and 824 new edges layered onto the Phase 0 seed's 22 nodes and 28 edges, zero tier violations, zero orphan edges. `scripts/research-os/ingest/out/sample-academy-preview.json`, `sample-canon-preview.json`, and `sample-review-list.json` are small committed samples of the real shape (the canon and review-list samples are the full current output; the academy sample is a two-node, one-edge slice of the full 487/820).

## What this slice does not do

No model call anywhere in either importer: `mapAtomKind` and `isLawFolder` are fixed-rule string checks, `computeRequiresDepth` is a graph walk, `matchAcademyAtom` is exact-or-nothing. This is deliberate for exactly the population these two importers cover, where the source data already carries enough structure (an atom's own `type`, a dossier's own folder name, an atom's own `requires` list) that a rule resolves it. It does not cover the case the architecture review's own ingestion section names as the harder half of the pipeline: proposing a **new** `prerequisite` or `generalizes` edge between two nodes that have no existing structural link to read a rule from (an Academy atom and a K-12 curriculum standard it was never authored against, for instance, or two canon dossiers with no shared folder-name signal). That step needs a model: an LLM proposes the candidate edge with a confidence score and its own provenance (`{source: "llm_infer", model, confidence}`, the shape `02-architecture.md` section 10 already specifies), and the edge queues for human review before it can affect routing, exactly the same review-list mechanism this slice already ships, extended with one more `ReviewItemKind` (`llm_proposed_edge`, unbuilt) for a human to accept or reject rather than a silent auto-merge. Every quality gate the architecture review names for that step applies unchanged: no node ships without at least one edge, no `prerequisite` edge ships without human review, no atom-to-canon `generalizes` edge ships without a canon-track reviewer. This slice's own deterministic matching already satisfies the last of the three for the two dossiers it did link, by construction, with no model call needed.
