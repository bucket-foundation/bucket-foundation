...
54 items: 24 in MVP, 23 near-term, 7 later. Ten MVP rows and three near-term rows are shipped, and six more are partial.

**Shipped.** Prime decomposition with its signature, penetration and tier; the decompose-further queue that holds model-proposed factors for human review; the made-of section on the node page; both patent slices; the Vercel gate with the pre-push check; the engine's local host; the grade-tier reconciliation (#209); the bead dispatcher that files each pending bead once (#214); the four surfaces ros-frontend 1 owed (#206); and, from near-term, the software atlas with its page (#203, #206), graph dedup with its review queue (#210).

**MVP, open.** The truth memo and the claim-provenance schema under it; the truth level per node with the network analysis that names the load-bearing primes; imports 1 through 3, which is the data model, the upload, and the extraction into passages and tables; the research on human-AI-computer work; this roadmap; and the standing rule that every task shows its work. The AI evidence search from `learning/research-os/ai/IMPLEMENTATION.md` adds six: read-authorization parity with durable quote receipts, the bead dispatcher, the admitted source corpus, the CPU encoder worker, Find by meaning into Quote, and quotes from imported passages. The pilot runs for one researcher on this machine, which is the MVP test. The corpus (#216, #217), the worker (#219), Find (#223) and the watch ledger (#220) are merged and read as partial until read-authorization lands: the durable quote receipt is #196 and the admission check inside Quote is #218.

**Near-term.** Funding wave 1 in three rows, since what each funder asks decides what blocks it. The Tools Competition, DPG registration, the Renaissance Philanthropy inquiry and the Sentry open-source plan take an individual, and the nearest live deadline among them is 2026-10-13. NLnet takes individuals too and carries its own question, whether a US applicant clears the European-dimension bar, 43 days before its 2026-11-03 close. Cloudflare Project Galileo, Vercel for Startups and Fast Forward's next cycle ask who the applicant is and wait on FD-8. Beside them sit both remaining patent slices, the first waiting on FD-1; the software atlas and the workbench design; the map layers and the canon-tag split; imports in search and sharing; the Awareness view, production placement, and roles as grants; first-hand verification; the local model tier; the site copy, waiting on FD-4; the atlas and workbench surfaces the two workbench memos owe; and four AI rows: the sealed evaluation that decides release to other pilot accounts, the engine parity fixture, the replication study, and the daily model watch as a ledger.
