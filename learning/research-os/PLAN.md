...
`PLAN-REVISION-4.md` reads thirty-two PRs merged since revision 3 (#70 through #113, including the engine loop's own retraction-propagation, contested-science-corpus, and prediction-register work) against this plan and against `PLAN-REVISION-1.md` through `PLAN-REVISION-3.md`: what shipped, with live app and canon-pipeline test counts and the engine's latest recorded count; four evidence-driven revisions from a fifth literature batch (`intake/ros-literature-5`, PR #90, read from its own branch since it had not yet merged), a real conflict in the required-participation literature with an autonomy-supportive-delivery mitigation, a reliability-and-resolution decomposition for the hypothesis engine's own holdout ledger with a reasoned floor for its validated-label threshold, a literature-grounded acceptance rubric for the disclosed understanding artifact, and a naming fix distinguishing the engine's own novelty score at ideation time from a validated claim; a Phase 1 scope narrowed to nine remaining items, four of them new items this revision's own evidence adds after four of revision 3's own nine items shipped; the same six founder decisions restated verbatim, plus the repo-hygiene untracking decision and the canon sign-off backlog carried forward as founder-facing decisions in their own right; and the same operational blockers updated, one new: three separate spend-limit pauses, including a two-and-a-half-day gap with no commit activity anywhere in the repository, that stopped agents mid-task without losing work. This file stays the iteration-1 record and `PLAN-REVISION-1.md` through `PLAN-REVISION-3.md` stay the first three revisions' own records; `PLAN-REVISION-4.md` is where the fourth revision lives.
