...
`PLAN-REVISION-3.md` reads eighteen PRs merged since revision 2 (#45 through #66) against this plan, `PLAN-REVISION-1.md`, and `PLAN-REVISION-2.md`: what shipped, with live app, engine, and canon-pipeline test counts; five evidence-driven revisions from a fourth literature batch, scaffolding for low-prior-knowledge learners, a provenance guard against production misconduct under required participation, a second-source corroboration step for the Check tool, a counter-evidence field for the Production envelope, and three new published effect-size anchors for the pre-registration's own power finding; a Phase 1 scope narrowed to nine remaining items, four of them new build items this revision's own evidence forces; the same five founder decisions restated verbatim with a sixth new decision on required participation; and the same six operational blockers updated, one of them, the Vercel free-tier deploy cap, new. This file stays the iteration-1 record, `PLAN-REVISION-1.md` stays the revision-1 record, and `PLAN-REVISION-2.md` stays the revision-2 record; `PLAN-REVISION-3.md` is where the third revision lives.
