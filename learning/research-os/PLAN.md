...
`PLAN-REVISION-2.md` reads nineteen of the twenty PRs merged since revision 1 (#20 through #40, PR #36 excluded as engine-only) against this plan and against `PLAN-REVISION-1.md`: what shipped, with live app and engine test counts; four more evidence-driven revisions from a third literature batch, human-AI complementarity on productions, progression unidimensionality, prerequisite annotation agreement, and the pre-registration's own power finding forcing a feasibility-pilot qualifier onto every Phase 1 description; a Phase 1 scope narrowed to seven remaining items with a bead id on each; the same five founder decisions restated with new information and a recommended default on each; six named operational blockers with an action and an owner; and an updated pilot-versus-district-scale table. This file stays the iteration-1 record and `PLAN-REVISION-1.md` stays the revision-1 record; `PLAN-REVISION-2.md` is where the second revision lives.

## Revision 3

`PLAN-REVISION-3.md` reads nineteen PRs merged since revision 2 (#45 through #66) against this plan, `PLAN-REVISION-1.md`, and `PLAN-REVISION-2.md`: what shipped, with live app, engine, and canon-pipeline test counts; five evidence-driven revisions from a fourth literature batch, scaffolding for low-prior-knowledge learners, a provenance guard against production misconduct under required participation, a second-source corroboration step for the Check tool, a counter-evidence field for the Production envelope, and three new published effect-size anchors for the pre-registration's own power finding; a Phase 1 scope narrowed to nine remaining items, four of them new build items this revision's own evidence forces; the same five founder decisions restated verbatim with a sixth new decision on required participation; and the same six operational blockers updated, two new: the Vercel free-tier deploy cap, and a hard reset that discarded uncommitted archive-runner outputs. This file stays the iteration-1 record, `PLAN-REVISION-1.md` stays the revision-1 record, and `PLAN-REVISION-2.md` stays the revision-2 record; `PLAN-REVISION-3.md` is where the third revision lives.

## Revision 4

`PLAN-REVISION-4.md` reads forty-six PRs merged since revision 3 (#70 through #124, plus #68 and #115 landing out of numeric order, including the engine loop's own retraction-propagation, contested-science-corpus, and prediction-register work) against this plan and against `PLAN-REVISION-1.md` through `PLAN-REVISION-3.md`: what shipped, with live app and canon-pipeline test counts and the engine's latest recorded count; four evidence-driven revisions from a fifth literature batch (PR #90, merged mid-pass), a real conflict in the required-participation literature with an autonomy-supportive-delivery mitigation, a reliability-and-resolution decomposition for the hypothesis engine's own holdout ledger with a reasoned floor for its validated-label threshold, a literature-grounded acceptance rubric for the disclosed understanding artifact, and a naming fix distinguishing the engine's own novelty score at ideation time from a validated claim; a Phase 1 scope narrowed to nine remaining items, four of them new items this revision's own evidence adds after four of revision 3's own nine items shipped; the same six founder decisions restated verbatim, plus the repo-hygiene untracking decision and the canon sign-off backlog carried forward as founder-facing decisions in their own right; and the same operational blockers updated, one new: four separate spend-limit pauses, including a two-and-a-half-day gap with no commit activity anywhere in the repository, that stopped agents mid-task without losing work. This file stays the iteration-1 record and `PLAN-REVISION-1.md` through `PLAN-REVISION-3.md` stay the first three revisions' own records; `PLAN-REVISION-4.md` is where the fourth revision lives.
