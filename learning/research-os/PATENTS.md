...
|---|---|---|
| 05-biophysics | `A61` (medical or veterinary science; hygiene), `C12` (biochemistry, microbiology, enzymology), `G01N` (investigating or analysing materials by determining their chemical or physical properties) | The largest branch, and the one where measurement of materials meets the literature |
...
A patent carries several symbols, so the importer needs a rule that gives one branch: the longest symbol in the table that matches the patent's main CPC symbol wins, which sends a `G01N` patent to biophysics and the rest of `G01` to physics. Where no symbol matches, the patent is left out of the slice.

The other ten branches stay out of the first slice. Each has patent classes that would reach it, `G01V` (geophysical prospecting) for earth and `H04L 9` (cryptographic arrangements) for mathematics among them, and adding a class costs the load and the review that follow it. ros-patents 2 loads the four branches above, records how many patents and links each symbol contributes, and those counts decide which class joins next.
...
- **What never gets filed.** Cohen, Nelson, and Walsh surveyed 1,478 US manufacturing R&D labs and found that patents "tend to be the least emphasized by firms in the majority of manufacturing industries, and secrecy and lead time tend to be emphasized most heavily" ([NBER working paper 7552, 2000](https://doi.org/10.3386/w7552)). The repository's quantum note reports the same pattern in its field, where decoders, calibration, and fabrication recipes stay secret. A patent landscape undercounts the work.
...
So a patent count is evidence that a field has applied activity, and it stays a count. Research OS shows patents as objects with their own links and never scores an idea by how many patents cite it; where a count appears, it names the measure, filings or families, and its date.
...
- The case for holding an invention open rests on measured follow-on effects: citations to a paper fall 9 to 17% after its patent grants, and Celera's two-year gene IP cut later research and product development by 20 to 30 percent.
