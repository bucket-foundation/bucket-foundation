...
| Depth and layer | The longest factor path from a node down to a prime. Primes sit at layer 0; each layer combines the layers below. The node page says "N layers above its primes". This is separate from a node's grade tier, the `tier` column that K-12 routing reads. |
...
Every prime and every unfactored idea gets decomposed by a model, checked by a second model and by Wikipedia's links, and decided by a person. Only a reviewer's decision at `/research-os/edges` writes `graph.edges`. Runner: `scripts/research-os/decompose-further.ts`. Pure logic: `src/lib/research-os/decompose-further.ts` and `src/lib/research-os/refd.ts`. Review writes: `src/lib/research-os/inference/review-actions.ts`.

**Targets.** Idea nodes only: concepts, laws, and derivations from the Academy atoms, canon entries, references, primary sources, and base ideas a reviewer added. Facts, primary-source records, figures, and sites are evidence. The 105 canon concept tags, the bridges, and the intake digests group other material (33 of the tags are people, such as Euler and Tesla), so they are neither targets nor factors. The queue decomposes the idea layer: idea nodes, with an edge wherever one idea rests on another directly or through evidence alone. The Rayleigh scattering law rests on "scattering strength depends on particle size" through Rayleigh's 1871 paper, so in the idea layer it rests on that idea and is a composite; "each color is a wavelength" rests only on facts, so it is a prime of the layer and the queue asks whether it breaks down further. The facts under an idea show on its node page as evidence. On the local graph that gives 41 targets: 39 primes and 2 unfactored ideas. The first run, before the idea rule, asked about 166 targets, most of them tags.
...
**Missing ideas.** The base ideas the proposer names go through one more model pass that groups synonyms and spots ideas that already have a node. An idea that has a node becomes a factor proposal for every target that named it. The rest go to `graph.node_proposals` with their aliases, the targets that named them and why, and the existing nodes they may duplicate by embedding at a cosine of 0.75 or more, a loose bar that flagged 21 of 66 in the last run: a hint for the reviewer to check, and many of the flags are wrong. Six of the 66 carry a hint at 0.80 or more, and those are worth a look first: "Vector spaces, bases and inner products" sits at 0.82 to the existing "Inner product spaces" and 0.77 to "Vector spaces". Each is checked against the semantic primes and the foundations of mathematics (number, set, function, equality, measurement) by head noun, and a match is flagged as a hint. The consolidation pass also writes a one-sentence definition of each idea; approval makes it the new node's summary unless the reviewer writes another, and refuses a node with none. A later run that names the same idea reuses its key. Approving a missing idea creates a concept node at the lowest grade tier among the targets that named it and queues an unchecked factor proposal to each of them, which the next run verifies. The new node is a target in the next run too, so decomposition goes on below it; the reviewer gates each level. It is called a missing idea until a review confirms it irreducible, since the ones named so far sit one step down, above the primes.
...
**Review.** The page opens on a summary line and a find box, then lists irreducible verdicts, missing ideas by how many targets named them, and factor proposals grouped by target, the group whose pairs are most uncertain and most rested on first, filterable by verdict and to cross-branch pairs. Each proposal shows both judgments, the Wikipedia score, where it came from, and any cycle. The reviewer picks the edge kind: "rests on" writes a `derives_from` edge and leaves learning order alone, and "learning order" writes a `prerequisite` edge and rebuilds the routing closure. A decision claims the row first, so two reviewers cannot decide it twice, and an approval that would close a cycle in the graph is refused, checked again after the claim. When a write fails and the claim cannot be released, or a new node cannot be cleaned up, the error says so. A learning-order approval rebuilds the routing closure for the target's branch and every branch resting on it, and warns the reviewer when a rebuild fails. Each pair shows both nodes' grade tiers, and choosing learning order for a factor at a higher grade tier than its target raises a warning: 61 of the 98 confirmed pairs are like that, since the Academy set grade tiers per course. Loop flags are computed when the page loads, from the pending pairs as they stand, and reload after every decision. Each pair is also marked when the graph already has it through other nodes, and when pairs the verifier confirmed already lead from its target to its factor, so approving those would make this pair a shortcut past them: 41 of the 172 pending pairs in the last run. The note names the nodes on the chain, and a filter shows the shortcuts alone. A pair that would close a loop shows the loop warning and no shortcut note. A decision on a row someone already decided reports the status the row holds now, and the queue reloads without it.

**On the node page.** Every public idea node has a "made of" section: prime, composite, or not decomposed yet; how many layers it sits above its primes; the primes under it, the most-reached first; its evidence; and the ideas one step down, each marked when the link runs through evidence. A reviewer also sees what waits on review for the node, the proposed factors with both verdicts, the missing ideas named for it with their definitions, and any irreducible verdict, with a link that lands on the node in the queue; everyone else sees how many items wait. The "around" section beside it lists learning order under "learned after" and "unlocks", so the prerequisite edges and the makeup each have their own label. Graph review, here and on `/research-os/edges`, is open to the `RESEARCH_OS_REVIEWER_EMAILS` allowlist alone, since anyone signed in can create a class and teach it. The section reads the decomposition from a snapshot of the public graph, refreshed each minute, and at once after an approval on the same server; on a host that runs several server instances, the others catch up within the minute.
...
Run on 2026-09-18 over the local graph's idea layer, proposer `claude-sonnet-5`, verifier `claude-opus-5`, both resolved at the start of the run, 41 targets (39 primes, 2 unfactored ideas). Every target answered; on the first pass 34 replies came from the cache of earlier runs with the same prompt and model, and the cited report replays all 41 from cache after later fixes that changed no prompt. Report: `scripts/research-os/ingest/out/decompose-further.json`, kept out of git; the committed summary of this run, the runs before it, and the baseline is `learning/research-os/decompose-further-runs.json`.

| Measure | Value |
|---|---|
| Factor proposals | 172: 157 from the proposer, 15 from missing ideas that already had a node |
| Confirmed by the verifier | 98: 89 checked blind, 9 checked with the factor named |
| Across branches | 110, of which 78 confirmed; same branch 62, of which 20 confirmed |
| On a loop with other proposals | 18, including 5 pairs proposed in both directions: in 3 the verifier confirmed one direction, in 2 it refused both |
| Already in the graph | 0 pairs close a loop with the graph's own edges, and 0 repeat a chain the graph already has |
| Blinded set | 317 pairs over 39 targets; the verifier held 89 of 157 picks and 1 of 160 passed-over candidates |
| Cohen's kappa, proposer against verifier | 0.56, 95% interval 0.47 to 0.66 |
| Wikipedia coverage | 475 nodes mapped to 429 articles, 470 of them through the lesson's own link; 163 of 172 proposals scored |
| RefD ROC area, confirmed against refuted | proposals 0.63, interval 0.54 to 0.72; picks alone 0.60, interval 0.51 to 0.69; passed-over candidates 0.98 from 1 confirmed pair, no interval |
| Missing ideas | 89 named; 14 matched an existing node, giving 15 pairs and 8 drops; 66 new, 2 of them named by two targets |
| Called irreducible | 0; How learning works and The neuron doctrine came back with no factors |

**The two models disagree on four picks in ten.** The verifier refused 68 of the proposer's 157 picks and accepted 1 of the 160 candidates it passed over. That shows the verifier separates picks from passed-over candidates. Which model is right about a disputed pick stays open until a person labels the pairs; the verifier's question is the stricter one, so many refusals say the factor is related to the target and can come after it. The founder's decisions at `/research-os/edges` are the first human labels, and kappa against them is the measure to report once there are enough. A kappa of 0.56 sits in the "moderate" band of Landis and Koch; half the blinded set is passed-over candidates both models reject, which lifts it.

**Runs vary, and one run cannot say why.** The run before this one decomposed the idea layer without contracting paths through evidence. On the 23 targets it asked anew, the proposer picked 136 factors against 88 before while the verifier held 58 against 56, which took its kappa to 0.45. That run also proposed 10 chains the graph already had and 2 pairs that close a loop with it, which the contraction removed. Whether the rise in picks came from the changed prompts or from sampling needs repeated runs to tell.

**Factors cross branches.** Two thirds of the proposals point across a branch, and the verifier confirmed those at a higher rate (78 of 110) than same-branch ones (20 of 62). Part of that gap is the shortlist: it leaves out every node that already rests on the target, and for a course's entry point that removes most of its own branch. Probability distribution and expectation, in information, a factor of the Boltzmann distribution in biophysics, is one confirmed cross-branch pair, with Wikipedia's links leaning the same way. The slice 1 finding that no prime crosses a branch was a property of how the Academy was written: every `requires` in the corpus stays inside its own file.

**Wikipedia's links lean with the verifier, weakly.** Over the proposals, a confirmed pair outscores a refuted one 63 percent of the time, interval 0.54 to 0.72; among the proposer's picks alone the interval, 0.51 to 0.69, only just clears 0.5. The links remain a third opinion beside the two models, and the page shows them as that.

**The missing ideas are intermediate ideas.** Asked for base ideas, the proposer named what the graph lacks one step down: convolution, flux, the Gamma function. A few reach the bottom: time as a physical parameter, propositions as objects, the law of bivalence (named twice), natural numbers, units and dimensions, and functions of a real variable name semantic primes or foundations. The head-noun hint flags 10: 7 are base ideas, and the 3 misses are convex functions, the Gamma function, and "Causality and the Kramers-Kronig relations", an optics result whose title names a cause. Each missing idea carries a one-sentence definition. Some names bundle two ideas the graph already holds, such as "Vector spaces, bases and inner products"; the consolidation pass matches a whole name to one node, so a bundle comes through as new. Approving one would add a node beside the ones it bundles, so the reviewer rejects it; linking its parts to the existing nodes waits on ros-graph-dedup.

**Some matches read the graph backward.** Eight matched ideas already rest on the target that named them: spacing, retrieval practice, and the fluency illusion under How learning works, and five biophysics pairs, such as Flory scaling under the ideal chain. The runner drops such a pair, since it would close a loop, and lists it in the report as a reversal to check: either the graph's edge or the proposer has the direction wrong. Bead ros-tier-fix takes these up with the grade tiers: 61 of the 98 confirmed pairs put a factor at a higher grade tier than its target.
...
2. **Surfaces** (ros-prime 4). The node page's "made of" section ships with this slice. What remains: the factor tree drawn down to the primes, the truth level with its interval, and map layers colored by tier, penetration, and truth level.
