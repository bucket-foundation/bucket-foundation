...
**On the node page.** Every public node has a "made of" section: prime, composite, or not decomposed yet; how many layers it sits above its primes; the primes under it, the most-reached first; and what it rests on directly. A reviewer also sees what waits on review for the node, the proposed factors with both verdicts, the missing ideas named for it with their definitions, and any irreducible verdict, with a link that lands on the node in the queue; everyone else sees how many items wait. The section reads the decomposition from a snapshot of the public graph, refreshed each minute and at once after an approval.
