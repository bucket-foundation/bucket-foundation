...
**Targets.** Idea nodes only: concepts, laws, and derivations from the Academy atoms, canon entries, references, primary sources, and base ideas a reviewer added. Facts, primary-source records, figures, and sites are evidence. The 105 canon concept tags, the bridges, and the intake digests group other material (33 of the tags are people, such as Euler and Tesla), so they are neither targets nor factors. The queue decomposes the idea layer: idea nodes and the factor edges between them. An idea that rests only on facts, like the Rayleigh scattering law in the K-12 sky chain, is a prime of that layer, so the queue asks whether it breaks down further; the facts under it show on its node page as evidence. On the local graph that gives 44 targets: 40 primes and 4 unfactored ideas. The first run, before the idea rule, asked about 166 targets, most of them tags.
...
Run on 2026-09-18 over the local graph's idea layer, proposer `claude-sonnet-5`, verifier `claude-opus-5`, both resolved at the start of the run, 44 targets (40 primes, 4 unfactored ideas). Every target answered; 17 replies came from the cache of the run before, where the prompt was unchanged. Report: `scripts/research-os/ingest/out/decompose-further.json`, kept out of git; the committed summary of this run, the one before, and the baseline is `learning/research-os/decompose-further-runs.json`.

| Measure | Value |
|---|---|
| Factor proposals | 223: 206 from the proposer, 17 from missing ideas that already had a node |
| Confirmed by the verifier | 106: 95 checked blind, 11 checked with the factor named |
| Across branches | 132, of which 78 confirmed; same branch 91, of which 28 confirmed |
| On a loop with other proposals | 39, including 11 pairs proposed in both directions |
| Blinded set | 413 pairs over 42 targets; the verifier held 95 of 205 picks and 3 of 208 passed-over candidates |
| Cohen's kappa, proposer against verifier | 0.45, 95% interval 0.38 to 0.52 |
| Wikipedia coverage | 475 nodes mapped to 429 articles, 470 of them through the lesson's own link; 195 of 223 proposals scored |
| RefD ROC area, confirmed against refuted | proposals 0.66, interval 0.58 to 0.73; picks alone 0.64, interval 0.56 to 0.72; passed-over candidates 0.98 from 3 confirmed pairs, no interval |
| Missing ideas | 136 named; 19 matched an existing node, giving 17 pairs and 7 drops; 109 new, 3 of them named by two targets |
| Called irreducible | 0; How learning works and The neuron doctrine came back with no factors |

**The two models disagree on half the picks.** The verifier refused 110 of the proposer's 205 picks and accepted 3 of the 208 candidates it passed over. That shows the verifier separates picks from passed-over candidates. Which model is right about a disputed pick stays open until a person labels the pairs; the verifier's question is the stricter one, so many refusals say the factor is related to the target and can come after it. The founder's decisions at `/research-os/edges` are the first human labels, and kappa against them is the measure to report once there are enough. A kappa of 0.45 sits in the "moderate" band of Landis and Koch.

**Decomposing the idea layer brought the equations in, and more same-branch noise.** Four sky-chain ideas, the Rayleigh scattering law among them, rested only on facts and were hidden as composites; now they are targets. The same change lowered many ideas' layers, so each branch's base quota offers more sibling primes, and the proposer picked more of them: same-branch proposals rose from 52 to 91, the verifier confirmed 28 of them, and 11 pairs came back proposed in both directions, each flagged as a loop for the reviewer to settle one way. The run before, over every public node, gave 151 proposals and a kappa of 0.59 on 40 targets.

**Factors cross branches.** Three fifths of the proposals point across a branch, and the verifier confirmed those at a higher rate (78 of 132) than same-branch ones (28 of 91). Part of that gap is the shortlist: it leaves out every node that already rests on the target, and for a course's entry point that removes most of its own branch. Probability distribution and expectation, in information, a factor of the Boltzmann distribution in biophysics, is one confirmed cross-branch pair, with Wikipedia's links leaning the same way. The slice 1 finding that no prime crosses a branch was a property of how the Academy was written: every `requires` in the corpus stays inside its own file.

**Wikipedia's links now lean with the verifier.** Over the proposals, a confirmed pair outscores a refuted one 66 percent of the time, and among the proposer's picks alone the interval, 0.56 to 0.72, sits above 0.5. The earlier run's picks-alone interval spanned 0.5. The links remain a third opinion beside the two models; the page shows them as that.

**The missing ideas are intermediate ideas.** Asked for base ideas, the proposer named what the graph lacks one step down: convolution, flux, the Gamma function, Hilbert space, state functions, Maxwell's equations. A few reach the bottom: time as a physical parameter, propositions as objects, bivalence (named twice), natural numbers, and combinatorial counting name semantic primes or foundations. The head-noun hint flags 16, about 9 of them base ideas; the misses are function names and titles such as "Level sets" and "Complex numbers". Each missing idea carries a one-sentence definition.

**Some matches read the graph backward.** Seven matched ideas already rest on the target that named them: spacing, retrieval practice, and the fluency illusion under How learning works, and four biophysics pairs. The runner drops such a pair, since it would close a loop, and lists it in the report as a reversal to check: either the graph's edge or the proposer has the direction wrong. Bead ros-tier-fix takes these up with the grade tiers: 65 of the 106 confirmed pairs put a factor at a higher grade tier than its target.
