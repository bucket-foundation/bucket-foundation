...
Run on 2026-09-18 over the local graph's idea layer, proposer `claude-sonnet-5`, verifier `claude-opus-5`, both resolved at the start of the run, 41 targets (39 primes, 2 unfactored ideas). Every target answered; 34 replies came from the cache of earlier runs with the same prompt and model. Report: `scripts/research-os/ingest/out/decompose-further.json`, kept out of git; the committed summary of this run, the runs before it, and the baseline is `learning/research-os/decompose-further-runs.json`.
...
**Runs vary, and one run cannot say why.** The run before this one decomposed the idea layer without contracting paths through evidence. On the 23 targets where its picks differed, the proposer picked 136 factors against 88 before while the verifier held 58 against 56, which took its kappa to 0.45. That run also proposed 10 chains the graph already had and 2 pairs that close a loop with it, which the contraction removed. Whether the rise in picks came from the changed prompts or from sampling needs repeated runs to tell.
