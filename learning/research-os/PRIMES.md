...
Run on 2026-09-18 over the local graph, proposer `claude-sonnet-5`, verifier `claude-opus-5`, both resolved at the start of the run, 40 targets (38 primes, 2 unfactored ideas). Every target answered with a fresh model call; the report cited replays those replies from cache, so its numbers are the fresh run's. Report: `scripts/research-os/ingest/out/decompose-further.json`, kept out of git; the committed summary of this run and the baseline is `learning/research-os/decompose-further-runs.json`.
...
| RefD ROC area, confirmed against refuted | proposals 0.59, interval 0.51 to 0.69; picks alone 0.58, interval 0.48 to 0.68; passed-over candidates 0.997 from 3 confirmed pairs, no interval |
| Missing ideas | 74 named; 16 matched an existing node, giving 14 pairs and 6 drops; 51 new, 3 of them named by two targets |
| Called irreducible | 0; How learning works and The neuron doctrine came back with no factors and 4 missing ideas each |
...
**Factors now cross branches.** Two thirds of the proposals point across a branch, and the verifier confirmed those at a higher rate (75 of 99) than same-branch ones (19 of 52). Part of that gap is the shortlist: it leaves out every node that already rests on the target, and for a course's entry point that removes most of its own branch, so the same-branch candidates left are later, unlinked course material. Probability distribution and expectation, in information, a factor of the Boltzmann distribution in biophysics, is one confirmed cross-branch pair, with Wikipedia's links leaning the same way. The slice 1 finding that no prime crosses a branch was a property of how the Academy was written: every `requires` in the corpus stays inside its own file.

**Wikipedia's links add little inside the proposer's picks.** Over the proposals, a confirmed pair outscores a refuted one 59 percent of the time, and the interval runs from 0.51 to 0.69. Among the picks alone the interval spans 0.5, so within what the proposer chose, the links cannot yet tell the verdicts apart. Over the passed-over candidates the area is near 1 from only 3 confirmed pairs, too few for an interval, and the report leaves it out below 10 on either side. The page shows the score as a third opinion and nothing more.

**The missing ideas are intermediate ideas.** Asked for base ideas, the proposer named what the graph lacks one step down: convolution, flux, the Gamma function, reference frames, density matrices. A few reach the bottom: time as a physical parameter, propositions as truth-bearers, the law of bivalence, and natural numbers name semantic primes or foundations, and ordered pairs names another. The head-noun hint, which reads past "X as Y" and "the law of X", flags 6, 4 of them right; it misses ordered pairs and flags two function names. Each missing idea carries a one-sentence definition.

**Some matches read the graph backward.** For How learning works, the proposer named spacing, retrieval practice, and the fluency illusion as missing base ideas; all three are Academy atoms that rest on it in the graph. The runner drops such a pair, since it would close a loop, and lists it in the report as a reversal to check: either the Academy edge or the proposer has the direction wrong. Two more matches were the target itself.
