...
| After | 84 | 88 | 0.95 | 0.89 to 0.98 |

After the change, 1 sample root is hidden, labeled correct, and none hidden is labeled wrong, so the sample shows 1 row of recall lost and no precision gained. 15 roots changed; relabeled, 12 are correct, 1 wrong homograph and 2 disagreements. The intervals overlap, and the sample claims no improvement. Its 120 rows hold 3 roots labeled wrong, too few to measure a homograph fix. The named errors change directly: *ne* now shows *\*ne* "not", *le mien* is withheld, and Polish *to* in BE (SOMEONE/SOMETHING) stays at 0.6, marked uncertain.

## Bands

The per-band check from `POLINGUAL-ROOTS.md` was rerun on the same seed, 25 rows per band of row `confidence`, with the two models in place of the hand check. Row confidence changed for 3 of 10,239 rows.
...
Run of 2026-09-23 on the local stack. node_words: 6,872 rows store a root, 6,139 show one; before, 6,135 showed one. nsm_exponents: 1,641 show a root, before 1,758. OSHB on node_words: 24 agree, 4 differ and were replaced, 90 are OSHB only; on nsm_exponents: 13 agree, 1 replaced, 35 OSHB only. The pages credit OSHB under CC BY 4.0.
