...
`evolution-live.timer` runs `scripts/evolution-live-runner.sh` once a day at 04:17 UTC. Each run fetches endoflife.date when 20 hours have passed since its last fetch while the `endoflife-mit` rule in `learning/research-os/ai/rights-policy.json` loads (it cleared its gate in #342), and weekly if that rule turns back into a gate. The Wikidata software tree through QLever runs when 6 days 20 hours have passed. Changed files land under `_intake/evolution/endoflife/` and `_intake/evolution/wikidata/`, the run manifest `_intake/evolution/live/runs/<UTC time>.json` records their sha256, and the runner calls `npm run evolution:computing-import -- --apply <paths>` with `EVOLUTION_BATCH_PROMOTION=off`, so new releases wait in silver for review. The import refuses any file whose sha256 differs from its manifest. A run with nothing due writes nothing. The runner holds the data-root disk lock for the whole write and import.
