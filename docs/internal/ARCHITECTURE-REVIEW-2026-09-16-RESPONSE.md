...
| PR-003 CORS `*` on API proxies, `canon/search` claims a limit it lacks | Confirmed | `src/app/api/research/hhfit/route.ts:31` sets `access-control-allow-origin: *` (lowercase, which a case-sensitive grep misses); `canon/search/route.ts:11` says "rate-limited via standard ip" and no limiter exists. |
