...
This is the engineering contract. It does **not** build x402/Viatika or the
...
2. **Wrong home.** The tools belong to Bucket's research surface, well away from the personal site.
...
The first poll (or inline in the submit response); heavy tools go through the queue.
...
### 3.2 Stand up the always-on gateway
...
### 3.3 Discovery: env var over a committed file
...
`cryotriage synth`, `trajmine demo-md`) and the UI labels them as demo. The async
contract is built now so that flipping on a GPU worker is a deploy, well short of a redesign.
...
- **Result storage:**
 ...
 record / Supabase row (HTML reports are already self-contained, < a few MB).
...
- **Retention:** unpublished job results expire (e.g. 7 days) from the hot store; published
 ...
 config, one layer below the contract.

---

## 5. "Publish result to canon" hook
...
> applications/derived analyses**, one layer below foundations. They publish as `canon_tier:"derived"`
...
## 6. Auth + metering seam: seam only / build deferred
...
, a **FastAPI wrapper** implementing the v1 job contract for LabBrain
 ...
 async lifecycle is real. Marked TODO: swap the in-memory job table for
...
## 8. Migration steps off gianyrox
