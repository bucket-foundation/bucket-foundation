# Research Tools Needs Analysis — What the Biophysics / Comp-Bio Community Actually Needs
...
**Grounding:** our 107-opportunity software map, 3,870 ranked advisors, 199-topic field landscape, $3.3B funding map, plus 2024–2026 web research.
...
## 1. The demand picture, by subfield (grounded in our numbers)

Two independent signals in our data agree on who the users are.

### 1a. Advisor count by subfield (n = 3,870 ranked PIs, `advisors_ranked.csv`)
...
### 1b. Research volume by subfield (`subfields_long.csv`, OpenAlex works aggregated across the top ~200 institutions)
...
### 1c. Funding by subfield (from `Biophysics_Software_Opportunity_Strategy.pdf`, NIH RePORTER + EU CORDIS scoped)

The money — which is what actually pays for tools, since funders pay a PI / a small business / an open-source project, not "a person who writes code" — concentrates even harder:
...
## 2. The universal layer (every subfield needs these)
...
**What the 7 cover well:** protein structure/stability/MD (3 tools), the cryo-EM picking bottleneck, the single biggest funded subfield (ion-channel electrophysiology via PatchSeqML), drug-screening, and the universal literature layer. That's a genuinely strong spine — it hits 5 of the top-7 subfields.
...
## 4. The full needs list — 54 tools, 10 categories
...
**We have 7–8; the other ~46 are gaps.** The honest read: our 7 are an excellent *vertical slice* (structure + one big electrophysiology tool + literature), but the *breadth* a researcher platform needs spans 10 categories, and the two largest user cohorts by headcount (DNA/RNA: 1,105; neuro: 938) are barely touched.
...
## 6. The biggest gap clusters (where to invest next)

1. **DNA/RNA biophysics tooling** — our single largest cohort (1,105 PIs, $381M), essentially uncovered. Cluster: `RNAFold3D-Serve`, `RNA-FM-Embeds`, `gRNA-Optimizer`, `ChromatinAccess`, `CellAtlasQuery`. **Highest leverage by headcount.**
...
- **Advisor tags overlap and are coarse.** "DNA/RNA biophysics & gene regulation" (1,105) is a broad bucket; not every PI in it needs RNA structure prediction. Treat the counts as *order-of-magnitude* demand, not precise TAM.
- **Funding ≠ tool spend.** The $1.57B "software-addressable" figure is a strategy-scan estimate of pools *touchable* by software, not committed tool budgets. Academic tool spend is small ($5–50K/yr core facilities, $10–100K/yr site licenses); the real money is core-facility and pharma-seat monetization, per the strategy doc.
- **Several "white space" tools may be white space because demand is thin, not because no one served them.** Quantum-bio (168 PIs) and 2D-IR assignment are flagged low-priority for this reason.
...
- **The 8 "we have it" rows are the *validated* tools**; productizing them on Bucket (multi-tenant, citeable outputs, x402/credit metering) is itself non-trivial and is the near-term work, not net-new builds.
