...
- Tools: **40 built** (was 20 → +8 real CPU tools on 2026-06-21 → +2 all-field horizontal tools on 2026-06-22 → +5 per-field non-bio tools on 2026-06-22 → +5 per-field classical-algorithm tools on 2026-06-24). **None deployed live.**
 ...
 - all-field horizontal x2 (FAIRCheck, RepliCheck), serve EVERY discipline (the 1.17M researchers) across fields: FAIR data management + statistics reproducibility, funder-mandated across NIH/NSF/Horizon/Wellcome/Gates.
...
- TimeSeriesForecast *(econ-social 42,276 + earth-climate 116,840 + universal)*, Holt-Winters triple exponential smoothing (level/trend/seasonal, α/β/γ fit by minimizing in-sample SSE) + classical additive decomposition + an holdout backtest (MAE/RMSE/MAPE vs a naive last-value baseline). Dependency-light numpy (scipy refine if present). Verified: a perfect linear trend y=2t is forecast on the line (40, 42, 44…); on the trend+12-season demo the seasonal HW beats naive on the 12-month holdout; additive decomposition recovers a planted period-4 seasonal spike.
...
**Deferred, non-GPU but out of scope this pass (infra/agent work spanning algorithms):**
...
## A · [graph] Entity resolution → the real graph **
...
## B · [funders] Complete the global funding map
...
## D · [platform] Live infra & deployment
...
34. cite-forever x402 payout · 35. Register + DOI the funding-map dataset (turn the flywheel once) **(P1)**

## F · [science] Metascience research
