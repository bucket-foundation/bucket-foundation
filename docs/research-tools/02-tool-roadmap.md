...
## 2. The 7 existing tools (built + validated; moving onto Bucket)
...
- **Productize in Bucket:** `/research/tools/stabilitydesigner`. **Inputs:** structure + (optional)
  ...
  sequence, top-N suggested mutations. **Publish hook:** publish the ΔΔG landscape for a target as a
  citeable result (a real "we predicted these N stabilizing mutations" artifact).

### 2.4 TrajMine — *MD trajectory → mechanism report (Markov-state)*
...
- **Productize in Bucket:** `/research/tools/trajmine`. **Inputs:** trajectory file(s) (DCD/XTC) +
  ...
  hook:** publish the mechanism report as a citeable analysis (the highest-leverage publish in the
...
### Tier 1 — ship now (low infra, RAG/agent moat, high-demand)
...
### Tier 2 — next (applied ML on assay data; needs data or modest GPU)
...
### Tier 3 — later (frontier; heavy GPU/QM, scarce data, deepest moat-but-slowest)

Representative high-value frontier opportunities (full list in `software_opportunities.csv`,
build-tier C). Ship 1–2 only as *signals of competence* to a target advisor, not as broad products:
...
- **Reuse, don't rebuild.** Tool pages reuse the canon viewer components, the Mol*/NGL viewer, the
  ...
  the runs list — not a new design system.
...
## 5. Sequencing — what to wire first, then rollout order

### Wire FIRST: **LabBrain** (the reference pattern)
