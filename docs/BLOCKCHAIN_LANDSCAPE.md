...
4. **Do NOT build our own chain.** The cost-to-value ratio on an OP Stack or Orbit rollup is punitive for a citation rail (see §5). Base already *is* our rollup. Our moat is the envelope + payout contract + canon curation. A custom VM adds nothing.
5. **The real competition is DeSci (Molecule, VitaDAO, ResearchHub) and data markets (Ocean, Vana), well ahead of Coinbase or Story.** Bucket is differentiated by being a *citation* rail (payouts to authors per query) rather than an IP-licensing DAO (Molecule) or a dataset marketplace (Ocean). Lean into that wedge; don't recreate RSC or OCEAN tokenomics.

---

## 2. Comparison Matrices

### A. Chains to transact on
...
|---|---|---|---|
| **Ocean Protocol** | Datatoken-wrapped dataset marketplace ([Ocean](https://oceanprotocol.com)) | Tokenized data access with no citation payouts | Adjacent |
...
| **Bittensor subnets** | Incentivized AI subnet economy | AI-native payouts with no citation layer | Out of scope |
| **Fetch.ai** | Agent-to-agent payments (via own FET chain) | Agent payments, but legacy stack | Less relevant post-x402 |
| **SingularityNET** | AI service marketplace | AI services with no citation layer | Out of scope |
| **Grass** | Bandwidth data DAO | Data DAO with no citation layer | Out of scope |
| **Farcaster Frames** | Social + micropayment surface | Discovery mechanism | **Use as distribution** |
...
**Strategic fit (D):** Bucket's wedge is **"pay-once-mint → author gets paid forever on every citation"**, Molecule + VitaDAO do this for biotech *licensing* (access to IP), leaving *citation* untouched. ResearchHub pays for peer review. Ocean pays for data access. **The citation-payout primitive is unoccupied.** Hold it.
...
**The moat is the envelope schema, the payout contract, and the canon curation.** The chain is commodity. Base is our chain. Don't fork the rail; fork the application layer.
