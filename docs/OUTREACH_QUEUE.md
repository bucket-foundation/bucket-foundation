...
**Why first:** He runs both Molecule AND VitaDAO. One conversation = two partnerships. He's the most crypto-native DeSci founder alive.

```
Tyler — Gian from bucket.foundation. We're a nonprofit building the missing post-publication revenue layer: per-citation USDC payouts to authors on Base via x402. Molecule mints IP-NFTs, VitaDAO holds them, but there's no recurring revenue primitive underneath — we solve that. Would love to pilot 10 VitaDAO IP-NFTs with bucket-enabled citation wallets, zero fee, 30 days. Open to a 15-min call?
```

**Character count:** 508. Fits X DM.
...
```
Hey BIO team — Gian from bucket.foundation (nonprofit, citation-rail primitive on Base). Every BioDAO's papers could be earning per-citation USDC from AI agents, recurring, routed to DAO treasury. One integration standard across all 8 BioDAOs. Proposing a joint blog post + bucket-by-default for the next cohort. No token deal, no equity — pure ecosystem-side. Worth a call?
```
...
```
Hi VitaDAO team — Gian, bucket.foundation (nonprofit, pre-501(c)(3), MIT code). We've built a per-citation author-payout rail: agents pay $0.005–$0.05 in USDC per research query on Base via x402, routed forever to a payout wallet. Your $38M IP portfolio could be earning recurring treasury income from agent citations. Proposing a 30-day pilot on 10 of your longevity IP-NFTs, zero fee, transparent on-chain dashboard. Loop in Tyler if that's the right path. Full proposal here: https://bucket.foundation (draft). Reply to this or DM @bucket_fdn.
```

**Character count:** 693. Fits web form; for X DM trim to first 4 sentences.
...
- Pitch competitors (anyone doing per-citation payouts) without first verifying they aren't pivoting toward it
