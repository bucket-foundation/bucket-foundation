...
from . import calibrate, export, llm, roles, tournament, unknowns
...
def _time_bins_for(corpus: Corpus, max_bins: int) -> list[int]:
    years = [g.year for g in corpus.ground_truth]
    if not years:
        return [time_bin_index(2000)]
    bins = sorted({time_bin_index(y) for y in years})
...
def _llm_proposed_hypotheses(
    ...
    for proposal in response.get("proposals", []):
        ...
        hyp = Hypothesis.from_placement(placement, vocab, claims=claim_ids)
...
def run_campaign(config: dict[str, Any] | None = None) -> RunArtifacts:
    ...
    llm_hypotheses = _llm_proposed_hypotheses(corpus, corpus.vocab, config=cfg, cache_dir=cache_dir, replay_only=replay_only, logger=logger)

    time_bins = _time_bins_for(corpus, cfg["max_time_bins"])
    ...
    for seed in range(cfg["seeds"]):
        combinatorial = list(enumerate_placements(corpus.vocab, time_bins, max_items=cfg["combinatorial_max_items"]))
        evidence_driven = from_evidence(corpus.evidence, corpus.vocab, resolution, seed=seed)
    ...
    views = export.timeline_views(survivors, opinions, elos, time_bins, top_k=cfg["top_k"])
