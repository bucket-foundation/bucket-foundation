...
def run_pipeline(config: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    **Cascade rule** (`bkt-hte-writeback-review`, PR #36's own review):
    ...
    if from_run_error is not None:
        ...
        # "Cascade rule"): a failed writeback (a missing --branch, or
        # `write_back` itself raising) never blocks committing/mirroring
        # the run and paper writeback leaves untouched either way.
