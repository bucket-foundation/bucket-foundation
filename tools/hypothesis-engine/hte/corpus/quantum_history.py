...
def _parse_card(path: Path) -> tuple[Source, list[EvidenceItem], list[GroundTruthEvent], int]:
    ...
    count) for that card's `RetrievalEnvelope`."""
    raw = path.read_text()
...
def _parse_chapter(path: Path) -> tuple[Source, list[EvidenceItem]]:
...
def ingest(corpus_dir: str | Path | None = None, *, retrieval_run_id: str = "fixture-quantum-history-ingest") -> Corpus:
    ...
    for path in card_paths:
        source, card_evidence, card_ground_truth, citation_count = _parse_card(path)
    ...
    if chapter_path.is_file():
        chapter_source, chapter_evidence = _parse_chapter(chapter_path)
