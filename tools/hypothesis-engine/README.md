...
| `hte/partition.py` | (none; out of `Bucket.*`'s scope) | `partition` (groups placement hypotheses by their (OBJECT, PLACE, TIME_BIN) explanandum and sequence hypotheses by their ordered event pair, into competing sets), `partition_odds` (normalized posterior share and evidence-only Bayes factor per member, `shared_evidence` held out of that factor, `STATISTICAL-AUDIT-2026-09-15.md`'s "Explanandum partitions" fix) |
