"""Property tests for `hte.corpus.literature`, the only module on
`tests/COVERAGE.md`'s least-covered list with no swarm file of its own
across every round to date (`swarm`, `swarm2`, `swarm3`,
`swarm-20260910`, `swarm-20260911`, `swarm-20260914`). `tests/
test_corpus_literature.py` already covers the module's real-fixture
happy paths end to end; this file targets the pure parsing and
classification helpers directly, with generated inputs, since those are
the functions most likely to regress silently as new card batches land
with formatting this module's own hand-written regexes were not tested
against.
"""
...
def test_unescape_is_identity_when_no_escaped_quote_is_present(text):
    # `_unescape` documents exactly one substitution, `\"` -> `"`; any
    # text carrying neither a backslash nor a bare quote should pass
    # through unchanged.
...
def test_iter_lines_with_offsets_reconstructs_the_original_text_exactly(lines):
    ...
    # Concatenating every returned line, in order, reproduces `raw` byte
    # for byte: no line is dropped, merged, or reordered.
    assert "".join(line for _, _, line in out) == raw
    # Every offset names its own line's real start position in `raw`.
    for lineno, offset, line in out:
        assert raw[offset : offset + len(line)] == line
    # Line numbers are exactly 1..N in order, one per line the split
    # produced.
...
def test_split_frontmatter_fields_every_group_starts_with_its_own_key_line():
    ...
    # Every group's own first line is the one that opened it, matching
    # `^(\\w+):`.
    for key, group_lines in fields:
        _, _, first_line = group_lines[0]
        assert first_line.startswith(f"{key}:")
    # The `authors` field's own continuation lines (the two `  - "..."`
    # entries) all land inside that one group.
...
def test_split_frontmatter_fields_keeps_a_blank_line_inside_the_open_field():
    ...
    # The blank line between "line one" and "line two" belongs to
    # `why_it_matters`'s own 4-line group: opener, 2 continuation lines,
    # 1 blank.
...
def test_parse_claims_char_offsets_locate_the_exact_text_in_the_original_line(word):
    # With no backslash-escaped quote in the claim text, `_unescape` is
    # the identity, so the recorded `char_start`/`char_end` range should
    # slice `raw` back to exactly `word`.
    raw = f'key_claims:\n  - "{word}"\n'
...
def test_detect_helpers_fall_back_to_other_id_with_no_lexicon_match(text):
    # A string built only from digits and punctuation matches none of
    # this module's own hand-written keyword lexicons (all lowercase
    # English words), so every `_detect_*` helper should fall back to
    # its own slot's `OTHER` id rather than raising or returning `None`.
...
def test_declared_batch_subfolders_reads_the_readme_marker_and_skips_a_stale_one(tmp_path):
    ...
    # The real, existing subfolder is returned, resolved; the stale
    # reference to a nonexistent path is silently skipped, per this
    # function's own documented best-effort contract.
