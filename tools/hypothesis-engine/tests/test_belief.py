...
def test_lift_equals_b_minus_d_and_is_invariant_to_the_prior():
    ...
    for _ in range(200):
        ...
        op2 = Opinion.from_evidence(r, s, 2.0, a2)
        assert op1.lift() == pytest.approx(op1.b - op1.d)
        assert op1.lift() == pytest.approx(op2.lift())

@pytest.mark.parametrize("op", [
    Opinion(b=0.7, d=0.3, u=0.0, a=0.5),
    Opinion(b=0.8, d=0.0, u=0.2, a=0.5),
    Opinion(b=0.1, d=0.0, u=0.1, a=0.5),
])
...
def test_fuse_with_uncertain_opinion_is_identity_like():
    ...
    o2 = Opinion(b=0.0, d=0.0, u=1.0, a=0.4)
    fused = fuse(o1, o2)
...
def test_pooled_weight_multi_kind_no_longer_gets_the_cross_kind_bonus():
    ...
    assert s_plus < expected_no_bonus * 1.3
