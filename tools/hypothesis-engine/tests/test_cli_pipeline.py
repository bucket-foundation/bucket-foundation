...
def test_build_parser_run_every_flag():
    ...
    args = parser.parse_args([
        ...
        "--writeback", "--branch", "07-mind", "--writeback-floor-p", "0.7", "--writeback-floor-u-max", "0.4",
        "--skip-publish",
    ])
...
def test_cmd_run_builds_config_with_writeback_flags(monkeypatch, capsys):
    ...
    rc = cli_pipeline.main([
        "run", "--from-run", "runs/x/y", "--writeback", "--branch", "07-mind",
...
def test_cmd_run_allows_writeback_with_branch_given(monkeypatch):
    """The same check does not fire when `--branch` IS given alongside
    `--writeback`: it fires only on the missing-branch combination."""
    monkeypatch.setattr(pipeline, "run_pipeline", lambda config: {
        "pipeline_dir": "runs/_pipeline/ts", "outcome": "ok",
        "stages": {"writeback": {"ran": True, "ok": True}},
        "run_dir": None, "paper_dir": None,
    })
    rc = cli_pipeline.main(["run", "--writeback", "--branch", "07-mind"])
    assert rc == 0
