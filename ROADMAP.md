...
> `bucket-foundation` Nucleus instance's `/issues` route is restored. Until then, this file is the
...
The instance's `/issues` route is returning 404 (`text/plain` body, server-side Go origin — not nginx
...
## P1 — Translation graph (the actual cross-lingual unlock)

The photon corpus is currently a *bag* — every photon has `relations: []`. The single biggest leverage
...
## P1 — Semantic embeddings (the meaning unlock)

Right now we match on surface form. Adding `bge-small-en-v1.5` (384-d) embeddings of `meaning_en`
...
## P2 — Phonetic embeddings (the sound unlock)
...
## P1 — Sentence-level photons (the canon unlock)
...
that's what unlocks the bucket-canon → photon-graph pipeline.
...
### bkt-roadmap-13 · `POST /api/photon/ingest` (authenticated)
...
## P2 — Polingual UI polish

### bkt-roadmap-20 · Language picker (pin favorites)
...
## P3 — Documentation + DevX

### bkt-roadmap-24 · `INGEST.md` — how to add a new source
...
Tag the current spec as `v0.2` (now that we have edges, embeddings, multiple kinds).
...
## P3 — Monetisation (future, per Bucket thesis)
...
real renaissance unlock. After those three, polingual becomes more than a fast Wiktionary mirror:
