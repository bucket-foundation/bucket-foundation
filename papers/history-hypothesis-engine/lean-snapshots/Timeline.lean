...
`TIMELINE-AND-COMBINATORICS-SPEC.md` Section 1 fixes the engine's time axis as a
...
`TIMELINE-AND-COMBINATORICS-SPEC.md` Section 1's `uncertainty.distribution` field
...
(`TIMELINE-AND-COMBINATORICS-SPEC.md` Section 1). `start <= stop` is carried as a
...
(`TIMELINE-AND-COMBINATORICS-SPEC.md` Section 1), the relation vocabulary for
...
theorem relate_total (a b : Interval) :
    exists  r : AllenRelation, relate a b = r /\ forall  r', relate a b = r' -> r' = r :=
...
theorem relate_meets_iff (a b : Interval) :
    relate a b = .meets <->
      (not  a.stop < b.start) /\ (not  b.stop < a.start) /\ a.stop = b.start := by
