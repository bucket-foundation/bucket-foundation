...
`TIMELINE-AND-COMBINATORICS-SPEC.md` Section 2 gives a hypothesis its slot frame,
...
SPEC.md` Section 6a adds an open-world `OTHER` concept to every slot so a value
...
/-- The seven hypothesis slots (`TIMELINE-AND-COMBINATORICS-SPEC.md` Section 2).
...
(`TIMELINE-AND-COMBINATORICS-SPEC.md` Section 2's `consensus_status`). `other`
...
(`TIMELINE-AND-COMBINATORICS-SPEC.md` Section 2). `priorLogit` is a `Float` here
...
(`IDEAL-STATE-AND-UNKNOWNS-SPEC.md` Section 6a): a near-neutral base rate
...
every slot" guarantee `IDEAL-STATE-AND-UNKNOWNS-SPEC.md` Section 6a asks for,
