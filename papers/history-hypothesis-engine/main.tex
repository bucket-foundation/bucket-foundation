...
\begin{abstract}
History accumulates claims faster than any one framework scores them: a
correlation, a dated fragment, and a contested attribution can each carry
their own confidence number, with no single credence a reader can compare
across hypotheses. We present a design for a hypothesis engine that treats
every placement of an actor, action, and mechanism into a dated interval,
and every ordered pair of such placements, as one point in a combinatorial
address space, generated whether or not the existing literature already
supports it. Each hypothesis carries a subjective-logic opinion, belief,
disbelief, and uncertainty mass, rather than one point estimate, so a
hypothesis nobody has examined reads apart from one actively refuted. A
structural-unknowns layer gives the engine's own blind spots, an unnamed
concept, an unexcavated gap, undiscovered evidence, a place inside the
model instead of leaving them for a human reviewer to notice later. We
derive the address scheme's size and injectivity, the opinion model's
normalization and probability bound, and lay out an engine loop of
generation, critique, ranking, and evolution, built against a sibling
engine designed for a different corpus. We close with a calibration
design, two holdout tests and two candidate pilot periods, and state what
in the model stays uncalibrated. No results are claimed here; this is a
...
belief model stands in its place. Sakana's AI Scientist runs a tree
search from idea to code to experiment to paper draft, with one output
passing automated peer review \autocite{lu2024aiscientist}; Stanford's
...
result, ten disease campaigns in roughly ten weeks and one validated
...
mechanism. Lean counterpart: \texttt{Bucket.Hypothesis.Placement}.
\end{definition}
...
who or what asserted it, and an edge strength $e \in [0, 1]$. An evidence
item is the argument \texttt{Bucket.Belief.fuse} pools over; it has no
independent Lean type of its own in this pass, since the definitional
weight the paper needs from it, kind, tier, and strength, is exactly the
tuple \texttt{fuse} consumes.
\end{definition}
...
The effective-count discount is threaded through
\texttt{Bucket.Belief.fuse}'s cluster-count argument; Lean counterpart:
\texttt{Bucket.Belief.fuse}.
\end{definition}
...
\Cref{fig:hypothesis-space} plots \Cref{lem:size}'s two sizes alongside a
...
\kappa} s_i$ and $S_{-}$ symmetrically for refuting evidence.
...
\Cref{def:slot}. Lean counterpart: \texttt{Bucket.Belief.fuse}.
\end{definition}
...
\Cref{eq:opinion-sum}, giving $P(h) = a(h)$ exactly: a hypothesis nobody
...
retrieval it names; the active-learning queue this score orders is the
mirror of \Cref{sec:calibration}'s founder-review queue, one ranks what
to judge, the other ranks what to go look at next.
...
\emph{Evolver.} \Cref{def:neighbors}'s one-slot mutation is the evolver's
smallest move; it also recombines two hypotheses sharing a cluster into
...
$\lambda$ (\Cref{eq:diminishing}), $\mu$ (the corpus's original
contradiction penalty, unchanged by the opinion model), and $W$
(\Cref{eq:opinion-sum}), fit by grid search and re-run on a schedule as
the corpus grows. Each recalibration carries a run id, the same audit
pattern the corpus's mirror jobs already write.
...
The constants $W = 2$, $\lambda = 0.5$, and $\mu = 0.5$ carried through
\Cref{eq:opinion-sum}, \Cref{eq:diminishing}, and \Cref{eq:rank} are
uncalibrated: they are the values the corpus's existing truth-score
formula already used, kept fixed through the move to an opinion model
rather than refit to it. \Cref{sec:calibration}'s recalibration pass is
designed but has not run.
...
\texttt{papers/template/lean/}. The six files below are being written in
parallel to this paper by a companion pass; each listing is guarded so
this build compiles whether or not that file has landed yet.
...
\Cref{def:stemma}, \Cref{def:opinion-fusion}, \Cref{lem:opinion-sum}).
\bucketlean{lean-snapshots/Belief.lean}{lean/Bucket/Belief.lean}{Bucket.Belief.Opinion, Bucket.Belief.project, Bucket.Belief.fuse}
...
\texttt{Bucket.Address.encode}'s companion decode function, and
\Cref{lem:injective}'s proof of injectivity, are the two results this
paper needs machine-checked rather than derived by hand; every other
listing above formalizes a definition with no accompanying lemma to
discharge. Detectability (\Cref{def:detect}) and gap nodes
(\Cref{def:gap}) carry no listing here, per \Cref{sec:limitations}, and
no \texttt{lake build} transcript is captured in this appendix: the
build has not run, because the files it would build do not yet exist at
