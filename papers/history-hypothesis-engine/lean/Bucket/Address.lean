...
`TIMELINE-AND-COMBINATORICS-SPEC.md` §3 fixes the first six primes
...
(`TIMELINE-AND-COMBINATORICS-SPEC.md` §3). Each field is the slot's
...
/-- The Gödel address of a slot-index tuple: the product of each slot's
fixed prime raised to the tuple's index at that slot, plus one
(`TIMELINE-AND-COMBINATORICS-SPEC.md` §3, `address(slots)`). -/
...
Settled by exhaustive computation over the 2^6 × 2^6 grid of tuples, no
