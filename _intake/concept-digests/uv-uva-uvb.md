# uv-uva-uvb

**Query**: `UV OR UVA OR UVB OR ultraviolet`  ·  **Hits**: 35


## pubmed (8)

- **Imaging of ultraweak photon emission for evaluating the oxidative stress of human skin.**
  - `pubmed/PMID-31349151-imaging-of-ultraweak-photon-emission-for-evaluating-the-oxid/info.md`
  -  … UPE is detectable from human skin, and its intensity increases by external stress such as <<ultraviolet>> (<<UV>>) irradiation. Presently, UPE measurement is used to … 
- **Full-Spectrum phototherapy in hair loss management: a systematic review of wavelength-dependent mechanisms, clinical efficacy, and future directions.**
  - `pubmed/PMID-40968340-full-spectrum-phototherapy-in-hair-loss-management-a-systema/info.md`
  -  … Specifically, <<UVB>>/<<UVA>> selectively induce apoptosis of T cells, thereby restoring immune privilege in alopecia areata (AA). Red light activates mitochondrial cytochrome c oxidase … 
- **Cannabigerol Reduces Lipid Peroxidation Influencing Oxidative Stress and Inflammation Signaling Pathways in Melanocytes Exposed to UVA Radiation.**
  - `pubmed/PMID-41914282-cannabigerol-reduces-lipid-peroxidation-influencing-oxidativ/info.md`
  -  … 2026-05-10T10:06:40

## Abstract

<<Ultraviolet>> A (<<UVA>>) radiation is a major environmental factor contributing to melanoma development. Melanocytes synthesize melanin, which provides … 
- **Genome-wide gene-environment interaction study uncovers 162 vitamin D status variants using a precise ambient UVB measure.**
  - `pubmed/PMID-41315334-genome-wide-gene-environment-interaction-study-uncovers-162/info.md`
  - # Genome-wide gene-environment interaction study uncovers 162 vitamin D status variants using a precise ambient <<UVB>> measure.

- **PMID**: 41315334
- **DOI**: 10.1038/ng … 
- **An ultraviolet light induced bacteriophage in Beneckea gazogenes.**
  - `pubmed/PMID-314618-an-ultraviolet-light-induced-bacteriophage-in-beneckea-gazog/info.md`
  - # An <<ultraviolet>> light induced bacteriophage in Beneckea gazogenes.

- **PMID**: 314618
- **DOI**: 10.1007/BF00932497
- **PMCID**:  
- **Journal**: Origins of life  ·  **Year**: 1979
- **Authors**: M Rambler … 
- **A novel ZIC1-BMP4 axis identified in photoaged dermal fibroblasts regulates melanogenesis.**
  - `pubmed/PMID-41864822-a-novel-zic1-bmp4-axis-identified-in-photoaged-dermal-fibrob/info.md`
  -  … Humans, Skin Aging, Bone Morphogenetic Protein 4, Transcription Factors, Fibroblasts, Melanocytes, Microphthalmia-Associated Transcription Factor, <<Ultraviolet>> Rays, Signal Transduction, Skin
- **URL**: https://pubmed.ncbi … 
- **Evaluation of subclinical chronic sun damage in the skin via the detection of long-lasting ultraweak photon emission.**
  - `pubmed/PMID-33998715-evaluation-of-subclinical-chronic-sun-damage-in-the-skin-via/info.md`
  -  … To evaluate subclinical photodamage in the skin especially from the early phase of <<ultraviolet>> (<<UV>>)-induced damage, we have focused on ultraweak photon emission … 
- **Ethanolic Extract of**
  - `pubmed/PMID-42074027-ethanolic-extract-of/info.md`
  -  … 2026-05-10T10:06:23

## Abstract

<<Ultraviolet>> (<<UV>>) irradiation induces complex skin damage, including hyperpigmentation, oxidative stress, and alterations in proteins related to keratinocyte … 

## youtube (6)

- **Dr. Jack Kruse-Nourish Vermont 2017 on Circadian Biology and the Sun**
  - `yt/zGAACx89jMU-dr-jack-kruse-nourish-vermont-2017-on-circadian-biology-and-/transcript.txt`
  -  … Okay, just for
those of you who don't know, this is <<UV>>
light, <<UVA>>, and that's red light. But
here's the … 
- **Dr. Jack Kruse Explains How Sunlight Controls Leptin, Melanin & Mitochondrial Repair | Photo-Repair**
  - `yt/yo4h0B_JMQY-dr-jack-kruse-explains-how-sunlight-controls-leptin-melanin-/transcript.txt`
  -  … And then I you know, the other thing I
told you, you don't make melanin from
palm C without <<UV>> light. <<UV>> light … 
- **Banned Neurosurgeon: Decentralize Your Health B4 It's Too Late! | Dr Jack Kruse**
  - `yt/unUTOBIXvdc-banned-neurosurgeon-decentralize-your-health-b4-it-s-too-lat/transcript.txt`
  -  … <<UVA>> and <<UVB>> so the answer&nbsp; there would absolutely to be use a tanning bed&nbsp;&nbsp;
uh that has <<UVA>> and <<UVB>> … 
- **Dr. Jack Kruse Nourish Vermont 2017 Q & A- part 10 of 10**
  - `yt/Uvm0dho_KRM-dr-jack-kruse-nourish-vermont-2017-q-a-part-10-of-10/transcript.txt`
  -  … around
<<uv>> light
you become euphoric why because you
release what beta endorphin
what else your blood pressure goes down
because <<uva>> light
nitric … 
- **Dr Jack Kruse: Light, Melanin, POMC in Human Evolution & Disease**
  - `yt/Ln3WszTq0uA-dr-jack-kruse-light-melanin-pomc-in-human-evolution-disease/transcript.txt`
  -  … comes from
releases extreme low frequency <<UV>> light
that <<UV>> light is you've seen more <<UVA>> or
B UVC you didn't you … 
- **Jack Kruse on How Light sculpts Your Microbiome & Implications for Gut Health and Mental Illness**
  - `yt/g_rlcavhGLs-jack-kruse-on-how-light-sculpts-your-microbiome-implications/transcript.txt`
  -  … light the secondary
control arm happens to be <<UVA>> and <<UVB>> light which is <<ultraviolet>> a and B and
those control arms are not … 

## intake (21)

- **uv-uva-uvb**
  - `_intake/concept-digests/uv-uva-uvb.md`
  - # <<uv>>-<<uva>>-<<uvb>>

**Query**: `<<UV>> OR <<UVA>> OR <<UVB>> OR <<ultraviolet>>`  ·  **Hits**: 35


## pubmed (7)

- **Imaging of ultraweak photon emission for evaluating the oxidative … 
- **biophoton-popp**
  - `_intake/concept-digests/biophoton-popp.md`
  -  … 30 … 
- **<<uv>>-<<uva>>-<<uvb>>**
  - `_intake/concept-digests/<<uv>>-<<uva>>-<<uvb>>.md`
  -  … 30


## pubmed (2)

- **Imaging of <<ultraweak photon>> emission for evaluating the oxidative stress … 
- **REALITY #3:  WAKE ME UP WHEN SEPTEMBER 2016 ENDS**
  - `_intake/kruse-blog-corpus/articles/reality-3-wake-september-2016-ends.md`
  -  … Moreover, <<UV>> light is known to break down most of the sex steroids in the skin under sun exposure. <<UVA>> and <<UVB>> do this … 
- **vitamin-d-sunlight**
  - `_intake/concept-digests/vitamin-d-sunlight.md`
  -  … level of <<Vitamin>> … 
- **<<uv>>-<<uva>>-<<uvb>>**
  - `_intake/concept-digests/<<uv>>-<<uva>>-<<uvb>>.md`
  -  … What makes nitric
oxide? <<<<UV>>>> light. <<<<UV>>>>.
&gt;&gt; What makes … 
- **Fritz-Albert Popp**
  - `_intake/canon-profiles/popp-fritz-albert.md`
  -  … 25


## pubmed (6)

- **<<Biophoton>> emission of human body.**
  - `pubmed … 
- **<<uv>>-<<uva>>-<<uvb>>**
  - `_intake/concept-digests/<<uv>>-<<uva>>-<<uvb>>.md`
  -  … wavelengths of <<<<UVA>>>>, with the … 
- **TENSEGRITY #14: COMMUNICATION BREAKDOWN IN THE GUT**
  - `_intake/kruse-blog-corpus/articles/tensegrity-8-communication-breakdown-gut.md`
  -  … Of the <<ultraviolet>> radiation that reaches the Earth’s surface, more than 95% is the longer wavelengths of <<UVA>>, with the small remainder <<UVB>> … 
- **methylene-blue**
  - `_intake/concept-digests/methylene-blue.md`
  -  … concept-digests/AGEs-RAGE … 
- **<<uv>>-<<uva>>-<<uvb>>**
  - `_intake/concept-digests/<<uv>>-<<uva>>-<<uvb>>.md`
  -  … intake/concept-digests/<<<<uv>>>>-<<<<uva>>>>-<<<<uvb>>>>.md`
  -  … PSORIASIS … 
- **<<methylene-blue … 
- **TIME #9: THE "DARK KNIGHT" OF REGENERATION?**
  - `_intake/kruse-blog-corpus/articles/the-dark-knight.md`
  -  … us?  <<UVA>> light shows up in late AM and earlier than <<UVB>> light which shows up closer to midday.  All frequencies of <<UV>> light … 
- **Concept Digests**
  - `_intake/concept-digests/INDEX.md`
  -  … citers:4, openalex-fanout:4, pubmed:19, youtube:1)
- [`<<uv>>-<<uva>>-<<uvb>>`](<<uv>>-<<uva>>-<<uvb>>.md) — 35 hits  (intake:24, pubmed:7, youtube:4 … 
- **TIME #8:  PSORIASIS AND THE PHOTOELECTRIC EFFECT**
  - `_intake/kruse-blog-corpus/articles/time-8-psoriasis-and-the-photoelectric-effect.md`
  -  … absorb the <<UVA>> and <<UVB>> light at the surface.  <<UVA>> and <<UVB>> light does not penetrate deep.  TO absorb the <<UV>> light we need … 
- **Per-Video References**
  - `_intake/kruse-references-mined/PER-VIDEO.md`
  -  … <<UV>>(35), vitamin D(23), mitochondria(19), WHO(19), AGE(13), iron(12), <<UVA>>(9), mitochondrial(7), <<UVB>>(7), hemoglobin(7), cytochrome(5), ATP … 
- **TIME #4: HOW CAN MAN HELP OUR EYE CLOCK IN A TECH WORLD?**
  - `_intake/kruse-blog-corpus/articles/how-man-helps-the-eye-clock.md`
  -  … That <<UVA>> or <<UVB>> light is not present 24/7 on the globe anywhere.* This includes the equator because of how <<UV>> light traverses … 
- **TIME #11   CAN YOU SUPPLEMENT SUNLIGHT?**
  - `_intake/kruse-blog-corpus/articles/time-10-can-you-supplement-sunlight.md`
  -  … of <<UVB>> light at our skin surface when Vitamin D3 is made.  Experiments have proven this.   What happens if you have no <<UV>> light … 
- **Quantum Thermodynamics #2: Why stopping oral vitamin D3 in a 5G world is wise**
  - `_intake/kruse-blog-corpus/articles/quantum-thermodynamics-2-why-stopping-oral-vitamin-d3-in-a-5g-world-is-wise.md`
  -  … in the skin by <<UV>> light exposure of the skin…….so if you do not get the <<UVA>> and <<UVB>> light frequencies in diurnal … 
- **TIME # 19:  IS TIME TOLD BY BIOPHOTON EMISSION?**
  - `_intake/kruse-blog-corpus/articles/time-for-biophotons.md`
  -  … humans need strong <<UV>> light every day at some level to keep autophagy and apoptosis working to self-regulate themselves.**  <<UVA>> light is present … 
- **UBIQUITINATION 20:  LINKING LIGHT TO PLANTS AND ANIMALS**
  - `_intake/kruse-blog-corpus/articles/ubiquitination-20-linking-light-to-plants-and-animals.md`
  -  … Something must have constricted the spectrum of light between <<UV>> and IR for a reason.

I decided to look into the topic our perception … 
- **TIME #6:  IS BIOLOGIC TIME LINKED TO DOPAMINE LEVELS?**
  - `_intake/kruse-blog-corpus/articles/time-6-time-and-dopamine.md`
  -  … Bright full spectrum light contains <<UVA>> light.  When <<UV>> light is missing dopamine levels are lower in the eye.  Anytime dopamine is lowered, it … 
- **TIME #20:  IS FAT BURNING DUE TO 100 HZ VIBRATION IN MITOCHONDRIA?**
  - `_intake/kruse-blog-corpus/articles/time-20-fat-burning-due-100-hz-vibration-mitochondria.md`
  -  … the DC electric are present it is a signal that <<UVA>> or <<UVB>> light is present to generate enough current and voltage to affect … 
- **TENSEGRITY #7:  BLACK HOLE SUN,  HERE WE COME**
  - `_intake/kruse-blog-corpus/articles/tensegrity-7-black-hole-sun-come.md`
  -  … eNOS stimulates nitric oxide from L- Arginine and it stimulates sulfate production from sulfur in the presence of <<UVB>> and IR light. **eNOS switches … 
- **TIME#16: TIME SUBTRACTION = LOSS OF THE SPINNING ATPase**
  - `_intake/kruse-blog-corpus/articles/time-subtraction-cancer-diagnosis.md`
  -  … and glutamine to maintain ECT function while inhibiting apoptosis because <<UVA>> and <<UVB>> light cannot stop ECT flow before the ATPase.  Anytime intracellular water … 
- **UBIQUITINATION #24: ARE MYOPIA, LIGHT, AND AGING LINKED?**
  - `_intake/kruse-blog-corpus/articles/ubiquitination-24-are-myopia-light-and-aging-linked.md`
  -  … the far shortwave (<<UVA>>), except for trace amounts.  Natural sunlight, however, allows [quite a bit of near long wavelength <<UV>> light.](http://nvlpubs.nist … 