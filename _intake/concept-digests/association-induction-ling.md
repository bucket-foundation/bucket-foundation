# association-induction-ling

**Query**: `Ling AND (association OR induction OR cells AND gels)`  ·  **Hits**: 35


## pubmed (11)

- **A quantitative theory of solute distribution in cell water according to molecular size.**
  - `pubmed/PMID-8115492-a-quantitative-theory-of-solute-distribution-in-cell-water-a/info.md`
  -  … 2026-05-09T09:41:35

## Abstract

A quantitative theory (based on polarized multilayer (PM) theory of <<cell>> water, a subsidiary of the <<association>>-<<induction>> … 
- **How much water is made "non-free" by 36% native hemoglobin?**
  - `pubmed/PMID-16268125-how-much-water-is-made-non-free-by-36-native-hemoglobin/info.md`
  -  … 100 microm from the surface of polyvinylalcohol <<gel>> (Zheng and Pollack) --in the light of <<Ling>>'s new theory of ad infinitum water polarization … 
- **THE ASSOCIATION-INDUCTION HYPOTHESIS.**
  - `pubmed/PMID-14157817-the-association-induction-hypothesis/info.md`
  - # THE <<ASSOCIATION>>-<<INDUCTION>> HYPOTHESIS.

- **PMID**: 14157817
- **DOI**: 
- **PMCID**:  
- **Journal**: Texas reports on biology and medicine  ·  **Year**: 1964
- **Authors**: G N <<LING>>
- **MeSH**: Adenosine Triphosphate … 
- **Active solute transport across frog skin and epithelial cell systems according to the association-induction hypothesis.**
  - `pubmed/PMID-7330099-active-solute-transport-across-frog-skin-and-epithelial-cell/info.md`
  -  … according to the <<association>>-<<induction>> hypothesis.

- **PMID**: 7330099
- **DOI**: 
- **PMCID**:  
- **Journal**: Physiological chemistry and physics  ·  **Year**: 1981
- **Authors**: G N <<Ling>>
- **MeSH**: Animals, Biological … 
- **Studies on ion accumulation in muscle cells.**
  - `pubmed/PMID-5943617-studies-on-ion-accumulation-in-muscle-cells/info.md`
  -  … according to the membrane theory (Donnan equilibrium) and according to the <<association>>-<<induction>> hypothesis. This comparison shows that both theories predict competitive effects of … 
- **The physical state of solutes and water in living cells according to the association-induction hypothesis.**
  - `pubmed/PMID-4577275-the-physical-state-of-solutes-and-water-in-living-cells-acco/info.md`
  - # The physical state of solutes and water in living cells according to the <<association>>-<<induction>> hypothesis.

- **PMID**: 4577275
- **DOI**: 10.1111/j.1749-6632 … 
- **What determines the normal water content of a living cell?**
  - `pubmed/PMID-15789970-what-determines-the-normal-water-content-of-a-living-cell/info.md`
  -  … Gilbert <<Ling>>
- **MeSH**: Animals, Anura, Body Water, <<Cell>> Membrane, <<Cell>> Membrane Permeability, <<Cell>> Polarity, <<Cell>> Survival, Centrifugation, Densitometry, Electrophoresis, Polyacrylamide <<Gel>>
- **URL**: https://pubmed … 
- **The association-induction hypothesis. A theoretical foundation provided for the possible beneficial effects of a low Na, high K diet and other similar regimens in the treatment of patients suffering from debilitating illnesses.**
  - `pubmed/PMID-6638318-the-association-induction-hypothesis-a-theoretical-foundatio/info.md`
  - # The <<association>>-<<induction>> hypothesis. A theoretical foundation provided for the possible beneficial effects of a low Na, high K diet and other similar regimens … 
- **Theory of active transport across frog skin and other bifacial cell systems: a subsidiary of the association-induction hypothesis.**
  - `pubmed/PMID-2080435-theory-of-active-transport-across-frog-skin-and-other-bifaci/info.md`
  -  … a subsidiary of the <<association>>-<<induction>> hypothesis.

- **PMID**: 2080435
- **DOI**: 
- **PMCID**:  
- **Journal**: Scanning microscopy  ·  **Year**: 1990
- **Authors**: G N <<Ling>>
- **MeSH**: Animals, Anura, Biological … 
- **A physical theory of the living state: application to water and solute distribution.**
  - `pubmed/PMID-3399856-a-physical-theory-of-the-living-state-application-to-water-a/info.md`
  -  … pump theory and the alternative theory of the living cell, the <<association>>-<<induction>> (AI) hypothesis. Being alive in the AI hypothesis represents the maintenance … 
- **How does reduced external K+ concentration affect the rate of Na+ efflux? Evidence against the K-Na coupled pump but in support of the association-induction hypothesis.**
  - `pubmed/PMID-311014-how-does-reduced-external-k-concentration-affect-the-rate-of/info.md`
  -  … support of the <<association>>-<<induction>> hypothesis.

- **PMID**: 311014
- **DOI**: 
- **PMCID**:  
- **Journal**: Physiological chemistry and physics  ·  **Year**: 1978
- **Authors**: G N <<Ling>>
- **MeSH**: Animals, Anura … 

## youtube (6)

- **Who the heck is Gilbert Ling?**
  - `yt/qHnE8yfuT_E-who-the-heck-is-gilbert-ling/transcript.txt`
  -  … called
the <<association>> <<induction>> hypothesis
one of his first assumptions is the
cytoplasm of the <<cell>> is highly
structured a <<gel>>-like matrix of … 
- **Cell Permeability based on the Ai Hypothesis - Gilbert Ling**
  - `yt/L3W-yuOyE2M-cell-permeability-based-on-the-ai-hypothesis-gilbert-ling/transcript.txt`
  -  … structured ways <<Ling>> emphasizes two key
Concepts <<Association>> and <<induction>> okay
break those down for me what do those
actually mean <<Association>> refers to … 
- **Gerald Pollack: Water & Rethinking Cell Biology | Ricci Flow Nutrition Podcast**
  - `yt/iK4gG7TBgNM-gerald-pollack-water-rethinking-cell-biology-ricci-flow-nutr/transcript.txt`
  -  … dozens of peer-reviewed journal
articles and several books including
<<cell>>'s <<gels>> in the engines of life as
well as the fourth phase … 
- **Dr Gerald Pollack _The Importance Of Water For Energy _Heart Health _Memory & Ageing**
  - `yt/EWthpbsfMJI-dr-gerald-pollack-the-importance-of-water-for-energy-heart-h/transcript.txt`
  -  … I from what Gilbert <<Ling>> was
thinking and and what I wrote about in
in the first book, <<Cells>>, <<Gels>>, and the
Engines of … 
- **Gerald Pollack| EZ Water, What is it, Why Do I Need It & How Do I Make It**
  - `yt/47FzeZNCYL0-gerald-pollack-ez-water-what-is-it-why-do-i-need-it-how-do-i/transcript.txt`
  -  … <<gel>> right and it turns out
turns out that if you stick an electrode
into a <<gel>> not the <<cell>> into a <<gel>> you … 
- **THE FOURTH PHASE OF WATER - Gerald Pollack PhD #79**
  - `yt/dOJt-wFNe3E-the-fourth-phase-of-water-gerald-pollack-phd-79/transcript.txt`
  -  … He's
the author of over 300 scientific papers
and four books for the general public,
including <<Cells>>, <<Gels>>, and the Engines
of Life … 

## intake (12)

- **association-induction-ling**
  - `_intake/concept-digests/association-induction-ling.md`
  - # <<association>>-<<induction>>-<<ling>>

**Query**: `<<Ling>> AND (<<association>> OR <<induction>> OR <<cells>> AND <<gels>>)`  ·  **Hits**: 35


## pubmed (11)

- **A quantitative theory of solute distribution in … 
- **Gilbert Ling**
  - `_intake/canon-profiles/ling-gilbert.md`
  -  … <<association>>-<<induction>>-<<ling>>**
  - `_intake/concept-digests/<<association>>-<<induction>>-<<ling>>.md`
  - # <<<<association>>>>-<<induction>>-<<<<ling>>>>

**Query**: `<<<<Ling>>>> AND (<<<<association>>>> OR <<induction>> OR <<<<cells>>>> AND <<<<gels>>>>)`  ·  **Hits … 
- **Gerald Pollack**
  - `_intake/canon-profiles/pollack-gerald.md`
  -  … <<cell>>>> … 
- **<<ez>>-<<water>>-<<pollack>>**
  - `_intake/concept-digests/<<ez>>-<<water>>-<<pollack>>.md … 
- **<<association>>-<<induction>>-<<ling>>**
  - `_intake/concept-digests/<<association>>-<<induction>>-<<ling>>.md`
  -  … <<cells>>>>>> AND <<<<<<gels>> … 
- **ez-water-pollack**
  - `_intake/concept-digests/ez-water-pollack.md`
  -  … of-<<water>>-why-the-<<water>> … 
- **<<association>>-<<induction>>-<<ling>>**
  - `_intake/concept-digests/<<association>>-<<induction>>-<<ling>>.md`
  -  … <<Water>> & Rethinking <<Cell>> Biology | Ricci Flow Nutrition Podcast**
  - `yt … 
- **fourth-phase-water**
  - `_intake/concept-digests/fourth-phase-water.md`
  -  … The <<<<<<4th Phase>>>>>> of <<Water … 
- **<<melanin>>-<<semiconductor … 
- **<<association>>-<<induction>>-<<ling>>**
  - `_intake/concept-digests/<<association>>-<<induction>>-<<ling>>.md`
  -  … Gerald Pollack)**
  - `yt/uEwYlfekOIY-homeopathy-new-evidence … 
- **Bucket Foundation — Research Mapping Queue**
  - `_intake/RESEARCH-MAPPING-QUEUE.md`
  -  … <<induction>>-1962` | Gilbert <<Ling>> — *A Physical Theory of the Living State* + <<Association>>-<<Induction>> Hypothesis papers | Kruse cites; alternative to membrane-pump theory. <<Cells>> & <<gels>> … 
- **Energy and Epigenetics 10: The Quantum Puzzle**
  - `_intake/kruse-blog-corpus/articles/energy-epigenetics-10-quantum-puzzle.md`
  -  … to listen to, let alone accommodate, the alternate, <<Ling>>/Pollack (<<Associated>> <<Induction>>) understanding of the <<cell>> and its membrane. But, Fahnestock contends, there are … 
- **chemiosmotic-mitchell**
  - `_intake/concept-digests/chemiosmotic-mitchell.md`
  -  … blog:1, intake:12, openalex-citers:6 … 
- **<<association>>-<<induction>>-<<ling>>**
  - `_intake/concept-digests/<<association>>-<<induction>>-<<ling>>.md`
  -  … need 5000 times more ATP than a … 
- **Peter Mitchell**
  - `_intake/canon-profiles/mitchell-peter.md`
  -  … In the mitochondrial matrix, <<protons>> are stripped of most … 
- **<<association>>-<<induction>>-<<ling>>**
  - `_intake/concept-digests/<<association>>-<<induction>>-<<ling>>.md`
  -  … need 5000 times more ATP … 
- **CPC #13:   HYPERFERRITIN CATARACT SYNDROME**
  - `_intake/kruse-blog-corpus/articles/cpc-13-hyperferritin-cataract-syndrome.md`
  -  … <<cell>> and I no longer do because of <<Ling>> and Pollack.  <<Ling>>’s calculation that we need 5000 times more ATP than a <<cell>> … 
- **Gilbert Ling — canon-target intake**
  - `_intake/ling-gilbert/README.md`
  -  … State (1962); <<Association>>-<<Induction>> Hypothesis papers

## Source candidates (in priority order)

- gilbertling.org / physiologicalchemistryandphysics.com
- archive.org: book + papers
- PubMed: <<Ling>> GN

## License / copyright … 
- **Concept Digests**
  - `_intake/concept-digests/INDEX.md`
  -  … openalex:2, openalex-fanout:3, pubmed:7, youtube:6)
- [`<<association>>-<<induction>>-<<ling>>`](<<association>>-<<induction>>-<<ling>>.md) — 35 hits  (intake:12, openalex:2, openalex-citers … 