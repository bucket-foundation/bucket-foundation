# iron-heme-redox

**Query**: `iron AND (heme OR redox)`  ·  **Hits**: 30


## youtube (1)

- **Interview with Dr. Jack Kruse (04-08-2025)**
  - `yt/wwNutyiyQ2I-interview-with-dr-jack-kruse-04-08-2025/transcript.txt`
  -  … Why did they choose <<iron>> as the first
<<heme>> protein to be innovated? Because
<<iron>> has two oxidation states, +2 and +3
as I … 

## intake (29)

- **iron-heme-redox**
  - `_intake/concept-digests/iron-heme-redox.md`
  - # <<iron>>-<<heme>>-<<redox>>

**Query**: `<<iron>> AND (<<heme>> OR <<redox>>)`  ·  **Hits**: 30


## youtube (1)

- **Interview with Dr. Jack Kruse (04-08-2025)**
  - `yt/wwNutyiyQ2I-interview … 
- **Per-Video References**
  - `_intake/kruse-references-mined/PER-VIDEO.md`
  -  … 13), <<redox>>(8), Becker(7), WHO(6), electromagnetic(5), copper(4), <<iron>>(3), EPA(2), photoelectric effect(1), photoelectric(1), collagen(1), <<heme>>(1 … 
- **exclusion-zone-water**
  - `_intake/concept-digests/exclusion-zone-water.md`
  -  … md`
  -  … Cruz … 
- **<<iron>>-<<heme>>-<<redox>>**
  - `_intake/concept-digests/<<iron>>-<<heme>>-<<redox>>.md`
  -  … 95
- **melatonin** — 91
- **methylene blue** — 81
- **<<exclusion>> <<zone>>** — 77
- **<<<<redox>>>>** — 73
- **UVA … 
- **methylene-blue**
  - `_intake/concept-digests/methylene-blue.md`
  -  … BIOHACKING “TIME” WITH <<METHYLENE BLUE>>



**BLOG TAKE AWAY … 
- **<<iron>>-<<heme>>-<<redox>>**
  - `_intake/concept-digests/<<iron>>-<<heme>>-<<redox>>.md`
  -  … This lowers are <<<<redox>>>> power … 
- **<<methylene … 
- **AGEs-RAGE-glycation**
  - `_intake/concept-digests/AGEs-RAGE-glycation.md`
  -  … The hydroxyl radical can damage virtually all types of macromolecules by energizing their … 
- **<<iron>>-<<heme>>-<<redox>>**
  - `_intake/concept-digests/<<iron>>-<<heme>>-<<redox>>.md`
  -  … This … 
- **biophoton-popp**
  - `_intake/concept-digests/biophoton-popp.md`
  -  … IS TIME TOLD BY <<BIOPHOTON>> EMISSION?



**THE TAKE HOME … 
- **<<iron>>-<<heme>>-<<redox>>**
  - `_intake/concept-digests/<<iron>>-<<heme>>-<<redox>>.md`
  -  … IS TIME TOLD BY <<BIOPHOTON … 
- **Concept Digests**
  - `_intake/concept-digests/INDEX.md`
  -  … RAGE-glycation.md) — 30 hits  (intake:29, youtube:1)
- [`<<iron>>-<<heme>>-<<redox>>`](<<iron>>-<<heme>>-<<redox>>.md) — 30 hits  (intake:29, youtube:1)
- [`photosynthesis-quantum … 
- **melanin-semiconductor**
  - `_intake/concept-digests/melanin-semiconductor.md`
  -  … Deuterium, <<4th phase>> of WATER, & cellular <<redox>> | Regenerative … 
- **<<iron>>-<<heme>>-<<redox>>**
  - `_intake/concept-digests/<<iron>>-<<heme>>-<<redox>>.md`
  -  … 14 hits  (intake:2, youtube:12 … 
- **deuterium-depleted-water**
  - `_intake/concept-digests/deuterium-depleted-water.md`
  -  … in our <<water>> … 
- **<<iron>>-<<heme>>-<<redox>>**
  - `_intake/concept-digests/<<iron>>-<<heme>>-<<redox>>.md`
  -  … <<deuterium>>-4th-phase-of-<<water>>-cellular-<<<<redox>>>>-re/transcript … 
- **<<deuterium>>-depleted … 
- **Fritz-Albert Popp**
  - `_intake/canon-profiles/popp-fritz-albert.md`
  -  … biology that studies light is … 
- **<<iron>>-<<heme>>-<<redox>>**
  - `_intake/concept-digests/<<iron>>-<<heme>>-<<redox>>.md`
  -  … UVA** — 62
- **<<<<heme>>>>** — 58
- **<<biophoton>>** — 55
- **vitamin A** — 54 … 
- **photoelectric-biology**
  - `_intake/concept-digests/photoelectric-biology.md`
  -  … biology>>.md) — 30 hits  (intake:30)
- [`biophoton … 
- **<<iron>>-<<heme>>-<<redox>>**
  - `_intake/concept-digests/<<iron>>-<<heme>>-<<redox>>.md`
  -  … THE <<PHOTOELECTRIC>> EFFECT**
  - `_intake/kruse-blog-corpus … 
- **fourth-phase-water**
  - `_intake/concept-digests/fourth-phase-water.md`
  -  … cellular-<<redox>>-re/transcript … 
- **<<iron>>-<<heme>>-<<redox>>**
  - `_intake/concept-digests/<<iron>>-<<heme>>-<<redox>>.md`
  -  … semiconductor.md`
  -  … Deuterium, <<4th phase>> of WATER, & cellular <<<<redox>>>> | Regenerative … 
- **photosynthesis-quantum**
  - `_intake/concept-digests/photosynthesis-quantum.md`
  -  … Light had to be connected to food because of how <<photosynthesis>> creates … 
- **Concept Digests**
  - `_intake/concept-digests/INDEX.md`
  -  … <<iron>>-<<heme>>-<<redox>>.md) — 30 … 
- **CPC #13:   HYPERFERRITIN CATARACT SYNDROME**
  - `_intake/kruse-blog-corpus/articles/cpc-13-hyperferritin-cataract-syndrome.md`
  -  … <<iron>> cycles are not controlled in our blood plasma it can oxidize our mitochondria in tissues where this occurs. This lowers are <<redox>> power … 
- **Kruse References Mined**
  - `_intake/kruse-references-mined/REFERENCES.md`
  -  … 95
- **melatonin** — 91
- **methylene blue** — 81
- **exclusion zone** — 77
- **<<redox>>** — 73
- **UVA** — 62
- **<<heme>>** — 58
- **biophoton** — 55
- **vitamin A** — 54
- **FDA** — 50
- **NIH** — 49 … 
- **TENSEGRITY #7:  BLACK HOLE SUN,  HERE WE COME**
  - `_intake/kruse-blog-corpus/articles/tensegrity-7-black-hole-sun-come.md`
  -  … The cysteine stores use vitamin B12 which fits perfectly with nNOS hydrophilic <<heme>> pocket. It then binds some glutathione from the RBC in the … 
- **ORGANIZATIONAL STRUCTURAL FAILURE #6:  THE MITOCHONDRIAL Rx**
  - `_intake/kruse-blog-corpus/articles/organizational-structural-failure-6-mitochondrial-rx.md`
  -  … These electrons are held by the <<redox>> potential of oxygen (+0.82V). They are pumped uphill to carbon atoms that are partially reduced to … 
- **REALITY# 8:  HOW WOMEN GAIN TIME AT MENOPAUSE**
  - `_intake/kruse-blog-corpus/articles/reality-8-women-gain-time-menopause.md`
  -  … Ferritin is the major storage protein for <<iron>>. Free <<iron>> is toxic for human cells because of its reactivity with other <<redox>> chemicals but … 
- **Energy and Epigenetics 8: Quantum Autism**
  - `_intake/kruse-blog-corpus/articles/energy-epigenetics-8-autism.md`
  -  … real epigenetic trigger.**

## The Biology of <<Iron>> and Copper

<<Iron>> is an essential mineral in the <<heme>> molecule of hemoglobin and the cytochrome proteins … 
- **ORGANIZATIONAL STRUCTURAL FAILURE #4:  HUMAN PRIONS**
  - `_intake/kruse-blog-corpus/articles/organizational-structural-failure-4-human-prions.md`
  -  … <<Iron>> content affects water chemistry in a huge way.  When <<iron>> levels drop in the brain, blood flow decreases and we lose oxygen tensions … 
- **TIME # 19:  IS TIME TOLD BY BIOPHOTON EMISSION?**
  - `_intake/kruse-blog-corpus/articles/time-for-biophotons.md`
  -  … an exciton with ease but it cannot do it to <<heme>> proteins with <<iron>>.  Recall that plants evolved before animals by about 50 million … 
- **THE DOPAMINE Rx  =  GOOD CHOICES OR BAD**
  - `_intake/kruse-blog-corpus/articles/the-dopamine-rx-good-choices-or-bad.md`
  -  … Globally, <<iron>> deficiency is the major cause of neuro-developmental delay in malnourished children (Pollitt, 1993) Moreover, <<iron>> is also a co factor in … 
- **UBIQUITINATION 12:  NITROGEN CYCLE ECODYNAMICS**
  - `_intake/kruse-blog-corpus/articles/ubiquitination-12-nitrogen-cycle-ecodynamics.md`
  -  … Hemoglobin, another <<heme>>/porphyrin protein, carries oxygen to muscles from our lungs has four nitrogen atoms that cradle each rust colored <<iron>> atom within … 
- **REALITY #3:  WAKE ME UP WHEN SEPTEMBER 2016 ENDS**
  - `_intake/kruse-blog-corpus/articles/reality-3-wake-september-2016-ends.md`
  -  … The <<iron>> and textile industries, along with the development of the steam engine, played central roles in the Industrial Revolution, which also saw improved … 
- **TIME #21:  SEMICONDUCTION REGENERATES TIME FOR CELLS**
  - `_intake/kruse-blog-corpus/articles/time-21-semiconduction-regenerates-time-cells.md`
  -  … This changes the optical and magnetic transparency.  Mitochondrial cytochromes have <<iron>>-sulfur couples. The <<iron>> and sulfur are quantum dots and can be programmed … 
- **TIME #6:  IS BIOLOGIC TIME LINKED TO DOPAMINE LEVELS?**
  - `_intake/kruse-blog-corpus/articles/time-6-time-and-dopamine.md`
  -  … When our eyes get stabbed by the flash of neon lights, that we <<ironically>> create, that “progress”, as we call it,  splits “our nights … 
- **TIME #7: THE PHOTOELECTRIC EFFECT**
  - `_intake/kruse-blog-corpus/articles/time-7-photoelectric-effect.md`
  -  … All <<heme>> proteins absorb red light.  Cytochrome C in mitochondria is a <<heme>> protein.  If not enough red light can get to this complex … 
- **ORGANIZATIONAL STRUCTURAL FAILURE # 5  PRIONS ARE COMPLIANT DESIGN FLAWS**
  - `_intake/kruse-blog-corpus/articles/organizational-structural-failure-5-prions-are-compliant-design-flaws.md`
  -  … Normally autophagy can go on indefinitely and get rid of mis-folded proteins if the <<redox>> potential stays high. The <<redox>> potential is 100 … 
- **ORGANIZATIONAL STRUCTURAL FAILURE #10:  MULTIPLE SCLEROSIS**
  - `_intake/kruse-blog-corpus/articles/organizational-structural-failure-10-multiple-sclerosis.md`
  -  … Cytochrome proteins normally use unpaired electrons, that contain ferrous metals (<<Iron>>) in their chemistry, which increases their magnetic susceptibility signal.  <<Iron>> is a transition … 