...
`RESEARCH-OS-K12-SYSTEM-REVIEW.md` and, on the `feat/research-os-k12` branch, `learning/research-
os/PLAN.md`) is a knowledge-graph learning platform: a student moves through five states per concept
node, works inside a constrained four-tool AI workspace, and can submit a production, a claim with
evidence and sources, for teacher review and payment. `tools/hypothesis-engine/` (merged in PR #2 and
PR #4) is a combinatorial hypothesis-generation engine: it fills concept slots at a time-binned
address, fuses evidence into a subjective-logic opinion, ranks the surviving population in a
tournament, and names the gaps in its own corpus as first-class nodes. Two teams building two things
independently arrived at the same shape twice: a graph of claims, evidence, and provenance, gated by a
constrained AI surface, scored for belief, and priced for payment. This document maps where the shapes
touch in code, and where they only look alike from a distance.
...
`PLAN.md` §5 (on `feat/research-os-k12`, not yet merged to `main`) gives a production a YAML shape:
...
| `learning/research-os/PLAN.md` §5 (`feat/research-os-k12` branch) | The production schema's source of truth and the HCI-review overlap map this document reads from | Written, branch not merged |
| `_intake/research-os-k12/RESEARCH-OS-K12-SYSTEM-REVIEW.md` §3 | Research OS's own graph schema, learner states, and four-tool workspace design | Written, local to the main tree |
...
3. **Does frontier-backward routing beat forward sequencing on retention and transfer.** Barnett and
   ...
   as a hypothesis awaiting the test its own section 7 describes.
4. **Do AI-generated hypotheses meet the novelty and testability bar a domain scientist applies, and
   ...
   generated hypotheses, read alongside Auchincloss and others (2014) on the missing-instrument and
   reporting-quality problem `PLAN.md` §5 already cites as a constraint on the production record.
...
8. **Does named human sign-off change how a downstream system should weight a claim's detectability and
   ...
   review ladder should set the tier a production enters belief fusion at.
...
10. **What division of cognitive labor between a student-researcher and the AI holds across the five
    ...
    labor in research, read against `PLAN.md` §5's own claim that the generator role never belongs in a
    student workspace at any age.
11. **Does the burden of knowledge predict where an engine's gap nodes cluster, and does routing
    ...
    GapNode` clustering as a measurable, generational signal rather than a per-run artifact.
