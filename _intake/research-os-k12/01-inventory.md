...
**Test setup**: none, in the conventional sense. No Jest/Vitest/Playwright config, no `*.test.*` files under `src/`. `package.json` `"test"` script runs one script, `scripts/test-kruse-token.ts`, a single manual token check. The standalone `learning/app/` sub-product has its own ad hoc node test scripts (`test-adaptive.js`, `test-assess-flow.mjs`, `test-assess.mjs`, `test-diagnostic.mjs`, `test-explorer.mjs`, `test-headless-flow.mjs`, `test-lang-flow.mjs`), none wired to CI.
...
No lint/typecheck/test/build workflow exists. Deploy is presumably Vercel's GitHub auto-deploy (`.vercel/project.json` present, `@vercel/analytics` + `@vercel/speed-insights` wired in), configured outside this repo's own workflow files.
...
Two Dynamic env-id vars are listed (`NEXT_PUBLIC_DYNAMIC_ENV_ID` and `NEXT_PUBLIC_DYNAMIC_ENVIRONMENT_ID`), an unreconciled duplication worth treating as live.
...
| `/api/photon/[id]`, `/api/photon/search` | A separate content surface ("photon"), undocumented elsewhere in the files read |
...
**Supabase migrations**: only 2 files exist, both scoped to the Academy only, outside the canon and the main site:
...
No other tables exist in this repo (no `profiles`, `courses`, `enrollments`, `classrooms`, `assignments`, `roles` tables). There is no migration for canon content, canon claims, bridges, papers, or research tools, those all live as static files (`.md`/`.yaml`/`.json`) in the repo tree, read from disk at request time or build time.
...
- **Supabase Auth** (passwordless email-OTP), used for **Academy learners** (progress sync, credentials). Confirmed in the migration SQL comments; no separate provider file was read directly.
...
### Canon
1,129 files total. Now **11 top-level branches**, up from the 7 named in the org CLAUDE.md (which is stale on this point):
...
**Entry frontmatter** (`primary-papers.yaml` record shape): `id (bkt-<hash>), title, authors:[{family,given,orcid}], year, venue:{name,issn,publisher}, doi, canonical_url, oa_status:{is_oa,oa_url,license,repository}, citation_count, concepts:[...], sources_consulted, fetched_at, canon_score (0-100), canon_score_reasons:[...], canon_branch_hints`. This is a **citation/provenance** schema (bibliographic quality gate), distinct from a pedagogical or prerequisite schema: no tier, no grade level, no "requires" field.
...
### The Academy content layer: the actual pedagogical unit
...
| `06-cosmology` | 59 |
| `07-mind` | 60 |
| `lang-core` (a language deck, outside the canon branches) | 244 |
...
- **Licensing commitments**: MIT for code (`package.json` `"license": "MIT"`, `LICENSE` file present), CC-BY-4.0 for canon prose per multiple corpus-meta notes (e.g. biophysics corpus `license: "CC-BY-4.0 (prose) / facts not copyrightable (equations)"`), CC0-in-intent for the feed402 protocol spec (per org CLAUDE.md; this repo's own `PROTOCOL.md` license line was not independently re-verified).
- **Design constraint that most limits an education-OS pivot**: canon holds **foundations only** (axioms/laws/principles/primary derivations), short of the applied, worked, scaffolded content a K-12 learner needs day to day; that gap is exactly what the separate `learning/` Academy layer was built to bridge, and it currently only reaches 7 STEM branches at adult/general-exam depth.
