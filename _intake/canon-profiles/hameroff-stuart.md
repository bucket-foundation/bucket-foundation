# Stuart Hameroff

**Slug**: `hameroff-stuart`  ·  **FTS query**: `Hameroff AND (microtubule OR consciousness OR Orch)`  ·  **Hits**: 30


## pubmed (14)

- **How quantum brain biology can rescue conscious free will.**
  - `pubmed/PMID-23091452-how-quantum-brain-biology-can-rescue-conscious-free-will/info.md`
  -  … The Penrose-<<Hameroff>> theory of "orchestrated objective reduction (<<Orch>> OR)" identifies discrete conscious moments with quantum computations in <<microtubules>> inside brain neurons, e.g … 
- **'Orch OR' is the most complete, and most easily falsifiable theory of consciousness.**
  - `pubmed/PMID-33232193-orch-or-is-the-most-complete-and-most-easily-falsifiable-th/info.md`
  -  … Stuart <<Hameroff>>
- **MeSH**: Biophysical Phenomena, Brain, <<Consciousness>>, Humans, <<Microtubules>>, Neurons, Quantum Theory
- **URL**: https://pubmed.ncbi.nlm.nih.gov/33232193/
- **Captured**: 2026-05-09T09 … 
- **The brain is both neurocomputer and quantum computer.**
  - `pubmed/PMID-21635328-the-brain-is-both-neurocomputer-and-quantum-computer/info.md`
  -  … <<Hameroff>> "<<Orch>> OR" quantum computational model of <<consciousness>>, arguing instead for neurocomputation as an explanation for mental phenomena. Here I clarify and defend <<Orch>> … 
- **Surface code model for Fibonacci helical pathways of the Orch OR microtubule.**
  - `pubmed/PMID-39978568-surface-code-model-for-fibonacci-helical-pathways-of-the-orc/info.md`
  -  … <<Hameroff>>, the theory proposed that the phenomenon of OR could also manifest in <<microtubules>> within neurons. This would, serve as the trigger for <<consciousness>> … 
- **Consciousness in the universe: a review of the 'Orch OR' theory.**
  - `pubmed/PMID-24070914-consciousness-in-the-universe-a-review-of-the-orch-or-theory/info.md`
  -  … Stuart <<Hameroff>>, Roger Penrose
- **MeSH**: Animals, Brain, <<Consciousness>>, Humans, Neurons, Quantum Theory
- **URL**: https://pubmed.ncbi.nlm.nih.gov/24070914/
- **Captured**: 2026-05-09T09 … 
- **Consciousness, the brain, and spacetime geometry.**
  - `pubmed/PMID-11349432-consciousness-the-brain-and-spacetime-geometry/info.md`
  -  … S <<Hameroff>>
- **MeSH**: Brain, <<Consciousness>>, Humans, <<Microtubules>>, Models, Neurological, Quantum Theory
- **URL**: https://pubmed.ncbi.nlm.nih.gov/11349432/
- **Captured**: 2026-05-10T10:59 … 
- **A quantum microtubule substrate of consciousness is experimentally supported and solves the binding and epiphenomenalism problems.**
  - `pubmed/PMID-40342554-a-quantum-microtubule-substrate-of-consciousness-is-experime/info.md`
  -  … of <<consciousness>> is a collective quantum state of <<microtubules>> and is specifically predicted by the Orchestrated Objective Reduction theory of Penrose and <<Hameroff>>. I … 
- **Conduction pathways in microtubules, biological quantum computation, and consciousness.**
  - `pubmed/PMID-11755497-conduction-pathways-in-microtubules-biological-quantum-compu/info.md`
  -  … the <<microtubule>> lattice structure, which lend themselves to topological quantum effects resistant to decoherence. The Penrose-<<Hameroff>> '<<Orch>> OR' model of <<consciousness>> is reviewed … 
- **Quantum walks in brain microtubules--a biomolecular basis for quantum cognition?**
  - `pubmed/PMID-24259348-quantum-walks-in-brain-microtubules-a-biomolecular-basis-for/info.md`
  -  … a Feynman chessboard are conceptually identical to 'topological qubits' in brain neuronal <<microtubules>>, as described in the Penrose-<<Hameroff>> '<<Orch>> OR' theory of <<consciousness>>.

- **Consciousness, the brain, and spacetime geometry: an addendum. Some new developments on the Orch OR model for consciousness.**
  - `pubmed/PMID-11349419-consciousness-the-brain-and-spacetime-geometry-an-addendum-s/info.md`
  -  … Some new developments on the <<Orch>> OR model for <<consciousness>>.

- **PMID**: 11349419
- **DOI**: 
- **PMCID**:  
- **Journal**: Annals of the New York Academy of Sciences  ·  **Year … 
- **A quantum approach to visual consciousness.**
  - `pubmed/PMID-11684479-a-quantum-approach-to-visual-consciousness/info.md`
  -  … visual <<consciousness>> that can accommodate brain-wide quantum computation according to the Penrose-<<Hameroff>> '<<Orch>> OR' model. In this view, visual <<consciousness>> occurs as … 
- **Quantum effects in the understanding of consciousness.**
  - `pubmed/PMID-25012711-quantum-effects-in-the-understanding-of-consciousness/info.md`
  -  … Stuart R <<Hameroff>>, Travis J A Craddock, Jack A Tuszynski
- **MeSH**: Animals, Brain, <<Consciousness>>, Humans, <<Microtubules>>, Models, Neurological, Neurons, Quantum Theory
- **URL**: https://pubmed … 
- **`Funda-Mentality': is the conscious mind subtly linked to a basic level of the universe?**
  - `pubmed/PMID-21227102-funda-mentality-is-the-conscious-mind-subtly-linked-to-a-ba/info.md`
  -  … In the Penrose-<<Hameroff>> model of `orchestrated objective reduction' (<<Orch>> OR), OR quantum computation occurs in cytoskeletal <<microtubules>> within the brain's neurons. The … 
- **Integration of intracellular signaling: Biological analogues of wires, processors and memories organized by a centrosome 3D reference system.**
  - `pubmed/PMID-30142359-integration-of-intracellular-signaling-biological-analogues/info.md`
  -  … Theoretical analysis presented here refers to (i) the Penrose-<<Hameroff>> theory of <<consciousness>> (Orchestrated Objective Reduction; <<Orch>> OR), (ii) the description of the centrosome … 

## intake (2)

- **Stuart Hameroff**
  - `_intake/canon-profiles/hameroff-stuart.md`
  - # Stuart <<Hameroff>>

**Slug**: `<<hameroff>>-stuart`  ·  **FTS query**: `<<Hameroff>> AND (<<microtubule>> OR <<consciousness>> OR <<Orch>>)`  ·  **Hits**: 30


## pubmed (13)

- **How quantum brain biology can rescue … 
- **Roger Penrose**
  - `_intake/canon-profiles/penrose-roger.md`
  -  … The <<Penrose>>-<<Hameroff>> '<<Orch>> OR' model of <<<<consciousness>>>> is reviewed … 
- **Quantum computation in brain <<microtubules>>: decoherence and biological feasibility.**
  - `pubmed/PMID-12188753-quantum-computation … 