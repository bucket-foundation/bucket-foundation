# Alan Turing

**Slug**: `turing-alan`  ·  **FTS query**: `Turing AND (computable OR machine OR morphogenesis)`  ·  **Hits**: 25


## archive (8)

- **Turing's vision : the birth of computer science**
  - `archive/turingsvisionbir0000bern/info.md`
  -  … To explain <<Turing>>\'s ideas, Bernhardt examines three well-known decision problems to explore the concept of undecidability; investigates theoretical <<computing>> <<machines>>, including <<Turing>> … 
- **Biokinetic Impacts on Structure and Imaging of the Lung the Concept of Biologically Closed Electric Circuits - Nordenstrom AJR 1985**
  - `archive/biokinetic-impacts-on-structure-and-imaging-of-the-lung-the-concept-of-biologica/Biokinetic impacts on structure and imaging of the lung the concept of biologically closed electric circuits - Nordenstrom AJR 1985_djvu.txt`
  -  … Furthermore, corona radiata refers to radiating struc- 
<<tures>>, which represent only one of several radiographically 
observable corona structures. All of the corona structures, 
however … 
- **Bioenergetics**
  - `archive/bioenergetics00szen/bioenergetics00szen_djvu.txt`
  -  … how  does  energy  drive  life?  How  does  it  move 
the  living  <<machine>>?  This  is  one  of  the  most  basic  problems  of 
biology  and,  at … 
- **Notes on the Paranormal**
  - `archive/ExtrasensoryPerceptionResearchFinding/Extrasensory_Perception_Research_Finding_djvu.txt`
  -  … Subjects each completed 
two runs on an electronic device similar to Schmidt’s <<machine>>. After 
these runs were completed, the <<computer>> generated a random … 
- **Newton's Principia : the mathematical principles of natural philosophy**
  - `archive/newtonspmathema00newtrich/newtonspmathema00newtrich_djvu.txt`
  -  … And  the  like  account  is  to  be  given  of  all  <<machines>>. 

The  power  and  use  of  <<machines>>  consist  only  in  this,  that  by  diminishing … 
- **The principles of chemistry**
  - `archive/principlesofchem01menduoft/principlesofchem01menduoft_djvu.txt`
  -  … 2  A  series  of  electric  sparks  may  be  obtained  by  an  ordinary  electrical  <<machine>>, 
the  electrophorous  <<machines>>  of  Holtz  and  Teploff,  &c.,  Leyden  jars … 
- **A treatise on electricity and magnetism**
  - `archive/electricandmagne01maxwrich/electricandmagne01maxwrich_djvu.txt`
  -  … Principle  of  Varley's  and  Thomson's  electrical  <<machines>>  ..       ..  256 

211.  Thomson's  water-dropping  <<machine>>          ..259 

212.  Holtz's  electrical  <<machine>>          260 

213 … 
- **LogicalCross**
  - `archive/logicalcross/essay_djvu.txt`
  -  … Then the residue at a — 1 <<computes>> to —1.64533 and 
the residue at B — 1 <<computes>> to 4.54930. The harmonic expression <<computes>> … 

## youtube (13)

- **Dr Jack Kruse _Anti-aging scientists wrong! Truth about  mTOR _Inflammation _ Mitochondria & POMC!**
  - `yt/cy8cByk8H00-dr-jack-kruse-anti-aging-scientists-wrong-truth-about-mtor-i/transcript.txt`
  -  … to go to <<Turing>> I want
you to go back and read turns paper oh
on night in 1951 about <<morphogenesis>>
right go back … 
- **Sir Roger Penrose & Stuart Hameroff: Collapsing a theory of quantum consciousness? Part 2 (248)**
  - `yt/OoDi856wLPM-sir-roger-penrose-stuart-hameroff-collapsing-a-theory-of-qua/transcript.txt`
  -  … I mean, you can maybe have a <<machine>>
which
I mean, they're very far from what's
called passing the <<Turing>> test even … 
- **Dr. Jack Kruse's MINDBLOWING Answer to "Who is Satoshi Nakamoto?"**
  - `yt/sIpXCMINceE-dr-jack-kruse-s-mindblowing-answer-to-who-is-satoshi-nakamot/transcript.txt`
  -  … in this whole
time outside the military they make
univac <<computers>> univac <<computers>> that
they wind up selling back to the US
military do … 
- **Joe Rogan Experience #1216 - Sir Roger Penrose**
  - `yt/GEw0ePZUMHA-joe-rogan-experience-1216-sir-roger-penrose/transcript.txt`
  -  … s
theorem and <<Turing>> <<machines>> <<Turing>>
<<machines>> being the mathematical notion
upon which modern <<computers>> are base for
all <<computers>> basically and the thing … 
- **"These People Must Pay" Bitcoin, Trump, COVID, and Zionism with Dr. Jack Kruse Ep. 2 (WiM545)**
  - `yt/yALOChgRrHQ-these-people-must-pay-bitcoin-trump-covid-and-zionism-with-d/transcript.txt`
  -  … <<Turing>> and that
information gets sent through the Enigma
<<machine>> you and I both learned when we
were young men that Allan <<Turing>> was … 
- **Uncomfortable Truths with Dr Jack Kruse**
  - `yt/Zka4Lnire1k-uncomfortable-truths-with-dr-jack-kruse/transcript.txt`
  -  … concerned with the 20th
century was Alan <<Turing>> and that paper
that he wrote on <<morphogenesis>> too that
should be a required reading for … 
- **Sir Roger Penrose & Dr. Stuart Hameroff: CONSCIOUSNESS AND THE PHYSICS OF THE BRAIN**
  - `yt/xGbgDf4HCHU-sir-roger-penrose-dr-stuart-hameroff-consciousness-and-the-p/transcript.txt`
  -  … we
this is just to illustrate basically
what a <<computer>> is like according to
<<Turing>> or vertical cartoon which is a
bit absurd it … 
- **Roger Penrose on Mind & Consciousness | Closer To Truth Chats**
  - `yt/vC4HNcqTQXk-roger-penrose-on-mind-consciousness-closer-to-truth-chats/transcript.txt`
  -  … So, why non-<<computational>>
and why the known laws of physics cannot
explain consciousness?
Well, with the why non-<<computational>>, my
question is the … 
- **Dr Jack Kruse On What Big Pharma & Big Food DON’T Want You to Know, Light & Magnetism Health Impact**
  - `yt/tha7IPtUTAU-dr-jack-kruse-on-what-big-pharma-big-food-don-t-want-you-to-/transcript.txt`
  -  … called Burrows which is a a
simple <<machine>> company that makes you
know calculators sewing <<machines>> things
like that nobody can figure out what … 
- **The War on Sunlight Is Real (And It’s Not an Accident) | Dr. Jack Kruse**
  - `yt/qa3FaUXZFSc-the-war-on-sunlight-is-real-and-it-s-not-an-accident-dr-jack/transcript.txt`
  -  … from Alan <<Turing>> from the Enigma
<<machine>>. Why was that listed as part of
the story? Because they use cryptography
uh to control the … 
- **The Jack Kruse Series 001**
  - `yt/T3Ob-DMtR1M-the-jack-kruse-series-001/transcript.txt`
  -  … can
check
with a mega <<machine>> mega <<machine>> being a
magnetoencephalogram
yes that's the same kind of <<machine>> but
we can now prove … 
- **Neurosurgeon talks about genetics and longevity: Dr jack kruse exclusive interview**
  - `yt/sf9VNlTiq5s-neurosurgeon-talks-about-genetics-and-longevity-dr-jack-krus/transcript.txt`
  -  … The other way I like to
describe it to give you another analogy
is use <<computers>>. Um, everybody knows
you have a hardware <<computer>> … 
- **Medisun Podcast #13 Dr. Jack Kruse: The modern medical system is broken and the fight for freedom**
  - `yt/CgKCJY182r4-medisun-podcast-13-dr-jack-kruse-the-modern-medical-system-i/transcript.txt`
  -  … They're
using, you know, programs and and
<<computers>>. And with programs and
<<computers>> now with AI, uh it's very easy
to give … 

## intake (4)

- **REALITY 11:  OPTICS IN A NON LINEAR WORLD:  WHEN UV LIGHT CHANGED EARTH**
  - `_intake/kruse-blog-corpus/articles/reality-11-optics-non-linear-world-uv-light-changed-earth.md`
  -  … The <<Turing>> <<machine>> was showed us <<computation>> is a function of how atoms and their parts work.  <<Turing>> described a perpetual motion <<machine>>.  Science … 
- **Bucket Foundation — Research Mapping Queue**
  - `_intake/RESEARCH-MAPPING-QUEUE.md`
  -  … Public access via Bell Labs archive. |
| `<<turing>>-1936-<<computable>>` | <<Turing>> — "On <<Computable>> Numbers..." (1936) | Public domain. |
| `chaitin-omega-papers` | Chaitin — algorithmic information theory papers … 
- **Kruse References Mined**
  - `_intake/kruse-references-mined/REFERENCES.md`
  -  … 1921 — 1
- Affair 1965 — 1
- China 1971 — 1
- Hoff 2005 — 1
- <<Turing>> 1951 — 1
- States 2024 — 1
- Vermont 2019 — 1
- Minutes 1977 — 1
- Ultra … 
- **Brain Gut 2: Viral Marketing**
  - `_intake/kruse-blog-corpus/articles/brain-gut-2-viral-marketing.md`
  -  … There is a very interesting book recently published–<<Turing>>’s Cathedral” by George Dyson–that tells the story of how nuclear weapons and <<computers>> … 