...
> A manifesto. Read this if you want to know why this project exists. Read `PROTOCOL.md` if you want to know how it works.
...
3. **Patronage routed value back to the makers.** Imperfectly and unevenly, but for the first time at scale: a Medici could pay a Galileo, a Fugger could pay a Dürer, a printer could pay a translator, and the work survived the lifetime of the patron.
...
2. **A payment rail that does not punish them for citing well.** The current incentives push toward citing the cheapest paywalled article over the one with the strongest foundations. Bucket inverts this: cite the foundation, pay the foundation's author, move on.
...
- **Neutral.** bucket has a thesis, and the thesis is stated here. We are not pretending to be a neutral commons. We are pretending to be a *commons with a known direction*.
...
- **The frontier models are good enough.** Not perfect, good enough to read primary literature, derive missing steps, and refuse to bullshit when refused well. The 2026 model class is the first one this is true of.
...
1. **Foundations only in the canon.** When in doubt, leave it out. The canon is small on purpose. Outcomes are downstream applications. Curate like a librarian.
...
3. **The Foundation is a temporary coordinator.** If, three years from now, three other buckets exist that hold equal or stronger canons under the same protocol, bucket.foundation has succeeded. If it is still the only or the largest bucket, it has failed. The goal is to *spawn the network*.
