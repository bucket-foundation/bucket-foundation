 ...
 * OpenAlex graph and surfaces the key related works + which are most central
...
export async function POST(req: NextRequest) {
  let body: { paper?: string; limit?: number };
  try {
    body = await req.json();
  } catch {
  ...
  if (paper.length < 4) {
    return json(
      { error: { code: "bad_request", message: "enter a DOI, OpenAlex ID, or paper title" } },
      400,
    );
  }
  ...
  let resp: Response;
  try {
    resp = await gatewayFetch("/v1/citationgraph/submit", {
      method: "POST",
      headers: { "content-type": "application/json" },
      body: JSON.stringify({ paper, limit: body.limit ?? 15 }),
    });
  } catch {
