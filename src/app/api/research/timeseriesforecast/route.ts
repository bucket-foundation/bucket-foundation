...
export async function POST(req: NextRequest) {
  ...
  const demo =
    body.demo === true ||
    (typeof values === "string" && values.trim().toLowerCase() === "demo");
  if (!demo && !(Array.isArray(values) && values.length >= 4)) {
    return json(
      { error: { code: "bad_request", message: "values must be a numeric array (length >= 4), or use demo" } },
      400,
    );
  }
  ...
  let resp: Response;
  try {
    resp = await gatewayFetch("/v1/timeseriesforecast/submit", {
      method: "POST",
