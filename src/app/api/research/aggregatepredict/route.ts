...
export async function POST(req: NextRequest) {
  ...
  const isDemo = typeof sequence === "string" && sequence.trim().toLowerCase() === "demo";
  const ok = isDemo || (typeof sequence === "string" && sequence.replace(/[^A-Za-z]/g, "").length >= 7);
  if (!ok) {
    return json(
      { error: { code: "bad_request", message: 'enter a protein sequence (>= 7 aa) or "demo"' } },
      400,
    );
  }
  ...
  let resp: Response;
  try {
    resp = await gatewayFetch("/v1/aggregatepredict/submit", {
      method: "POST",
