 ...
 * scoring (GC sweet-spot, homopolymer / Pol-III penalties, position rules),
...
export async function POST(req: NextRequest) {
  let body: { sequence?: string; pam?: string; guide_len?: number; limit?: number };
  try {
    body = await req.json();
  } catch {
  ...
  let resp: Response;
  try {
    resp = await gatewayFetch("/v1/grnaoptimizer/submit", {
      method: "POST",
