...
const TOOL = "materialsfeaturizer";
const GATEWAY_URL =
...
export async function POST(req: NextRequest) {
  ...
  const demo = body.demo === true ||
    (typeof body.formula === "string" && body.formula.trim().toLowerCase() === "demo");
  if (!demo) {
    const formula = typeof body.formula === "string" ? body.formula.trim() : "";
    if (!formula) {
      return json(
        { error: { code: "bad_request", message: 'enter a chemical formula (e.g. "Fe2O3"), or use demo' } },
        400,
      );
    }
  }
  ...
  let resp: Response;
  try {
    resp = await gatewayFetch("/v1/materialsfeaturizer/submit", {
      method: "POST",
