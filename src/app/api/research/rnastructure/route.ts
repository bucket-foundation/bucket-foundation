/**
 * bucket.foundation, /api/research/rnastructure
 * ----------------------------------------------
 * Same-origin proxy for the RNAStructure DNA/RNA tool. RNAStructure predicts
 * RNA secondary structure via ViennaRNA: MFE dot-bracket structure, MFE energy,
 * base-pair probabilities (partition function), and a readable summary. Backend
 * logic is REAL (services/research-tools/tools_dnarna.py:run_rna_structure).
 *
 * POST /api/research/rnastructure → gateway POST /v1/rnastructure/submit
 * body { sequence } → { job_id, status, mode, price, [result] }
 * GET /api/research/rnastructure?job=<id>[&result=1] → status / result
 *
...
export async function POST(req: NextRequest) {
  ...
  let resp: Response;
  try {
    resp = await gatewayFetch("/v1/rnastructure/submit", {
      method: "POST",
