...
export async function POST(req: NextRequest) {
  ...
  const demo = body.demo === true ||
    (typeof rec === "string" && rec.trim().toLowerCase() === "demo");
  if (!demo) {
    const isObject = typeof rec === "object" && rec !== null && !Array.isArray(rec);
    const isJsonStr = typeof rec === "string" && rec.trim().length >= 2;
    if (!isObject && !isJsonStr) {
      return json(
        { error: { code: "bad_request", message: 'provide an experiment record (JSON object or fields), or "demo"' } },
        400,
      );
    }
  }
  ...
  let resp: Response;
  try {
    resp = await gatewayFetch("/v1/mlreprocard/submit", {
      method: "POST",
