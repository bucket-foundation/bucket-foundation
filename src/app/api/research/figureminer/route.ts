...
export async function POST(req: NextRequest) {
  ...
  const ok = isDemo || (typeof text === "string" && text.trim().length >= 20);
  if (!ok) {
    return json(
      { error: { code: "bad_request", message: 'paste paper text (>= 20 chars) or use "demo"' } },
      400,
    );
  }
  ...
  let resp: Response;
  try {
    resp = await gatewayFetch("/v1/figureminer/submit", {
      method: "POST",
