...
export async function POST(req: NextRequest) {
  ...
  const ok = isDemo || (typeof sequence === "string" && sequence.replace(/[^A-Za-z]/g, "").length >= 20);
  if (!ok) {
    return json(
      { error: { code: "bad_request", message: 'enter a DNA sequence (>= 20 nt) or "demo"' } },
      400,
    );
  }
  ...
  let resp: Response;
  try {
    resp = await gatewayFetch("/v1/chromatinaccess/submit", {
      method: "POST",
