...
export async function POST(req: NextRequest) {
  let body: { methods?: string; title?: string };
  try {
    body = await req.json();
  } catch {
  ...
  if (methods.length < 15) {
    return json(
      { error: { code: "bad_request", message: "paste a methods/SOP description (>= 15 chars)" } },
      400,
    );
  }
  ...
  let resp: Response;
  try {
    resp = await gatewayFetch("/v1/protocolgpt/submit", {
      method: "POST",
      headers: { "content-type": "application/json" },
      body: JSON.stringify({ methods, title: (body.title ?? "").trim() || undefined }),
    });
  } catch {
