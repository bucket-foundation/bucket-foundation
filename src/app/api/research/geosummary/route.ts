...
export async function POST(req: NextRequest) {
  ...
  const demo = body.demo === true ||
    (typeof body.values === "string" && body.values.trim().toLowerCase() === "demo");
  if (!demo && !(Array.isArray(body.values) && body.values.length > 0)) {
    return json(
      { error: { code: "bad_request", message: "values must be a non-empty numeric array, or use demo" } },
      400,
    );
  }
  ...
  let resp: Response;
  try {
    resp = await gatewayFetch("/v1/geosummary/submit", {
      method: "POST",
