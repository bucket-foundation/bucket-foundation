...
export async function POST(req: NextRequest) {
  let body: { reference?: unknown; deformed?: number[][]; window?: number; step?: number; search?: number };
  try {
    body = await req.json();
  } catch {
  ...
  const isDemo = typeof reference === "string" && reference.trim().toLowerCase() === "demo";
  const isArray = Array.isArray(reference) && reference.length > 0 && Array.isArray(body.deformed) && body.deformed.length > 0;
  if (!isDemo && !isArray) {
    return json(
      { error: { code: "bad_request", message: 'provide reference + deformed images, or reference="demo"' } },
      400,
    );
  }
  ...
  let resp: Response;
  try {
    resp = await gatewayFetch("/v1/tractionforceml/submit", {
      method: "POST",
