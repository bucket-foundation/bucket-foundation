...
export async function POST(req: NextRequest) {
  ...
  const demo = seqA.toLowerCase() === "demo";
  if (!demo) {
    const seqB = typeof body.seq_b === "string" ? body.seq_b.trim() : "";
    if (!seqA || !seqB) {
      return json(
        { error: { code: "bad_request", message: 'provide two sequences (seq_a, seq_b), or use "demo"' } },
        400,
      );
    }
  }
  ...
  let resp: Response;
  try {
    resp = await gatewayFetch("/v1/seqalign/submit", {
      method: "POST",
