...
export async function POST(req: NextRequest) {
  let body: { toxin?: string; limit?: number };
  try {
    body = await req.json();
  } catch {
  ...
  if (toxin.length < 3) {
    return json(
      { error: { code: "bad_request", message: "enter a toxin/peptide name or sequence (>= 3 chars)" } },
      400,
    );
  }
  ...
  let resp: Response;
  try {
    resp = await gatewayFetch("/v1/toxinchannelfinder/submit", {
      method: "POST",
      headers: { "content-type": "application/json" },
      body: JSON.stringify({ toxin, limit: body.limit ?? 10 }),
    });
  } catch {
