 ...
 * deterministic rubric (Wilkinson 2016's 15 sub-principles + funder DMSP
...
export async function POST(req: NextRequest) {
  ...
  const isDemo = typeof record === "string" && record.trim().toLowerCase() === "demo";
  ...
  if (!isDemo && !isObject && !isJsonStr) {
    return json(
      {
        error: {
          code: "bad_request",
          message: 'provide a metadata record (JSON object or fields), or "demo"',
        },
      },
      400,
    );
  ...
  let resp: Response;
  try {
    resp = await gatewayFetch("/v1/faircheck/submit", {
      method: "POST",
