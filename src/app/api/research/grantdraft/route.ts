 ...
 * OpenAlex literature fallback) and drafts specific-aims bullets grounded in
...
export async function POST(req: NextRequest) {
  let body: { topic?: string; limit?: number };
  try {
    body = await req.json();
  } catch {
  ...
  let resp: Response;
  try {
    resp = await gatewayFetch("/v1/grantdraft/submit", {
      method: "POST",
      headers: { "content-type": "application/json" },
      body: JSON.stringify({ topic, limit: body.limit ?? 8 }),
    });
  } catch {
