 ...
 * citation velocity, and returns a citeable digest. Backend logic is REAL
...
export async function POST(req: NextRequest) {
  let body: { interests?: string; since_days?: number; limit?: number };
  try {
    body = await req.json();
  } catch {
  ...
  let resp: Response;
  try {
    resp = await gatewayFetch("/v1/paperradar/submit", {
      method: "POST",
      headers: { "content-type": "application/json" },
      body: JSON.stringify({
        interests,
        since_days: body.since_days ?? 540,
        limit: body.limit ?? 12,
      }),
    });
  } catch {
