...
export async function POST(req: NextRequest) {
  let body: { claim?: string; limit?: number };
  try {
    body = await req.json();
  } catch {
  ...
  if (claim.length < 8) {
    return json(
      { error: { code: "bad_request", message: "state a quantum-biology claim (>= 8 chars)" } },
      400,
    );
  }
  ...
  let resp: Response;
  try {
    resp = await gatewayFetch("/v1/quantumbiorag/submit", {
      method: "POST",
      headers: { "content-type": "application/json" },
      body: JSON.stringify({ claim, limit: body.limit ?? 15 }),
    });
  } catch {
