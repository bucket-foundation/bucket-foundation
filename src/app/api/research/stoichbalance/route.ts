 ...
 * compute the limiting reagent + theoretical yields. Backend is REAL
 * (tools_stoich.py).
 *
...
export async function POST(req: NextRequest) {
  ...
  if (!demo && eq.length < 3) {
    return json(
      { error: { code: "bad_request", message: 'enter a chemical equation (e.g. "H2 + O2 -> H2O"), or use demo' } },
      400,
    );
  }
  ...
  let resp: Response;
  try {
    resp = await gatewayFetch("/v1/stoichbalance/submit", {
      method: "POST",
