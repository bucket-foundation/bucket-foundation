...
export default function WaitlistForm({ wanted }: { wanted: string | null }) {
  ...
  async function submit(e: FormEvent) {
    ...
    try {
      ...
      if (!res.ok || !data.ok) {
        setError(
          data.error ?? "Your signup did not save. Try again in a minute.",
        );
        return;
      }
      setJoined(address);
  ...
  const status = (
    <p role="status" aria-live="polite" className="sr-only">
      {joined
        ? `You are on the list. We will email ${joined} the day Research OS opens.`
        : ""}
    </p>
  );
    ...
    return (
      <>
        ...
        <div
          ref={doneRef}
          tabIndex={-1}
          className="mt-8 border-l-2 border-[color:var(--gold)] pl-4 py-1 focus:outline-none"
        >
          <p className="text-[15px] text-[color:var(--basalt)]">
            You are on the list.
          </p>
          <p className="mt-2 text-[13px] text-[color:var(--basalt-2)]">
            We will email <span className="break-all">{joined}</span> the day
            Research OS opens.
          </p>
        </div>
      </>
    ...
    <>
      ...
      <form onSubmit={submit} className="mt-8 flex flex-col gap-3">
        ...
        <p className="mt-2 text-[12px] leading-relaxed text-[color:var(--basalt-3)]">
          One email when Research OS opens. Reply to it and we take you off the
          list. Under 13? Ask a parent or teacher to sign up for you.
        </p>
      </form>
    </>
  );
