...
### The long arc of knowledge *access*

Era by era, and the one pattern that has never broken.

_education-atlas landscape analysis. Generated by
[`analysis/landscape/build_historical.py`](https://github.com/bucket-foundation/education-atlas/blob/master/analysis/landscape/build_historical.py) → `results_historical.json`; figures by
`make_figures_historical.py` (`fig_literacy_longrun.png`, `fig_access_arc.png`).
The **time axis** of the knowledge-access gradient, pushed back
...
recorded history, how has access to knowledge moved, and for whom?**
...
and only the first is democratizing, is argued from a 24-year window.
If the claim is real, it should hold from 2000 to 2024 and from the
...
Knowledge bounded by memory and mortality.
...
## 3. The printing press: the first great democratization / the canonical case
...
and 1600**, so the press was a cultural event and a measurable economic
...
Still needed literacy, a press or a printer's contract, and a patron or
...
Intact. *(Cost-collapse and output figures are real economic-history
anchors; the consume-vs-produce reading is the analysis's interpretation.)*

---

## 4. Public libraries and mass schooling

Consume-access goes from elite to majority.
...
computed over rather than a true fall in the number of literate people, flagged in the
source and in `build_historical.py`.)

On the produce side, the nineteenth and twentieth centuries did something
two-edged. By making nearly everyone literate they *expanded the
...
## 5. The internet and Wikipedia, information abundance

The internet drove the marginal cost of a *copy*, the thing the press attacked
once and libraries attacked again, to zero. Once a text is online,
it can be read by one more person for no additional cost, anywhere, instantly.
Global internet penetration went from **0% in 1990 to about 68% in
...
This is consume-access at abundance: cheaper books and free, instant,
...
*publish a webpage*, and the web lowered the bar to broadcasting an
...
Unchanged. The pattern holds.

---

## 6. Open access and preprints

Consume-access reaches the frontier for the first time.
...
The first time the verdict is not yet written.

Every era above resolves cleanly into the same verdict: consume-access widened,
produce-access did not. AI is the first entry where an analyst cannot yet
...
new contribution to the L3→L4 bridge that doc `02` finds unbuilt. But
explaining the frontier is still *reading* it. The institutional machinery that
gates production, funding (NIH R01 success ~13%), a research position,
...
*move* knowledge from store to reader, it can *operate* on it.
...
non-institutional person *do* research rather than only consume it,
which is precisely the L3→L4→L5 white space the master synthesis names as the
highest-impact region. The earliest gestures in this direction (research-agent
...
The position is that **both readings are defensible on current
evidence**, and the corpus refuses to resolve what the data cannot. The
deskilling studies are too thin to prove AI is ** a consumer; the
...
Pattern: faster reading, better explanations, and more papers summarized are
consume-access wins the arc has delivered before. The break would be a
...
*open*. *(Wholly interpretive. The corpus rates AI conditional.)*
...
| AI consume-vs-produce verdict | **OPEN** | rated conditional (consistent with `02`, `foundations/04`) |
...
denominator artifact of decolonization rather than a true fall. (4) Every *verdict* in
the ledger is an interpretation of the data; the underlying series are
real; the consume-vs-produce reading is the analysis's. (5) The AI row is
undecided and is presented as a two-sided open question rather than a
prediction.
...
> *read*, leaving untouched who could *originate*. Public libraries and mass schooling took
...
Files: [`analysis/landscape/build_historical.py`](https://github.com/bucket-foundation/education-atlas/blob/master/analysis/landscape/build_historical.py) (analysis + era ledger),
...
[`analysis/landscape/build_historical.py`](https://github.com/bucket-foundation/education-atlas/blob/master/analysis/landscape/build_historical.py) + `make_figures_historical.py`.*
