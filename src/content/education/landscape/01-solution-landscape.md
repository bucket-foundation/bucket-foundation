# The Education Solution Landscape

Mapped on the Age × Knowledge-depth grid.

### Bucket Foundation landscape brief 01
...
> asks where the grid is **crowded** and where it is **empty**. The
> structured catalog is `data/landscape/solutions.csv` (12 columns, one row per
> player, each with a source URL). The map and the reading of it follow.
...
, including where the data complicates the story.
...
## 1. The grid: column by column

### 1.1 Early-childhood, depth L0
...
Undergraduate and professional knowledge, stopping short of the research frontier.
...
column is the **credential** alone.

### 1.6 The frontier column: the heart of the test
...
made concrete: the frontier exists but is priced for institutions rather than people.
...
is the acknowledgment that the access problem is so acute that the
...
| **Perplexity Deep Research** [68] | dozens of searches → cited report | L2, L5 general, below scholarly rigor |
...
| **FutureHouse / Edison Scientific** [71] | autonomous science agents: **PaperQA2** beat PhDs on lit-review; **Robin** generated a hypothesis (ripasudil for dry-AMD); **Kosmos** "AI Scientist" | **L5, the current frontier of AI *doing* science**, though wet-lab validation still needs humans |
...
| **L1 K-12** | thin | **crowded** | - | thin | medium |
...
clear majority of players and all of the consumer-edtech capital.**
...
## 3. How this map lines up with the atlas's problems

Matching solution density to the problem inventory from `EDUCATION_PROBLEMS.md`
and the deep-dives:

| Problem (from the atlas) | Solution density | read |
|---|---|---|
...
| **Financing** (3.6% of GDP, below the 4% floor) | None (not a product space) | A policy lever with no market; absent from this catalog. |
...
| **Learning-to-learn / philosophy / agency** (`deep/03`) | **Near-empty** | No category leader targets metacognition, epistemology, or self-direction. A product gap *inside* the crowded K-12 column. |
...
## 4. The white-space finding: testing the top-right thesis
...
every L0, L3 cell, the L5 cell has *two* serious occupants. No
product bridges the crowded L3 upskilling column up to the L4 frontier. And the
frontier tools that exist are built for credentialed academics rather than for the
...
**The qualification, be about who is already there.** The top-right is
...
- **AI-assisted *doing* has two credible, fast-moving incumbents.** **Elicit**
 ...
 empty, and is being entered by well-resourced players.**
...
The verdict: **the top-right is the least-served region (thesis
...
And *how knowledge production is paid for*, leaving agent-building to them.
