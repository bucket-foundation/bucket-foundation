...
Quantifying Knowledge Access on the Age × Knowledge-Depth Grid.

### A data-science measurement of *who can reach how deep into knowledge*

Where the access cliff is.

_education-atlas `docs/landscape` analysis. Generated by [`analysis/landscape/build_access.py`](https://github.com/bucket-foundation/education-atlas/blob/master/analysis/landscape/build_access.py)
...
Education debates measure *enrollment*, are children in a classroom?
This analysis asks a harder question: **for a person of a given age, how deep
into a body of knowledge can they go?** And the inverse: **what fraction
...
## 1. The constructed scale

The **two axes are not measured by any authoritative body**, they are an
analytical frame this project defines in [`analysis/landscape/scale.py`](https://github.com/bucket-foundation/education-atlas/blob/master/analysis/landscape/scale.py). What is
...
### Finding 1: The access cliff runs **down the depth axis**
...
is the *vertical* distance between rungs. **Depth is the binding
constraint.**

### Finding 2: The equity gradient

Income decides depth.
...
### Finding 3: Coverage white-space

The top-right is empty.
...
### Finding 4: Consume vs. Create

~0.1% of humanity ever produces new knowledge.
...
~75× gap** in who even reaches L4. L5 (publishing) is smaller again.
...
The depth ceiling of a *typical* person.

The median depth a typical member of each tier reaches (highest rung
with ≥50% access):
...
## 5. The access channel

Internet by life-stage.
...
**Known limitations.** (1) The depth ladder is an analytical construct, loosely aligned with
ISCED levels. (2) World-average rows are unweighted tier means, not
...
R&D series); they are conservative relative to-cited figures (world
...
> **Education's binding constraint is depth, and it is bought by income.**
...
Files: [`analysis/landscape/scale.py`](https://github.com/bucket-foundation/education-atlas/blob/master/analysis/landscape/scale.py) (constructed axes),
