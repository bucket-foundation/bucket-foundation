...
[`analysis/landscape/build_geographic.py`](https://github.com/bucket-foundation/education-atlas/blob/master/analysis/landscape/build_geographic.py) → `results_geographic.json`; figures by
...
is not a one-country spike. Weights are **documented and fixed in advance of any target**.
...
the shut-out countries don't appear on the chart at all, because they
have no number.
...
the world's flagship R&D indicator has no value for it. Data sparsity is
...
The shut-out countries can't be ranked, because they don't have a number to
rank.
...
choice made on judgment; the *ranking* holds up under reasonable reweighting,
the absolute scores less so. (4) Women-in-research and rural/wealth numbers are
cited regional/global anchors, taken at that level of aggregation.
...
> gender cut shows the gap opens **after the degree**: women now out-enroll
...
Files: [`analysis/landscape/build_geographic.py`](https://github.com/bucket-foundation/education-atlas/blob/master/analysis/landscape/build_geographic.py) (analysis),
