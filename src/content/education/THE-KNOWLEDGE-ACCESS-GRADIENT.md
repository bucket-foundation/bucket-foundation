...
### Who can reach how deep into human knowledge

At what age, at what cost, and what that means for reforming education.
...
@techreport{dichio2026knowledgeaccess,
  title        = {The Knowledge-Access Gradient: who can reach how deep into
                  human knowledge, at what age, at what cost, and what that
...
Citation numbers `[n]` in the text point to the numbered [References](#references)
at the end of this document. Version 1.0 shipped 2026-06-20; v1.1 (this release,
2026-06-25) added the historical, geographic, and modality dimensions. License:
CC-BY-4.0 (text and data) / MIT (code). Corresponding repository:
[`bucket-foundation/education-atlas`](https://github.com/bucket-foundation/education-atlas).
...
magnitude (book prices fell ~⅔ in fifty years, output rose ~10⁴ per century) [6][7] and lit
...
10-year-olds worldwide cannot read a simple text; 86.5% in Sub-Saharan Africa) [1], an
**access** crisis that has shrunk but moved up a level (51M primary-age and 61M
lower-secondary-age children out of school) [1], and a **financing** crisis underwriting
both (world education spend is 3.6% of GDP, below the agreed 4% floor, with 92 of ~200
countries beneath it) [1]. But the deepest inequity is not visible in the access statistics
...
researcher intensity has a Gini of **0.646** [2], and, the sharpest equity signal, women
...
reaches production (apprenticeship, the lab) cannot scale, and that empty
"scalable-and-production-reaching" cell **is** the consume-versus-produce gap. The causes
are structural: an industrial schooling model built to *sort*, a
...
highest-impact capacities (learning-to-learn, the body, the ceiling on the ablest). Yet
...
socialization are legitimate functions. So any reform is a value
...
And against folk theory (learning styles is null). The highest-impact region for the
...
here as **one defensible choice among the contested aims**, bounded: it does not
...
body of knowledge can they go, at what cost, against how many gates; and what
...
- **Y, Knowledge depth,** a six-rung ladder:
 ...
 - **L5** *producing* new knowledge, doing research
...
rest of the atlas shows the binding constraint is depth, bought by income, gated by
institutions, and then asks,, what a foundation can and cannot do about it.

### 1.1 The 5,000-year spine

Every knowledge technology widened reading, none widened producing.
...
54%, 2000-2024), and *still* produce-access did not move: the rent relocated to the
...
entire arc where an analyst cannot yet sign the verdict**, it might be the most
...
~0.14% ever moves. With the spine in place, the rest of the atlas is the cross-section
of the arc's final frame: where the cliff is *today*, what shape it has across seven
dimensions, why it is shaped that way, and what,, can be done about it.

---

## 2. The measurable state

Three crises, an access cliff, and a consume-versus-create gap.
...
`SE.LPV.PRIM` [1]). Nearly half of all children reach age 10 unable to do the one thing
...
in the atlas: a policy lever with a known fix.
...
so the cliff is the *vertical* distance between rungs. **Depth is the binding
constraint, and age is flat.** No income tier's *typical* person reaches graduate depth: the median
...
Free to read, no free path to produce.

The cost-to-reach-each-depth curve is **bimodal**. It does not rise monotonically with depth. Reaching the floor
(L0, L2) and *reading* the frontier (L4) both have a **$0 path**, public
...
*Nature*'s 2026 open-access article-processing charge [10]: **$12,850 to publish one
article.**
...
### 3.3 Temporal: reading is democratizing
...
answer: **access to *consume* the frontier is democratizing; access to *produce*
It is persistent.**
...
world-average normalized to the L0 cohort, a cross-sectional presence curve rather than a
...
First, the **coverage finding**, which is the most result in the layer because it is
...
### 4.1 The industrial model: a machine built to sort
...
### 4.3 What the systems lack

Learning-to-learn, the body, the ceiling.
...
contested, reasoning helps when embedded in rich domain knowledge rather than taught as a
standalone class.)
...
administrative rather than evidentiary. The equity-versus-excellence tension is real and held
open: for most of the world's children the binding problem is the floor, and a
...
## 5. What education is FOR: the caveat the data cannot settle
...
cast as villains have legitimate versions. **Sorting and credentialing** solve a
allocation problem (some seats are scarce and someone must fill them; a
transparent exam-based meritocracy is *fairer* than the heredity and patronage it
...
*right* aim, or which closing matters most, because "what for?" is a value choice rather than an
Empirical one. **Any reform is therefore a value choice and must be stated as one.** The
sections that follow name a high-impact region and a specific thesis, but as *one
...
And where the biggest gains are.
...
facilitated, and single-context., **metacognition is the rare case where the
...
image: the only channel that ** reaches production (it transfers the tacit knowledge
...
why the highest-impact move below pairs an AI comprehension bridge with an author-routed
...
The highest-impact region for the *specific* aim of opening **advanced** knowledge is
therefore the **L3→L4→L5 zone**, at the intersection of a won open-access asset, an unsolved
gate, and a what-works mechanism:

1. **The L3→L4 comprehension bridge (biggest gain).** Reading the frontier is largely
...
And it is bounded. Bucket is a foundation with no ministry-of-education authority. **It does
...
A synthesis that overclaims is worse than none. The boundaries of this evidence:
...
- **Several headline cells are estimates flagged as such.** L3 (graduate) is a real
 ...
 assumptions. World-average rows are unweighted tier means. Population weighting is not applied.
- **The corpus has a STEM and anglophone lean.** The research-atlas is STEM-built, so the
 ...
 the finding rather than a measured census), and cost anchors and many cited studies are
 US/OECD-centric, the *shape* generalizes, the exact numbers do not.
- **The AI evidence is early.** LLM-tutor RCTs are short-term, facilitated,
 and single-context (the strongest, Kestin et al. 2025, is two weeks, 194 students,
 middle-order skills, an *engineered* tutor; Tutor CoPilot is human-AI); the
 deskilling/cognitive-offloading studies are correlational or small-n with methodological
 critiques. The corpus rates AI conditional, and so does this synthesis, in
 both the historical (§1.1) and modality (§6.2a) framings, the AI verdict is presented as a
 two-sided open question rather than a prediction.
- **The continuity funnel is a cross-sectional presence curve over cohorts rather than tracked
 individuals**, and the gate inventory is a representative enumeration rather than an exhaustive one.
- **The historical arc's *verdicts* are interpretive.** The long-run series (literacy
 ...
 do not. The 1950 literacy dip is a decolonization denominator artifact rather than a true fall.
- **The geographic layer rests on a data anchor that fails worst where access is worst.** The
 ...
 are a defensible choice rather than a derived optimum (the *ranking* holds up under reweighting, the
 absolute scores less so); and women-in-research and rural/wealth figures are cited
 regional/global UNESCO anchors rather than per-country pulls.
- **The modality depth-ceilings are a constructed mapping.** Where each channel "tops out" on
 ...
 headcount) and are used as orders of magnitude rather than like-for-like comparisons. Real learners braid
...
peak. The base is a achievement, near-universal primary access, 87.7% adult
...
1. World Bank. *EdStats: Education Statistics.* `SE.LPV.PRIM` (learning poverty),
   `SE.PRM.UNER` / `SE.SEC.UNER.LO` (out-of-school children), `SE.XPD.TOTL.GD.ZS`
   (education spend, % GDP), `SE.TER.ENRR.FE` / `.MA` (tertiary enrollment by
   gender). https://databank.worldbank.org/source/education-statistics-%5e-all-indicators
2. UNESCO Institute for Statistics (UIS). *Researchers per million inhabitants*
   (`SP.POP.SCIE.RD.P6`, mirrored via World Bank), *Women in Science*, and the
   *GEM/WIDE* World Inequality Database on Education. http://uis.unesco.org/
...
5. OpenAlex, queried through Bucket Foundation's research-atlas corpus (the
   1,438,636-researcher / 320,879-active-publisher bottom-up count in §2.3).
   https://openalex.org and https://github.com/bucket-foundation/research-atlas
...
`python3 scripts/build_all.py` then `python3 scripts/findings.py`; the `analysis/landscape` analyses
