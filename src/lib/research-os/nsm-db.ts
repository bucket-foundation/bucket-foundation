...
import { assemble, MIN_CONFIDENCE, type NsmExponentRow, type NsmPrime, type NsmPrimeRow } from "./nsm";

const PRIME_COLUMNS = "id,label,category,english,ord,en_word,en_pos,sense,sense_match";
const EXPONENT_COLUMNS = "prime_id,lang,word,rank,roman,sense,sense_match,confidence,root_lang,root_form,root_gloss";

type Page<T> = Promise<{ data: T[] | null; error: { message: string } | null }>;

export async function loadNsm(opts: { lang?: string | null; includeUnconfirmed?: boolean } = {}): Promise<NsmPrime[]> {
  ...
  const exponents = await pagedRead<NsmExponentRow>((page) => {
    ...
    if (!opts.includeUnconfirmed) q = q.gte("confidence", MIN_CONFIDENCE);
    return q.order("prime_id").order("lang").order("word").range(page.from, page.to) as unknown as Page<NsmExponentRow>;
  });
  return assemble(primes, exponents, { includeUnconfirmed: opts.includeUnconfirmed });
}
