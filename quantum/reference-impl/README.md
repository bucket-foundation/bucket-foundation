...
| Error mitigation | readout calibration + zero-noise extrapolation → **77%** error reduction (0.189 → 0.044) |
...
1. **Amplitude-encode** a normalized vector $x$ into a quantum state $|\psi_x\rangle = \frac{1}{\lVert x\rVert}\sum_i x_i\,|i\rangle$ (a 1024-dim vector fits in 10 qubits).
2. The **overlap of two such states equals the cosine similarity** of the originals: $\langle\psi_u|\psi_v\rangle = \cos(u,v)$. That is the whole engine.
3. Measure that overlap two ways — the **swap test** (gives $|\cos|^2$) and the **Hadamard test** (gives signed $\cos$) — stack the pairwise values into a **quantum kernel matrix**, and feed it to a classifier.
...
python tests/test_mitigation.py     # mitigation primitives (readout cal, ZNE, folding)
...
python -m src.error_budget          # 5-condition readout + ZNE error budget
```
...
writeup/error-mitigation.md   mitigation deep-dive (readout cal, ZNE, destructive swap)
src/
  encode.py            amplitude encoding (classical vector -> quantum state)
  swap_test.py         swap test -> |cos|^2
  destructive_swap.py  ancilla-free low-depth swap variant
  ...
  noise_models.py      gate-depolarizing + readout noise model
  mitigation.py        readout-error calibration + zero-noise extrapolation
  error_budget.py      the 5-condition error budget + figures
  angle_sweep.py       the low-depth signed-range hardware experiment
tests/                 test_estimators.py (quantum == classical) + test_mitigation.py
results/               figures + json (shot / noise / error-budget / hardware)
```
