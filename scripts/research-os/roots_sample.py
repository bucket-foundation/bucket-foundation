...
COLUMNS = ["key", "table", "lang", "word", "roman", "sense", "root_lang", "root_form", "root_gloss", "confidence", "root_confidence"]
...
def shown_rows(db_url, hide_below=node_words.HIDE_BELOW):
    nw = fetch(db_url, (
        "select 'nw:' || id::text as key, 'node_words' as \"table\", lang, word, roman, coalesce(nullif(gloss, ''), sense) as sense, "
    ...
    nsm = fetch(db_url, (
        "select 'nsm:' || prime_id || '|' || lang || '|' || word as key, 'nsm_exponents' as \"table\", lang, word, roman, sense, "
