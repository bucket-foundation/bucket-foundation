...
export const ERROR_EXCEPTIONS: ErrorException[] = [
  ...
  { at: "src/lib/academy/credential/store.ts::revokeCredential::<unknown>::data::1", because: "not yet triaged, empty-guard: this read binds error and returns the same empty value either way, and nothing here has read the caller to say whether that is right" },
  ...
  { at: "src/lib/research-os/access-db.ts::loadViewerGroups::class_members::data::1", because: "not yet triaged, empty-guard: this read binds error and returns the same empty value either way, and nothing here has read the caller to say whether that is right" },
  { at: "src/lib/research-os/access-db.ts::loadRequestsForNode::access_requests::data::1", because: "not yet triaged, empty-guard: this read binds error and returns the same empty value either way, and nothing here has read the caller to say whether that is right" },
  { at: "src/lib/research-os/access-db.ts::loadRequestsByRequester::access_requests::data::1", because: "not yet triaged, empty-guard: this read binds error and returns the same empty value either way, and nothing here has read the caller to say whether that is right" },
  { at: "src/lib/research-os/access-db.ts::loadOwnedNodes::nodes::data::1", because: "not yet triaged, empty-guard: this read binds error and returns the same empty value either way, and nothing here has read the caller to say whether that is right" },
  { at: "src/lib/research-os/access-db.ts::loadNodeAccess::nodes::data::1", because: "not yet triaged, empty-guard: this read binds error and returns the same empty value either way, and nothing here has read the caller to say whether that is right" },
  { at: "src/lib/research-os/access-db.ts::loadGrants::node_grants::data::1", because: "not yet triaged, empty-guard: this read binds error and returns the same empty value either way, and nothing here has read the caller to say whether that is right" },
  ...
  { at: "src/lib/research-os/production-node.ts::createNodeFromProduction::nodes::related::1", because: "not yet triaged, empty-guard: this read binds error and returns the same empty value either way, and nothing here has read the caller to say whether that is right" },
  { at: "src/lib/research-os/access-db.ts::filterSubgraphForViewer::node_grants::data::1", because: "not yet triaged: a failed read here answers an empty collection, and nothing has checked what that reaches" },
  ...
  { at: "src/lib/research-os/db.ts::loadCurrentStage::learner_node_state::data::1", because: "a maybeSingle lookup whose miss and whose failure both mean carry on without it" },
  ...
  { at: "src/app/api/research-os/consent/route.ts::POST::class_members::member::1", because: "a maybeSingle lookup whose miss and whose failure both mean carry on without it" },
  ...
  { at: "src/app/api/research-os/probe/route.ts::POST::nodes::prereqNodes::1", because: "not yet triaged: a failed read here answers an empty collection, and nothing has checked what that reaches" },
  ...
  { at: "src/app/api/research-os/review/route.ts::GET::nodes::nodes::1", because: "not yet triaged: a failed read here answers an empty collection, and nothing has checked what that reaches" },
  ...
  { at: "src/app/api/research-os/workspace/route.ts::POST::nodes::prereqNodes::1", because: "not yet triaged: a failed read here answers an empty collection, and nothing has checked what that reaches" },
