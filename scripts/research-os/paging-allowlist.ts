...
export const PAGING_EXCEPTIONS: PagingException[] = [
  ...
  { at: "scripts/research-os/ingest/canon-all.ts:241", because: "graph.nodes.slug is not null unique, so one row per slug, and the loop hands in() at most 200 slugs a call" },
  { at: "scripts/research-os/ingest/lib/apply-drafts.ts:26", because: "graph.nodes.slug is not null unique, so one row per slug, and the loop hands in() at most 60 slugs a call" },
  ...
  { at: "src/lib/research-os/class-db.ts:115", because: "UNTRIAGED: reads assignments by class_id, and nothing here has checked the rows per value against the schema" },
  { at: "src/lib/research-os/class-db.ts:125", because: "nodes.id is the primary key, so one row per id and at most as many rows as the list is long" },
  { at: "src/lib/research-os/class-db.ts:126", because: "classes.id is the primary key, so one row per id and at most as many rows as the list is long" },
  { at: "src/lib/research-os/class-db.ts:127", because: "learner_node_state has primary key (learner_id, node_id) and learner_id is pinned with eq, so one row per node" },
  { at: "src/lib/research-os/class-db.ts:128", because: "UNTRIAGED: reads productions by target_node_id, and nothing here has checked the rows per value against the schema" },
  ...
  { at: "src/lib/research-os/consent.ts:250", because: "classes.id is the primary key, so one row per id and at most as many rows as the list is long" },
  ...
  { at: "src/app/api/research-os/probe/route.ts:133", because: "nodes.id is the primary key, so one row per id and at most as many rows as the list is long" },
  ...
  { at: "src/app/api/research-os/workspace/route.ts:558", because: "nodes.id is the primary key, so one row per id and at most as many rows as the list is long" },
];
