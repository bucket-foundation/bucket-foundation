...
if [[ -n "$PREV_SHA" ]]; then
  ...
  # dev and main with no previous sha. Vercel drops it after a canceled
  # deployment, so the branch's own first parent is the base: what the
  # branch held before this merge.
  DIFF_BASE="$CUR_SHA^"
  DIFF_CUR="$CUR_SHA"
