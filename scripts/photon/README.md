# Polingual: vectorization + query backbone
...
# build-time only: wordfreq powers the commonness ranking. Not a runtime
...
- **Semantic model**: `sentence-transformers/LaBSE` (768-d, Apache-2.0,
 ...
 primary-sense gloss"** (the dominant Kaikki sense, replacing the old joined blob) so
...
- Per-language masks for the semantic/translate axes are precomputed + cached so
 a hot language pays the O(n) cost once, amortized across requests.
...
- Brute-force cosine is still fine at ~209k (single matmul, ~10-30 ms). An ANN
 ...
 a separate future path, pgvector, one layer beyond this local index). If shipped to a
