...
2. **Per-claim citeable canonical URLs + structured data** (JSON-LD
   ...
   an agent or a search engine cites a *stable foundation page*, not a
...
3. **The "refused → fixed" write-up as the earned-media + credibility anchor** —
   ...
   infra writer, a funder doing diligence) can point at honestly.
...
6. **A canonical "what canon is / how it is sourced / provenance & epistemics"
   page** so every backlink lands on *integrity*, not a raw query — and so the
...
**The gate is the strategy's first-class constraint, not a footnote.**
...
## 0. Operating principles (apply to every tactic)
...
- **Backlinks must attribute, not just link.** A link that does not carry the
  canonical claim URL + license is traffic, not attribution. We optimise for
  *attribution that survives being cited by an agent*, which is the only
  durable backlink in an agent-era research economy.
- **Land on integrity, not on a query.** Every inbound link resolves to a
...
*found* by registering pointers where agents and crawlers actually look.
...
- **Where to register pointers** (each is a *pointer to the canonical surface*,
  ...
  - **Dataset registries** (Hugging Face datasets, Data.gov-style open-data
    ...
    citeable. This is the highest-leverage *post-P0* registration and the most
    damaging *pre-P0* one.
- **Maintenance owner.** This is `bkt-epic-gtm` standing work, not one-shot:
...
re-crawled, not re-verified. The canon-quality bar: flagship query + one
...
asset a careful, skeptical third party can link to *honestly* — it leads with
...
- **It is the canonical inbound target for credibility links** — not the
  landing page, not a query. "Here is a research-integrity org that published
  the AI refusal that taught it, and the fix" is linkable; "here is a research
  API" is not.
- **Who would link it, honestly pitched (no hype, the quote carries it):**
...
- **Copy-citation affordance** on every claim page and in every envelope
  ...
  protocol's defined citation action ("copy the `citation` block verbatim");
...
- No tracking, no analytics beacon in the embed — a research-integrity
  ...
  attribution infrastructure, not a growth pixel.
...
## 5. Inbound cross-link map (properties we already control)
...
|---|---|---|---|
| **gianyrox.com** (founder site) → bucket | `/manifesto` then `/protocol/agent-trust` | "bucket.foundation — the nonprofit research-canon protocol I maintain" | Founder/COI is already disclosed in `GOVERNANCE.md`; the link must mirror that disclosure, not obscure it. |
| **Book — *Your Money Is Broken*** (gianyrox.com book page / back-matter) → bucket | `/protocol` and `/cite-forever/v0.1` | "the x402 research-payment rail (open feed402 protocol)" | The book is about stablecoins/x402 on Base — feed402 is a concrete, honest application. Link the *protocol*, not the canon, until P0. |
...
| **bucket → out** (reciprocal, in `llms.txt` "Source" + footers) | feed402, x402-research-gateway, gianyrox.com | existing factual anchors (already present in `llms.txt`) | Keep reciprocal so the graph is corroborating, not circular-looking. |
...
**What.** A single, stable page that is the **honest landing surface for every
credibility backlink**. It exists so a careful reader who follows any inbound
link arrives at *what the canon is and how it is built*, not at a raw query
...
05-biophysics, not its centre; outcomes are downstream applications, not
...
- **The Kruse epistemics, explicitly**: the Kruse corpus is *one partial
  source* feeding the 05-biophysics branch, not its centre; it is labelled,
...
This page is the **link target for §3 (credibility), §5 (cross-links), and
...
exact opposite converts an honest page into a documented contradiction (the
...
sentence on it. The canon-quality bar is therefore *literally the text of this
...
research; transcript material is correctly demoted/labelled/excluded) → **go**
...
## 7. Posting cadence & channels (X, LinkedIn) — sequencing only
...
4. **No standalone "we built an API" post, ever.** The post is always about an
   *idea* (the refusal that became a spec; what a foundation is), never a
   product announcement. This is a nonprofit voice constraint, not a tactic
   choice.
...
in `EMBARGO.md`) → multiply the output (this playbook). This document is the
