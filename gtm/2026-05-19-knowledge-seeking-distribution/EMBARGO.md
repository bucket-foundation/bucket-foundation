...
"directories don't count." The gate is the gate.
...
- evidence including `canon:mitochondria/003-because-guess-what` ("the guy that
 ...
 DNA") and 0 ("99 of the NIH Budget"), misheard transcription-error
...
- [ ] **Cross-branch flagship spot-check.** One flagship/demo query per branch
 (01-07 minimum) returns something a domain scientist would cite,
 a real paper, law, or primary derivation instead of an auto-transcript chunk,
 not "served from canon" emptiness. Queries + responses captured to a
 dated fixture file. Verified by: __________ on __________
- [ ] **Kruse demoted.** Any Kruse-corpus-derived material is labelled
...
- [ ] **Structured data validates.** Every JSON-LD block shipped (PLAYBOOK.md
 ...
 canonical per-claim page that returns real canon instead of a raw query string.
...
Endpoint, never lower, they are exactly what agents cache and trust).
...
bearing reason this is a *hard* gate:
