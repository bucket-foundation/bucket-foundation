...
Out*. The checklist here is the gate.
...
- [ ] **On-camera zero-key citation verified.** A real screen recording exists
 ...
 `data`, real `citation`, `demo: false`), with no mock and no staged JSON
...
|---|---|---|
| "AI agents pay to cite our research" | A real agent has paid and cited zero-key, on camera | "Agents can *discover and cite* the canon zero-key; the citation fee routes to the author by design." Do not imply autonomous agent payment is happening. |
...
| "The protocol is fixed / solved" | bkt-1 and bkt-2 both closed AND the after-tests pass | "We changed the protocol so paid-to-cite is provably not pay-to-proceed." Describe the change. |
...
- Nonprofit voice throughout. Precise, humble, technically credible.
...
<!-- voice-ignore-next 1: quotes the banned hype adjectives it forbids -->
