...
# Longtail video — "We showed our research protocol to the most careful AI we could find. It refused. Then we fixed it."
...
## Title options (pick one; all non-hype, nonprofit voice)
...
4. **"build the past. build history. — and the refusal that made the protocol honest."**

Recommended: **#2** for Longtail/YouTube, **#3** as the X video caption.

---

## Narrative spine (do not deviate)

This is the honest **refused → fixed** arc. Not "watch agents pay to cite"
...
3. ChatGPT: *discovery worked* — found `llms.txt` → feed402 manifest →
   ...
   But it tried to pay the x402 challenge itself, had no wallet, and honestly
...
## CUT A — 60–90s (X / LinkedIn native / Longtail short)
...
| A1 | 0:05–0:14 | Stone card: **build the past.** Then under it, one line: *feed402 · USDC on Base · author payout ≥80%*. | "Pay once. Cite forever. The author gets paid, not a publisher. That is the whole machine." |
...
| A6 | 1:00–1:12 | **[GATED bkt-1+bkt-2]** Screen-capture: zero-key request, HTTP **200**, real `data`, `citation`, `demo: false`. Caption: *agent never holds a wallet. agent never sees a challenge.* | "The agent never holds a wallet. The agent never sees a payment challenge. Citation is a passive record, not an action. Paid-to-cite is now provably not pay-to-proceed." |
...
## CUT B — 3–4 min (Longtail long / YouTube / funder share)

Same spine, room to breathe and to show the technical substance. ~520–620 words.

### Scene 1 — What we built (0:00–0:35)
...
- **VO:** "Research access is broken in a specific, boring way. Readers pay
  ...
  it is citeable forever — and the citation fee routes to the author, not a
  publisher. Free to read. Paid to cite. That is the whole thesis."

### Scene 2 — The test (0:35–1:05)
...
### Scene 3 — Discovery worked (1:05–1:50)
...
- **VO:** "The discovery half worked, end to end. One agent read the
  ...
  canon, and correctly identified Peter Mitchell's 1961 chemiosmotic-coupling
...
### Scene 4 — The two failures (1:50–2:40)
...
- **VO:** "Then both agents stopped, for two different reasons, and both
  ...
  itself. It had no wallet, could not, and honestly declined to cite a paid
  result it never actually obtained. The second agent did something more
...
### Scene 5 — Why the refusal is the gift (2:40–3:10)
...
- **VO:** "We could have edited that out. For a project whose entire value is
  ...
  every safety-tuned agent will raise — stated more clearly than we could have
  stated it ourselves. So it became the specification for the fix."

### Scene 6 — The fix (3:10–3:45) — **[GATED bkt-1 + bkt-2]**
- **Shots:** **[GATED]** zero-key request → HTTP **200** → real envelope with
  ...
  citation is a record, not an action.* Then **[GATED]** `/llms-full.txt §4`
  trust-model section on screen.
- **VO:** "The fix is structural, not cosmetic. On the zero-key path the agent
  never holds a wallet and never sees a payment challenge — the infrastructure
  carries the cost, capped, server-side. Citation is a passive record that the
  author was credited, not an action the agent performs. The trust and payment
  model is written down in plain language where an agent will actually read
  it. Paid-to-cite is now provably distinct from pay-to-proceed."

### Scene 7 — The proof (3:45–4:00) — **[GATED bkt-2]**
...
## On-screen artifact checklist (all must be REAL captures)
