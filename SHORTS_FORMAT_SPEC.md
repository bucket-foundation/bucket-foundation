...
3. **Length follows payoff, not the clock.** 2026 educational sweet spot
...
## 2. Hard rules (the format contract)
...
| **Stack** | 0:35–0:45 | Open gap 2 → route to canon | "The deeper version lives at `bucket.foundation/canon/<id>`" (or Longtail shop, in priority order). Subtle, not hard-sell. |
...
## 4. Sub-brand palettes (per canon branch)
...
## 5. The 4-variant RLHF loop (the training mechanism)
...
when, across the last 3 batches, the human pick is consistently the
...
(human reviews 1 in N, not every batch). **Not v1.**
...
## 7. Hook variants (the randomized factor)
...
| `contrarian` | A wrong common belief, corrected by the reveal | "Most people think X. They're wrong. Here's what's actually happening." |
